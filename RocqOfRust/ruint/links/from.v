Require Import links.RocqOfRust.
Require Import core.convert.links.mod.
Require Import core.fmt.links.mod.
Require Import core.fmt.links.rt.
Require Import core.links.array.
Require Import core.links.panicking.
Require Import core.links.result.
Require Import core.num.links.error.
Require Import core.num.links.mod.
Require Import ruint.links.lib.
Require Import ruint.from.

(*
  pub enum ToUintError<T> {
      ValueTooLarge(usize, T),
      ValueNegative(usize, T),
      NotANumber(usize),
  }
*)
Module ToUintError.
  RocqOfRustLinkGenericEnum "ruint::from::ToUintError" [ T ] :=
  | ValueTooLarge (size : usize, value : T)
  | ValueNegative (size : usize, value : T)
  | NotANumber (size : usize)
  .
End ToUintError.
Export (hints) ToUintError.

(*
  pub trait UintTryFrom<T>: Sized {
      #[doc(hidden)]
      fn uint_try_from(value: T) -> Result<Self, ToUintError<Self>>;
  }
*)
Module UintTryFrom.
  Definition trait (Self T : Set) `{Link Self} `{Link T} : TraitHeader.t :=
    {|
      TraitHeader.trait_name := "ruint::from::UintTryFrom";
      TraitHeader.trait_consts := [];
      TraitHeader.trait_tys := [Φ T];
      TraitHeader.self_ty := Φ Self;
    |}.

  Class Method_uint_try_from (Self T : Set) `{Link Self} `{Link T} : Set := {
    uint_try_from : PolymorphicFunction.t;
    uint_try_from_is_method :: IsTraitMethod.C (trait Self T) "uint_try_from" uint_try_from;
    run_uint_try_from (value : T) :: Run.Trait uint_try_from [] [] [ φ value ] (Result.t Self (ToUintError.t Self));
  }.

  Class Run (Self : Set) (T : Set) `{Link Self} `{Link T} : Set := {
    method_uint_try_from :: Method_uint_try_from Self T;
  }.
End UintTryFrom.
Export (hints) UintTryFrom.

(*
  impl<const BITS: usize, const LIMBS: usize, T> UintTryFrom<T> for Uint<BITS, LIMBS>
  where
      Self: TryFrom<T, Error = ToUintError<Self>>,
*)
Module UintTryFrom_T_for_Uint_where_TryFrom.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Instance run_uint_try_from {BITS LIMBS : usize} {T : Set} `{Link T}
      `{run : !TryFrom.Run (Self BITS LIMBS) T (ToUintError.t (Self BITS LIMBS))}
      (value : T) :
    Run.Trait
      (from.Impl_ruint_from_UintTryFrom_where_core_convert_TryFrom_ruint_Uint_BITS_LIMBS_T_T_for_ruint_Uint_BITS_LIMBS.uint_try_from (φ BITS) (φ LIMBS) (Φ T))
      [] [] [φ value]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
    destruct run.
    run_symbolic.
  Defined.
  Global Opaque run_uint_try_from.

  Instance method_uint_try_from {BITS LIMBS : usize} {T : Set} `{Link T}
      `{!TryFrom.Run (Self BITS LIMBS) T (ToUintError.t (Self BITS LIMBS))} :
    UintTryFrom.Method_uint_try_from (Self BITS LIMBS) T.
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_ruint_from_UintTryFrom_where_core_convert_TryFrom_ruint_Uint_BITS_LIMBS_T_T_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { apply run_uint_try_from. }
  Defined.

  Instance run {BITS LIMBS : usize} {T : Set} `{Link T}
      `{!TryFrom.Run (Self BITS LIMBS) T (ToUintError.t (Self BITS LIMBS))} :
    UintTryFrom.Run (Self BITS LIMBS) T :=
  {}.
End UintTryFrom_T_for_Uint_where_TryFrom.
Export (hints) UintTryFrom_T_for_Uint_where_TryFrom.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<u64> for Uint<BITS, LIMBS> *)
Module TryFrom_u64_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Lemma Error_IsAssociated (BITS LIMBS : usize) :
    IsTraitAssociatedType
      "core::convert::TryFrom" [] [Φ u64] (Φ (Self BITS LIMBS)) "Error"
      (Φ (Error BITS LIMBS)).
  Proof.
    eexists; split.
    { apply from.Impl_core_convert_TryFrom_u64_for_ruint_Uint_BITS_LIMBS.Implements. }
    { reflexivity. }
  Qed.

  Instance run_try_from (BITS LIMBS : usize) (value : u64) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_u64_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
    (* FIX: infinite loop, might require giving a precise type definition for Uint *)
    (* run_symbolic. *)
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) u64 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_u64_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) u64 (Error BITS LIMBS) :=
  {}.
End TryFrom_u64_for_Uint.
Export (hints) TryFrom_u64_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<u128> for Uint<BITS, LIMBS> *)
Module TryFrom_u128_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : u128) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_u128_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) u128 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_u128_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) u128 (Error BITS LIMBS) :=
  {}.
End TryFrom_u128_for_Uint.
Export (hints) TryFrom_u128_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<bool> for Uint<BITS, LIMBS> *)
Module TryFrom_bool_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : bool) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_bool_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
    destruct (TryFrom_u64_for_Uint.run BITS LIMBS).
    run_symbolic.
  Defined.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) bool (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_bool_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) bool (Error BITS LIMBS) :=
  {}.
End TryFrom_bool_for_Uint.
Export (hints) TryFrom_bool_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<u8> for Uint<BITS, LIMBS> *)
Module TryFrom_u8_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : u8) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_u8_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
    destruct (TryFrom_u64_for_Uint.run BITS LIMBS).
    run_symbolic.
  Defined.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) u8 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_u8_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) u8 (Error BITS LIMBS) :=
  {}.
End TryFrom_u8_for_Uint.
Export (hints) TryFrom_u8_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<u16> for Uint<BITS, LIMBS> *)
Module TryFrom_u16_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : u16) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_u16_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
    destruct (TryFrom_u64_for_Uint.run BITS LIMBS).
    run_symbolic.
  Defined.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) u16 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_u16_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) u16 (Error BITS LIMBS) :=
  {}.
End TryFrom_u16_for_Uint.
Export (hints) TryFrom_u16_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<u32> for Uint<BITS, LIMBS> *)
Module TryFrom_u32_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : u32) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_u32_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
    destruct (TryFrom_u64_for_Uint.run BITS LIMBS).
    run_symbolic.
  Defined.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) u32 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_u32_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) u32 (Error BITS LIMBS) :=
  {}.
End TryFrom_u32_for_Uint.
Export (hints) TryFrom_u32_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<usize> for Uint<BITS, LIMBS> *)
Module TryFrom_usize_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : usize) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_usize_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
    destruct (TryFrom_u64_for_Uint.run BITS LIMBS).
    run_symbolic.
  Defined.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) usize (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_usize_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) usize (Error BITS LIMBS) :=
  {}.
End TryFrom_usize_for_Uint.
Export (hints) TryFrom_usize_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<i8> for Uint<BITS, LIMBS> *)
Module TryFrom_i8_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : i8) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_i8_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) i8 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_i8_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) i8 (Error BITS LIMBS) :=
  {}.
End TryFrom_i8_for_Uint.
Export (hints) TryFrom_i8_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<i16> for Uint<BITS, LIMBS> *)
Module TryFrom_i16_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : i16) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_i16_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) i16 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_i16_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) i16 (Error BITS LIMBS) :=
  {}.
End TryFrom_i16_for_Uint.
Export (hints) TryFrom_i16_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<i32> for Uint<BITS, LIMBS> *)
Module TryFrom_i32_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : i32) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_i32_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) i32 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_i32_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) i32 (Error BITS LIMBS) :=
  {}.
End TryFrom_i32_for_Uint.
Export (hints) TryFrom_i32_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<i64> for Uint<BITS, LIMBS> *)
Module TryFrom_i64_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : i64) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_i64_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) i64 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_i64_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) i64 (Error BITS LIMBS) :=
  {}.
End TryFrom_i64_for_Uint.
Export (hints) TryFrom_i64_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<i128> for Uint<BITS, LIMBS> *)
Module TryFrom_i128_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : i128) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_i128_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) i128 (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_i128_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) i128 (Error BITS LIMBS) :=
  {}.
End TryFrom_i128_for_Uint.
Export (hints) TryFrom_i128_for_Uint.

(* impl<const BITS: usize, const LIMBS: usize> TryFrom<isize> for Uint<BITS, LIMBS> *)
Module TryFrom_isize_for_Uint.
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  Definition Error (BITS LIMBS : usize) : Set :=
    ToUintError.t (Self BITS LIMBS).

  Instance run_try_from (BITS LIMBS : usize) (value : isize) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_isize_for_ruint_Uint_BITS_LIMBS.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t (Self BITS LIMBS) (ToUintError.t (Self BITS LIMBS))).
  Proof.
    constructor.
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from (Self BITS LIMBS) isize (Error BITS LIMBS).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_isize_for_ruint_Uint_BITS_LIMBS.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run (Self BITS LIMBS) isize (Error BITS LIMBS) :=
  {}.
End TryFrom_isize_for_Uint.
Export (hints) TryFrom_isize_for_Uint.

Module Impl_Uint.
  (* Uint<BITS, LIMBS> *)
  Definition Self (BITS LIMBS : usize) : Set :=
    Uint.t BITS LIMBS.

  (*
  pub fn from<T>(value: T) -> Self
        where
            Self: UintTryFrom<T>,
  *)
  Instance run_from
    (BITS LIMBS : usize)
    (T : Set) `{Link T}
    {run_UintTryFrom_for_Self : UintTryFrom.Run (Self BITS LIMBS) T}
    (value : T) :
    Run.Trait
      (from.Impl_ruint_Uint_BITS_LIMBS.from (φ BITS) (φ LIMBS)) [] [ Φ T ] [ φ value ]
      (Self BITS LIMBS).
  Proof.
    constructor.
    destruct run_UintTryFrom_for_Self.
    run_symbolic.
    apply (Impl_Arguments.run_new_byte_str
      {| Integer.value := 26 |}
      {| Integer.value := 1 |}).
  Defined.
  Global Opaque run_from.
End Impl_Uint.
Export (hints) Impl_Uint.

(*
pub enum FromUintError<T> {
    Overflow(usize, T, T),
}
*)
Module FromUintError.
  Inductive t (T : Set) : Set :=
  | Overflow (size : usize) (value : T) (max : T).
  Arguments Overflow {T} size value max.

  Parameter to_value : forall {T : Set}, t T -> Value.t.

  Instance IsLink (T : Set) `{Link T} : Link (t T) :=
  {
    Φ := Ty.apply (Ty.path "ruint::from::FromUintError") [] [ Φ T ];
    φ := to_value;
  }.

  Definition of_ty (T_ty : Ty.t) :
    OfTy.t T_ty ->
    OfTy.t (Ty.apply (Ty.path "ruint::from::FromUintError") [] [ T_ty ]).
  Proof.
    intros [T].
    eapply OfTy.Make with (A := t T).
    subst.
    reflexivity.
  Defined.
  Smpl Add eapply of_ty : of_ty.
End FromUintError.
Export (hints) FromUintError.

Module TryFrom_Uint_for_u64.
  Definition Self : Set :=
    u64.

  Instance run_try_from (BITS LIMBS : usize) (value : Impl_Uint.Self BITS LIMBS) :
    Run.Trait
      (from.Impl_core_convert_TryFrom_ruint_Uint_BITS_LIMBS_for_u64.try_from (φ BITS) (φ LIMBS))
      [] [] [ φ value ]
      (Result.t Self (FromUintError.t u64)).
  Proof.
    constructor.
    run_symbolic.
  Admitted.
  Global Opaque run_try_from.

  Instance method_try_from {BITS LIMBS : usize} :
    TryFrom.Method_try_from Self (Impl_Uint.Self BITS LIMBS) (FromUintError.t u64).
  Proof.
    eexists.
    { constructor.
      eapply IsTraitMethod.Defined.
      { apply from.Impl_core_convert_TryFrom_ruint_Uint_BITS_LIMBS_for_u64.Implements. }
      { reflexivity. }
    }
    { typeclasses eauto. }
  Defined.

  Instance run (BITS LIMBS : usize) :
    TryFrom.Run Self (Impl_Uint.Self BITS LIMBS) (FromUintError.t u64) :=
  {}.
End TryFrom_Uint_for_u64.
Export (hints) TryFrom_Uint_for_u64.
