(* Generated by rocq-of-rust *)
Require Import RocqOfRust.RocqOfRust.

Module slice.
  Module rotate.
    Axiom BufType :
      (Ty.path "core::slice::rotate::BufType") =
        (Ty.apply (Ty.path "array") [ Value.Integer IntegerKind.Usize 32 ] [ Ty.path "usize" ]).
    
    (*
    pub(super) const unsafe fn ptr_rotate<T>(left: usize, mid: *mut T, right: usize) {
        if T::IS_ZST {
            return;
        }
        // abort early if the rotate is a no-op
        if (left == 0) || (right == 0) {
            return;
        }
        // `T` is not a zero-sized type, so it's okay to divide by its size.
        if !cfg!(feature = "optimize_for_size")
            // FIXME(const-hack): Use cmp::min when available in const
            && const_min(left, right) <= size_of::<BufType>() / size_of::<T>()
        {
            // SAFETY: guaranteed by the caller
            unsafe { ptr_rotate_memmove(left, mid, right) };
        } else if !cfg!(feature = "optimize_for_size")
            && ((left + right < 24) || (size_of::<T>() > size_of::<[usize; 4]>()))
        {
            // SAFETY: guaranteed by the caller
            unsafe { ptr_rotate_gcd(left, mid, right) }
        } else {
            // SAFETY: guaranteed by the caller
            unsafe { ptr_rotate_swap(left, mid, right) }
        }
    }
    *)
    Definition ptr_rotate (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
      match ε, τ, α with
      | [], [ T ], [ _ as left; mid; _ as right ] =>
        ltac:(M.monadic
          (let left := M.alloc (| Ty.path "usize", left |) in
          let mid := M.alloc (| Ty.apply (Ty.path "*mut") [] [ T ], mid |) in
          let right := M.alloc (| Ty.path "usize", right |) in
          M.catch_return (Ty.tuple []) (|
            ltac:(M.monadic
              (M.read (|
                let~ _ : Ty.tuple [] :=
                  M.match_operator (|
                    Ty.tuple [],
                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                    [
                      fun γ =>
                        ltac:(M.monadic
                          (let γ :=
                            get_constant (|
                              "core::mem::SizedTypeProperties::IS_ZST",
                              Ty.path "bool"
                            |) in
                          let _ :=
                            is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                          M.never_to_any (| M.read (| M.return_ (| Value.Tuple [] |) |) |)));
                      fun γ => ltac:(M.monadic (Value.Tuple []))
                    ]
                  |) in
                let~ _ : Ty.tuple [] :=
                  M.match_operator (|
                    Ty.tuple [],
                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                    [
                      fun γ =>
                        ltac:(M.monadic
                          (let γ :=
                            M.alloc (|
                              Ty.path "bool",
                              LogicalOp.or (|
                                M.call_closure (|
                                  Ty.path "bool",
                                  BinOp.eq,
                                  [ M.read (| left |); Value.Integer IntegerKind.Usize 0 ]
                                |),
                                ltac:(M.monadic
                                  (M.call_closure (|
                                    Ty.path "bool",
                                    BinOp.eq,
                                    [ M.read (| right |); Value.Integer IntegerKind.Usize 0 ]
                                  |)))
                              |)
                            |) in
                          let _ :=
                            is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                          M.never_to_any (| M.read (| M.return_ (| Value.Tuple [] |) |) |)));
                      fun γ => ltac:(M.monadic (Value.Tuple []))
                    ]
                  |) in
                M.alloc (|
                  Ty.tuple [],
                  M.match_operator (|
                    Ty.tuple [],
                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                    [
                      fun γ =>
                        ltac:(M.monadic
                          (let γ :=
                            M.alloc (|
                              Ty.path "bool",
                              M.call_closure (| Ty.path "bool", UnOp.not, [ Value.Bool false ] |)
                            |) in
                          let _ :=
                            is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                          let γ :=
                            M.alloc (|
                              Ty.path "bool",
                              M.call_closure (|
                                Ty.path "bool",
                                BinOp.le,
                                [
                                  M.call_closure (|
                                    Ty.path "usize",
                                    M.get_function (| "core::slice::rotate::const_min", [], [] |),
                                    [ M.read (| left |); M.read (| right |) ]
                                  |);
                                  M.call_closure (|
                                    Ty.path "usize",
                                    BinOp.Wrap.div,
                                    [
                                      M.call_closure (|
                                        Ty.path "usize",
                                        M.get_function (|
                                          "core::mem::size_of",
                                          [],
                                          [
                                            Ty.apply
                                              (Ty.path "array")
                                              [ Value.Integer IntegerKind.Usize 32 ]
                                              [ Ty.path "usize" ]
                                          ]
                                        |),
                                        []
                                      |);
                                      M.call_closure (|
                                        Ty.path "usize",
                                        M.get_function (| "core::mem::size_of", [], [ T ] |),
                                        []
                                      |)
                                    ]
                                  |)
                                ]
                              |)
                            |) in
                          let _ :=
                            is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                          M.read (|
                            let~ _ : Ty.tuple [] :=
                              M.call_closure (|
                                Ty.tuple [],
                                M.get_function (|
                                  "core::slice::rotate::ptr_rotate_memmove",
                                  [],
                                  [ T ]
                                |),
                                [ M.read (| left |); M.read (| mid |); M.read (| right |) ]
                              |) in
                            M.alloc (| Ty.tuple [], Value.Tuple [] |)
                          |)));
                      fun γ =>
                        ltac:(M.monadic
                          (M.match_operator (|
                            Ty.tuple [],
                            M.alloc (| Ty.tuple [], Value.Tuple [] |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let γ :=
                                    M.alloc (|
                                      Ty.path "bool",
                                      M.call_closure (|
                                        Ty.path "bool",
                                        UnOp.not,
                                        [ Value.Bool false ]
                                      |)
                                    |) in
                                  let _ :=
                                    is_constant_or_break_match (|
                                      M.read (| γ |),
                                      Value.Bool true
                                    |) in
                                  let γ :=
                                    M.alloc (|
                                      Ty.path "bool",
                                      LogicalOp.or (|
                                        M.call_closure (|
                                          Ty.path "bool",
                                          BinOp.lt,
                                          [
                                            M.call_closure (|
                                              Ty.path "usize",
                                              BinOp.Wrap.add,
                                              [ M.read (| left |); M.read (| right |) ]
                                            |);
                                            Value.Integer IntegerKind.Usize 24
                                          ]
                                        |),
                                        ltac:(M.monadic
                                          (M.call_closure (|
                                            Ty.path "bool",
                                            BinOp.gt,
                                            [
                                              M.call_closure (|
                                                Ty.path "usize",
                                                M.get_function (|
                                                  "core::mem::size_of",
                                                  [],
                                                  [ T ]
                                                |),
                                                []
                                              |);
                                              M.call_closure (|
                                                Ty.path "usize",
                                                M.get_function (|
                                                  "core::mem::size_of",
                                                  [],
                                                  [
                                                    Ty.apply
                                                      (Ty.path "array")
                                                      [ Value.Integer IntegerKind.Usize 4 ]
                                                      [ Ty.path "usize" ]
                                                  ]
                                                |),
                                                []
                                              |)
                                            ]
                                          |)))
                                      |)
                                    |) in
                                  let _ :=
                                    is_constant_or_break_match (|
                                      M.read (| γ |),
                                      Value.Bool true
                                    |) in
                                  M.call_closure (|
                                    Ty.tuple [],
                                    M.get_function (|
                                      "core::slice::rotate::ptr_rotate_gcd",
                                      [],
                                      [ T ]
                                    |),
                                    [ M.read (| left |); M.read (| mid |); M.read (| right |) ]
                                  |)));
                              fun γ =>
                                ltac:(M.monadic
                                  (M.call_closure (|
                                    Ty.tuple [],
                                    M.get_function (|
                                      "core::slice::rotate::ptr_rotate_swap",
                                      [],
                                      [ T ]
                                    |),
                                    [ M.read (| left |); M.read (| mid |); M.read (| right |) ]
                                  |)))
                            ]
                          |)))
                    ]
                  |)
                |)
              |)))
          |)))
      | _, _, _ => M.impossible "wrong number of arguments"
      end.
    
    Global Instance Instance_IsFunction_ptr_rotate :
      M.IsFunction.C "core::slice::rotate::ptr_rotate" ptr_rotate.
    Admitted.
    Global Typeclasses Opaque ptr_rotate.
    
    (*
    const unsafe fn ptr_rotate_memmove<T>(left: usize, mid: *mut T, right: usize) {
        // The `[T; 0]` here is to ensure this is appropriately aligned for T
        let mut rawarray = MaybeUninit::<(BufType, [T; 0])>::uninit();
        let buf = rawarray.as_mut_ptr() as *mut T;
        // SAFETY: `mid-left <= mid-left+right < mid+right`
        let dim = unsafe { mid.sub(left).add(right) };
        if left <= right {
            // SAFETY:
            //
            // 1) The `if` condition about the sizes ensures `[mid-left; left]` will fit in
            //    `buf` without overflow and `buf` was created just above and so cannot be
            //    overlapped with any value of `[mid-left; left]`
            // 2) [mid-left, mid+right) are all valid for reading and writing and we don't care
            //    about overlaps here.
            // 3) The `if` condition about `left <= right` ensures writing `left` elements to
            //    `dim = mid-left+right` is valid because:
            //    - `buf` is valid and `left` elements were written in it in 1)
            //    - `dim+left = mid-left+right+left = mid+right` and we write `[dim, dim+left)`
            unsafe {
                // 1)
                ptr::copy_nonoverlapping(mid.sub(left), buf, left);
                // 2)
                ptr::copy(mid, mid.sub(left), right);
                // 3)
                ptr::copy_nonoverlapping(buf, dim, left);
            }
        } else {
            // SAFETY: same reasoning as above but with `left` and `right` reversed
            unsafe {
                ptr::copy_nonoverlapping(mid, buf, right);
                ptr::copy(mid.sub(left), dim, left);
                ptr::copy_nonoverlapping(buf, mid.sub(left), right);
            }
        }
    }
    *)
    Definition ptr_rotate_memmove (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
      match ε, τ, α with
      | [], [ T ], [ _ as left; mid; _ as right ] =>
        ltac:(M.monadic
          (let left := M.alloc (| Ty.path "usize", left |) in
          let mid := M.alloc (| Ty.apply (Ty.path "*mut") [] [ T ], mid |) in
          let right := M.alloc (| Ty.path "usize", right |) in
          M.read (|
            let~ rawarray :
                Ty.apply
                  (Ty.path "core::mem::maybe_uninit::MaybeUninit")
                  []
                  [
                    Ty.tuple
                      [
                        Ty.apply
                          (Ty.path "array")
                          [ Value.Integer IntegerKind.Usize 32 ]
                          [ Ty.path "usize" ];
                        Ty.apply (Ty.path "array") [ Value.Integer IntegerKind.Usize 0 ] [ T ]
                      ]
                  ] :=
              M.call_closure (|
                Ty.apply
                  (Ty.path "core::mem::maybe_uninit::MaybeUninit")
                  []
                  [
                    Ty.tuple
                      [
                        Ty.apply
                          (Ty.path "array")
                          [ Value.Integer IntegerKind.Usize 32 ]
                          [ Ty.path "usize" ];
                        Ty.apply (Ty.path "array") [ Value.Integer IntegerKind.Usize 0 ] [ T ]
                      ]
                  ],
                M.get_associated_function (|
                  Ty.apply
                    (Ty.path "core::mem::maybe_uninit::MaybeUninit")
                    []
                    [
                      Ty.tuple
                        [
                          Ty.apply
                            (Ty.path "array")
                            [ Value.Integer IntegerKind.Usize 32 ]
                            [ Ty.path "usize" ];
                          Ty.apply (Ty.path "array") [ Value.Integer IntegerKind.Usize 0 ] [ T ]
                        ]
                    ],
                  "uninit",
                  [],
                  []
                |),
                []
              |) in
            let~ buf : Ty.apply (Ty.path "*mut") [] [ T ] :=
              M.cast
                (Ty.apply (Ty.path "*mut") [] [ T ])
                (M.call_closure (|
                  Ty.apply
                    (Ty.path "*mut")
                    []
                    [
                      Ty.tuple
                        [
                          Ty.apply
                            (Ty.path "array")
                            [ Value.Integer IntegerKind.Usize 32 ]
                            [ Ty.path "usize" ];
                          Ty.apply (Ty.path "array") [ Value.Integer IntegerKind.Usize 0 ] [ T ]
                        ]
                    ],
                  M.get_associated_function (|
                    Ty.apply
                      (Ty.path "core::mem::maybe_uninit::MaybeUninit")
                      []
                      [
                        Ty.tuple
                          [
                            Ty.apply
                              (Ty.path "array")
                              [ Value.Integer IntegerKind.Usize 32 ]
                              [ Ty.path "usize" ];
                            Ty.apply (Ty.path "array") [ Value.Integer IntegerKind.Usize 0 ] [ T ]
                          ]
                      ],
                    "as_mut_ptr",
                    [],
                    []
                  |),
                  [ M.borrow (| Pointer.Kind.MutRef, rawarray |) ]
                |)) in
            let~ dim : Ty.apply (Ty.path "*mut") [] [ T ] :=
              M.call_closure (|
                Ty.apply (Ty.path "*mut") [] [ T ],
                M.get_associated_function (| Ty.apply (Ty.path "*mut") [] [ T ], "add", [], [] |),
                [
                  M.call_closure (|
                    Ty.apply (Ty.path "*mut") [] [ T ],
                    M.get_associated_function (|
                      Ty.apply (Ty.path "*mut") [] [ T ],
                      "sub",
                      [],
                      []
                    |),
                    [ M.read (| mid |); M.read (| left |) ]
                  |);
                  M.read (| right |)
                ]
              |) in
            M.alloc (|
              Ty.tuple [],
              M.match_operator (|
                Ty.tuple [],
                M.alloc (| Ty.tuple [], Value.Tuple [] |),
                [
                  fun γ =>
                    ltac:(M.monadic
                      (let γ :=
                        M.alloc (|
                          Ty.path "bool",
                          M.call_closure (|
                            Ty.path "bool",
                            BinOp.le,
                            [ M.read (| left |); M.read (| right |) ]
                          |)
                        |) in
                      let _ := is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                      M.read (|
                        let~ _ : Ty.tuple [] :=
                          M.call_closure (|
                            Ty.tuple [],
                            M.get_function (| "core::ptr::copy_nonoverlapping", [], [ T ] |),
                            [
                              M.call_closure (|
                                Ty.apply (Ty.path "*const") [] [ T ],
                                M.pointer_coercion
                                  M.PointerCoercion.MutToConstPointer
                                  (Ty.apply (Ty.path "*mut") [] [ T ])
                                  (Ty.apply (Ty.path "*const") [] [ T ]),
                                [
                                  M.call_closure (|
                                    Ty.apply (Ty.path "*mut") [] [ T ],
                                    M.get_associated_function (|
                                      Ty.apply (Ty.path "*mut") [] [ T ],
                                      "sub",
                                      [],
                                      []
                                    |),
                                    [ M.read (| mid |); M.read (| left |) ]
                                  |)
                                ]
                              |);
                              M.read (| buf |);
                              M.read (| left |)
                            ]
                          |) in
                        let~ _ : Ty.tuple [] :=
                          M.call_closure (|
                            Ty.tuple [],
                            M.get_function (| "core::ptr::copy", [], [ T ] |),
                            [
                              M.call_closure (|
                                Ty.apply (Ty.path "*const") [] [ T ],
                                M.pointer_coercion
                                  M.PointerCoercion.MutToConstPointer
                                  (Ty.apply (Ty.path "*mut") [] [ T ])
                                  (Ty.apply (Ty.path "*const") [] [ T ]),
                                [ M.read (| mid |) ]
                              |);
                              M.call_closure (|
                                Ty.apply (Ty.path "*mut") [] [ T ],
                                M.get_associated_function (|
                                  Ty.apply (Ty.path "*mut") [] [ T ],
                                  "sub",
                                  [],
                                  []
                                |),
                                [ M.read (| mid |); M.read (| left |) ]
                              |);
                              M.read (| right |)
                            ]
                          |) in
                        let~ _ : Ty.tuple [] :=
                          M.call_closure (|
                            Ty.tuple [],
                            M.get_function (| "core::ptr::copy_nonoverlapping", [], [ T ] |),
                            [
                              M.call_closure (|
                                Ty.apply (Ty.path "*const") [] [ T ],
                                M.pointer_coercion
                                  M.PointerCoercion.MutToConstPointer
                                  (Ty.apply (Ty.path "*mut") [] [ T ])
                                  (Ty.apply (Ty.path "*const") [] [ T ]),
                                [ M.read (| buf |) ]
                              |);
                              M.read (| dim |);
                              M.read (| left |)
                            ]
                          |) in
                        M.alloc (| Ty.tuple [], Value.Tuple [] |)
                      |)));
                  fun γ =>
                    ltac:(M.monadic
                      (M.read (|
                        let~ _ : Ty.tuple [] :=
                          M.call_closure (|
                            Ty.tuple [],
                            M.get_function (| "core::ptr::copy_nonoverlapping", [], [ T ] |),
                            [
                              M.call_closure (|
                                Ty.apply (Ty.path "*const") [] [ T ],
                                M.pointer_coercion
                                  M.PointerCoercion.MutToConstPointer
                                  (Ty.apply (Ty.path "*mut") [] [ T ])
                                  (Ty.apply (Ty.path "*const") [] [ T ]),
                                [ M.read (| mid |) ]
                              |);
                              M.read (| buf |);
                              M.read (| right |)
                            ]
                          |) in
                        let~ _ : Ty.tuple [] :=
                          M.call_closure (|
                            Ty.tuple [],
                            M.get_function (| "core::ptr::copy", [], [ T ] |),
                            [
                              M.call_closure (|
                                Ty.apply (Ty.path "*const") [] [ T ],
                                M.pointer_coercion
                                  M.PointerCoercion.MutToConstPointer
                                  (Ty.apply (Ty.path "*mut") [] [ T ])
                                  (Ty.apply (Ty.path "*const") [] [ T ]),
                                [
                                  M.call_closure (|
                                    Ty.apply (Ty.path "*mut") [] [ T ],
                                    M.get_associated_function (|
                                      Ty.apply (Ty.path "*mut") [] [ T ],
                                      "sub",
                                      [],
                                      []
                                    |),
                                    [ M.read (| mid |); M.read (| left |) ]
                                  |)
                                ]
                              |);
                              M.read (| dim |);
                              M.read (| left |)
                            ]
                          |) in
                        let~ _ : Ty.tuple [] :=
                          M.call_closure (|
                            Ty.tuple [],
                            M.get_function (| "core::ptr::copy_nonoverlapping", [], [ T ] |),
                            [
                              M.call_closure (|
                                Ty.apply (Ty.path "*const") [] [ T ],
                                M.pointer_coercion
                                  M.PointerCoercion.MutToConstPointer
                                  (Ty.apply (Ty.path "*mut") [] [ T ])
                                  (Ty.apply (Ty.path "*const") [] [ T ]),
                                [ M.read (| buf |) ]
                              |);
                              M.call_closure (|
                                Ty.apply (Ty.path "*mut") [] [ T ],
                                M.get_associated_function (|
                                  Ty.apply (Ty.path "*mut") [] [ T ],
                                  "sub",
                                  [],
                                  []
                                |),
                                [ M.read (| mid |); M.read (| left |) ]
                              |);
                              M.read (| right |)
                            ]
                          |) in
                        M.alloc (| Ty.tuple [], Value.Tuple [] |)
                      |)))
                ]
              |)
            |)
          |)))
      | _, _, _ => M.impossible "wrong number of arguments"
      end.
    
    Global Instance Instance_IsFunction_ptr_rotate_memmove :
      M.IsFunction.C "core::slice::rotate::ptr_rotate_memmove" ptr_rotate_memmove.
    Admitted.
    Global Typeclasses Opaque ptr_rotate_memmove.
    
    (*
    const unsafe fn ptr_rotate_gcd<T>(left: usize, mid: *mut T, right: usize) {
        // Algorithm 2
        // Microbenchmarks indicate that the average performance for random shifts is better all
        // the way until about `left + right == 32`, but the worst case performance breaks even
        // around 16. 24 was chosen as middle ground. If the size of `T` is larger than 4
        // `usize`s, this algorithm also outperforms other algorithms.
        // SAFETY: callers must ensure `mid - left` is valid for reading and writing.
        let x = unsafe { mid.sub(left) };
        // beginning of first round
        // SAFETY: see previous comment.
        let mut tmp: T = unsafe { x.read() };
        let mut i = right;
        // `gcd` can be found before hand by calculating `gcd(left + right, right)`,
        // but it is faster to do one loop which calculates the gcd as a side effect, then
        // doing the rest of the chunk
        let mut gcd = right;
        // benchmarks reveal that it is faster to swap temporaries all the way through instead
        // of reading one temporary once, copying backwards, and then writing that temporary at
        // the very end. This is possibly due to the fact that swapping or replacing temporaries
        // uses only one memory address in the loop instead of needing to manage two.
        loop {
            // [long-safety-expl]
            // SAFETY: callers must ensure `[left, left+mid+right)` are all valid for reading and
            // writing.
            //
            // - `i` start with `right` so `mid-left <= x+i = x+right = mid-left+right < mid+right`
            // - `i <= left+right-1` is always true
            //   - if `i < left`, `right` is added so `i < left+right` and on the next
            //     iteration `left` is removed from `i` so it doesn't go further
            //   - if `i >= left`, `left` is removed immediately and so it doesn't go further.
            // - overflows cannot happen for `i` since the function's safety contract ask for
            //   `mid+right-1 = x+left+right` to be valid for writing
            // - underflows cannot happen because `i` must be bigger or equal to `left` for
            //   a subtraction of `left` to happen.
            //
            // So `x+i` is valid for reading and writing if the caller respected the contract
            tmp = unsafe { x.add(i).replace(tmp) };
            // instead of incrementing `i` and then checking if it is outside the bounds, we
            // check if `i` will go outside the bounds on the next increment. This prevents
            // any wrapping of pointers or `usize`.
            if i >= left {
                i -= left;
                if i == 0 {
                    // end of first round
                    // SAFETY: tmp has been read from a valid source and x is valid for writing
                    // according to the caller.
                    unsafe { x.write(tmp) };
                    break;
                }
                // this conditional must be here if `left + right >= 15`
                if i < gcd {
                    gcd = i;
                }
            } else {
                i += right;
            }
        }
        // finish the chunk with more rounds
        // FIXME(const-hack): Use `for start in 1..gcd` when available in const
        let mut start = 1;
        while start < gcd {
            // SAFETY: `gcd` is at most equal to `right` so all values in `1..gcd` are valid for
            // reading and writing as per the function's safety contract, see [long-safety-expl]
            // above
            tmp = unsafe { x.add(start).read() };
            // [safety-expl-addition]
            //
            // Here `start < gcd` so `start < right` so `i < right+right`: `right` being the
            // greatest common divisor of `(left+right, right)` means that `left = right` so
            // `i < left+right` so `x+i = mid-left+i` is always valid for reading and writing
            // according to the function's safety contract.
            i = start + right;
            loop {
                // SAFETY: see [long-safety-expl] and [safety-expl-addition]
                tmp = unsafe { x.add(i).replace(tmp) };
                if i >= left {
                    i -= left;
                    if i == start {
                        // SAFETY: see [long-safety-expl] and [safety-expl-addition]
                        unsafe { x.add(start).write(tmp) };
                        break;
                    }
                } else {
                    i += right;
                }
            }
    
            start += 1;
        }
    }
    *)
    Definition ptr_rotate_gcd (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
      match ε, τ, α with
      | [], [ T ], [ _ as left; mid; _ as right ] =>
        ltac:(M.monadic
          (let left := M.alloc (| Ty.path "usize", left |) in
          let mid := M.alloc (| Ty.apply (Ty.path "*mut") [] [ T ], mid |) in
          let right := M.alloc (| Ty.path "usize", right |) in
          M.read (|
            let~ x : Ty.apply (Ty.path "*mut") [] [ T ] :=
              M.call_closure (|
                Ty.apply (Ty.path "*mut") [] [ T ],
                M.get_associated_function (| Ty.apply (Ty.path "*mut") [] [ T ], "sub", [], [] |),
                [ M.read (| mid |); M.read (| left |) ]
              |) in
            let~ tmp : T :=
              M.call_closure (|
                T,
                M.get_associated_function (| Ty.apply (Ty.path "*mut") [] [ T ], "read", [], [] |),
                [ M.read (| x |) ]
              |) in
            let~ i : Ty.path "usize" := M.read (| right |) in
            let~ gcd : Ty.path "usize" := M.read (| right |) in
            let~ _ : Ty.tuple [] :=
              M.read (|
                M.loop (|
                  Ty.tuple [],
                  ltac:(M.monadic
                    (let~ _ : Ty.tuple [] :=
                      M.write (|
                        tmp,
                        M.call_closure (|
                          T,
                          M.get_associated_function (|
                            Ty.apply (Ty.path "*mut") [] [ T ],
                            "replace",
                            [],
                            []
                          |),
                          [
                            M.call_closure (|
                              Ty.apply (Ty.path "*mut") [] [ T ],
                              M.get_associated_function (|
                                Ty.apply (Ty.path "*mut") [] [ T ],
                                "add",
                                [],
                                []
                              |),
                              [ M.read (| x |); M.read (| i |) ]
                            |);
                            M.read (| tmp |)
                          ]
                        |)
                      |) in
                    M.alloc (|
                      Ty.tuple [],
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ :=
                                M.alloc (|
                                  Ty.path "bool",
                                  M.call_closure (|
                                    Ty.path "bool",
                                    BinOp.ge,
                                    [ M.read (| i |); M.read (| left |) ]
                                  |)
                                |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.read (|
                                let~ _ : Ty.tuple [] :=
                                  let β := i in
                                  M.write (|
                                    β,
                                    M.call_closure (|
                                      Ty.path "usize",
                                      BinOp.Wrap.sub,
                                      [ M.read (| β |); M.read (| left |) ]
                                    |)
                                  |) in
                                let~ _ : Ty.tuple [] :=
                                  M.match_operator (|
                                    Ty.tuple [],
                                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                    [
                                      fun γ =>
                                        ltac:(M.monadic
                                          (let γ :=
                                            M.alloc (|
                                              Ty.path "bool",
                                              M.call_closure (|
                                                Ty.path "bool",
                                                BinOp.eq,
                                                [ M.read (| i |); Value.Integer IntegerKind.Usize 0
                                                ]
                                              |)
                                            |) in
                                          let _ :=
                                            is_constant_or_break_match (|
                                              M.read (| γ |),
                                              Value.Bool true
                                            |) in
                                          M.never_to_any (|
                                            M.read (|
                                              let~ _ : Ty.tuple [] :=
                                                M.call_closure (|
                                                  Ty.tuple [],
                                                  M.get_associated_function (|
                                                    Ty.apply (Ty.path "*mut") [] [ T ],
                                                    "write",
                                                    [],
                                                    []
                                                  |),
                                                  [ M.read (| x |); M.read (| tmp |) ]
                                                |) in
                                              M.break (||)
                                            |)
                                          |)));
                                      fun γ => ltac:(M.monadic (Value.Tuple []))
                                    ]
                                  |) in
                                M.alloc (|
                                  Ty.tuple [],
                                  M.match_operator (|
                                    Ty.tuple [],
                                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                    [
                                      fun γ =>
                                        ltac:(M.monadic
                                          (let γ :=
                                            M.alloc (|
                                              Ty.path "bool",
                                              M.call_closure (|
                                                Ty.path "bool",
                                                BinOp.lt,
                                                [ M.read (| i |); M.read (| gcd |) ]
                                              |)
                                            |) in
                                          let _ :=
                                            is_constant_or_break_match (|
                                              M.read (| γ |),
                                              Value.Bool true
                                            |) in
                                          M.read (|
                                            let~ _ : Ty.tuple [] :=
                                              M.write (| gcd, M.read (| i |) |) in
                                            M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                          |)));
                                      fun γ => ltac:(M.monadic (Value.Tuple []))
                                    ]
                                  |)
                                |)
                              |)));
                          fun γ =>
                            ltac:(M.monadic
                              (M.read (|
                                let~ _ : Ty.tuple [] :=
                                  let β := i in
                                  M.write (|
                                    β,
                                    M.call_closure (|
                                      Ty.path "usize",
                                      BinOp.Wrap.add,
                                      [ M.read (| β |); M.read (| right |) ]
                                    |)
                                  |) in
                                M.alloc (| Ty.tuple [], Value.Tuple [] |)
                              |)))
                        ]
                      |)
                    |)))
                |)
              |) in
            let~ start : Ty.path "usize" := Value.Integer IntegerKind.Usize 1 in
            M.loop (|
              Ty.tuple [],
              ltac:(M.monadic
                (M.alloc (|
                  Ty.tuple [],
                  M.match_operator (|
                    Ty.tuple [],
                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                    [
                      fun γ =>
                        ltac:(M.monadic
                          (let γ :=
                            M.alloc (|
                              Ty.path "bool",
                              M.call_closure (|
                                Ty.path "bool",
                                BinOp.lt,
                                [ M.read (| start |); M.read (| gcd |) ]
                              |)
                            |) in
                          let _ :=
                            is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                          M.read (|
                            let~ _ : Ty.tuple [] :=
                              M.write (|
                                tmp,
                                M.call_closure (|
                                  T,
                                  M.get_associated_function (|
                                    Ty.apply (Ty.path "*mut") [] [ T ],
                                    "read",
                                    [],
                                    []
                                  |),
                                  [
                                    M.call_closure (|
                                      Ty.apply (Ty.path "*mut") [] [ T ],
                                      M.get_associated_function (|
                                        Ty.apply (Ty.path "*mut") [] [ T ],
                                        "add",
                                        [],
                                        []
                                      |),
                                      [ M.read (| x |); M.read (| start |) ]
                                    |)
                                  ]
                                |)
                              |) in
                            let~ _ : Ty.tuple [] :=
                              M.write (|
                                i,
                                M.call_closure (|
                                  Ty.path "usize",
                                  BinOp.Wrap.add,
                                  [ M.read (| start |); M.read (| right |) ]
                                |)
                              |) in
                            let~ _ : Ty.tuple [] :=
                              M.read (|
                                M.loop (|
                                  Ty.tuple [],
                                  ltac:(M.monadic
                                    (let~ _ : Ty.tuple [] :=
                                      M.write (|
                                        tmp,
                                        M.call_closure (|
                                          T,
                                          M.get_associated_function (|
                                            Ty.apply (Ty.path "*mut") [] [ T ],
                                            "replace",
                                            [],
                                            []
                                          |),
                                          [
                                            M.call_closure (|
                                              Ty.apply (Ty.path "*mut") [] [ T ],
                                              M.get_associated_function (|
                                                Ty.apply (Ty.path "*mut") [] [ T ],
                                                "add",
                                                [],
                                                []
                                              |),
                                              [ M.read (| x |); M.read (| i |) ]
                                            |);
                                            M.read (| tmp |)
                                          ]
                                        |)
                                      |) in
                                    M.alloc (|
                                      Ty.tuple [],
                                      M.match_operator (|
                                        Ty.tuple [],
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                        [
                                          fun γ =>
                                            ltac:(M.monadic
                                              (let γ :=
                                                M.alloc (|
                                                  Ty.path "bool",
                                                  M.call_closure (|
                                                    Ty.path "bool",
                                                    BinOp.ge,
                                                    [ M.read (| i |); M.read (| left |) ]
                                                  |)
                                                |) in
                                              let _ :=
                                                is_constant_or_break_match (|
                                                  M.read (| γ |),
                                                  Value.Bool true
                                                |) in
                                              M.read (|
                                                let~ _ : Ty.tuple [] :=
                                                  let β := i in
                                                  M.write (|
                                                    β,
                                                    M.call_closure (|
                                                      Ty.path "usize",
                                                      BinOp.Wrap.sub,
                                                      [ M.read (| β |); M.read (| left |) ]
                                                    |)
                                                  |) in
                                                M.alloc (|
                                                  Ty.tuple [],
                                                  M.match_operator (|
                                                    Ty.tuple [],
                                                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                                    [
                                                      fun γ =>
                                                        ltac:(M.monadic
                                                          (let γ :=
                                                            M.alloc (|
                                                              Ty.path "bool",
                                                              M.call_closure (|
                                                                Ty.path "bool",
                                                                BinOp.eq,
                                                                [ M.read (| i |); M.read (| start |)
                                                                ]
                                                              |)
                                                            |) in
                                                          let _ :=
                                                            is_constant_or_break_match (|
                                                              M.read (| γ |),
                                                              Value.Bool true
                                                            |) in
                                                          M.never_to_any (|
                                                            M.read (|
                                                              let~ _ : Ty.tuple [] :=
                                                                M.call_closure (|
                                                                  Ty.tuple [],
                                                                  M.get_associated_function (|
                                                                    Ty.apply
                                                                      (Ty.path "*mut")
                                                                      []
                                                                      [ T ],
                                                                    "write",
                                                                    [],
                                                                    []
                                                                  |),
                                                                  [
                                                                    M.call_closure (|
                                                                      Ty.apply
                                                                        (Ty.path "*mut")
                                                                        []
                                                                        [ T ],
                                                                      M.get_associated_function (|
                                                                        Ty.apply
                                                                          (Ty.path "*mut")
                                                                          []
                                                                          [ T ],
                                                                        "add",
                                                                        [],
                                                                        []
                                                                      |),
                                                                      [
                                                                        M.read (| x |);
                                                                        M.read (| start |)
                                                                      ]
                                                                    |);
                                                                    M.read (| tmp |)
                                                                  ]
                                                                |) in
                                                              M.break (||)
                                                            |)
                                                          |)));
                                                      fun γ => ltac:(M.monadic (Value.Tuple []))
                                                    ]
                                                  |)
                                                |)
                                              |)));
                                          fun γ =>
                                            ltac:(M.monadic
                                              (M.read (|
                                                let~ _ : Ty.tuple [] :=
                                                  let β := i in
                                                  M.write (|
                                                    β,
                                                    M.call_closure (|
                                                      Ty.path "usize",
                                                      BinOp.Wrap.add,
                                                      [ M.read (| β |); M.read (| right |) ]
                                                    |)
                                                  |) in
                                                M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                              |)))
                                        ]
                                      |)
                                    |)))
                                |)
                              |) in
                            let~ _ : Ty.tuple [] :=
                              let β := start in
                              M.write (|
                                β,
                                M.call_closure (|
                                  Ty.path "usize",
                                  BinOp.Wrap.add,
                                  [ M.read (| β |); Value.Integer IntegerKind.Usize 1 ]
                                |)
                              |) in
                            M.alloc (| Ty.tuple [], Value.Tuple [] |)
                          |)));
                      fun γ =>
                        ltac:(M.monadic
                          (M.never_to_any (|
                            M.read (|
                              let~ _ : Ty.tuple [] :=
                                M.never_to_any (| M.read (| M.break (||) |) |) in
                              M.alloc (| Ty.tuple [], Value.Tuple [] |)
                            |)
                          |)))
                    ]
                  |)
                |)))
            |)
          |)))
      | _, _, _ => M.impossible "wrong number of arguments"
      end.
    
    Global Instance Instance_IsFunction_ptr_rotate_gcd :
      M.IsFunction.C "core::slice::rotate::ptr_rotate_gcd" ptr_rotate_gcd.
    Admitted.
    Global Typeclasses Opaque ptr_rotate_gcd.
    
    (*
    const unsafe fn ptr_rotate_swap<T>(mut left: usize, mut mid: *mut T, mut right: usize) {
        loop {
            if left >= right {
                // Algorithm 3
                // There is an alternate way of swapping that involves finding where the last swap
                // of this algorithm would be, and swapping using that last chunk instead of swapping
                // adjacent chunks like this algorithm is doing, but this way is still faster.
                loop {
                    // SAFETY:
                    // `left >= right` so `[mid-right, mid+right)` is valid for reading and writing
                    // Subtracting `right` from `mid` each turn is counterbalanced by the addition and
                    // check after it.
                    unsafe {
                        ptr::swap_nonoverlapping(mid.sub(right), mid, right);
                        mid = mid.sub(right);
                    }
                    left -= right;
                    if left < right {
                        break;
                    }
                }
            } else {
                // Algorithm 3, `left < right`
                loop {
                    // SAFETY: `[mid-left, mid+left)` is valid for reading and writing because
                    // `left < right` so `mid+left < mid+right`.
                    // Adding `left` to `mid` each turn is counterbalanced by the subtraction and check
                    // after it.
                    unsafe {
                        ptr::swap_nonoverlapping(mid.sub(left), mid, left);
                        mid = mid.add(left);
                    }
                    right -= left;
                    if right < left {
                        break;
                    }
                }
            }
            if (right == 0) || (left == 0) {
                return;
            }
        }
    }
    *)
    Definition ptr_rotate_swap (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
      match ε, τ, α with
      | [], [ T ], [ _ as left; mid; _ as right ] =>
        ltac:(M.monadic
          (let left := M.alloc (| Ty.path "usize", left |) in
          let mid := M.alloc (| Ty.apply (Ty.path "*mut") [] [ T ], mid |) in
          let right := M.alloc (| Ty.path "usize", right |) in
          M.catch_return (Ty.tuple []) (|
            ltac:(M.monadic
              (M.never_to_any (|
                M.read (|
                  M.loop (|
                    Ty.path "never",
                    ltac:(M.monadic
                      (let~ _ : Ty.tuple [] :=
                        M.match_operator (|
                          Ty.tuple [],
                          M.alloc (| Ty.tuple [], Value.Tuple [] |),
                          [
                            fun γ =>
                              ltac:(M.monadic
                                (let γ :=
                                  M.alloc (|
                                    Ty.path "bool",
                                    M.call_closure (|
                                      Ty.path "bool",
                                      BinOp.ge,
                                      [ M.read (| left |); M.read (| right |) ]
                                    |)
                                  |) in
                                let _ :=
                                  is_constant_or_break_match (|
                                    M.read (| γ |),
                                    Value.Bool true
                                  |) in
                                M.read (|
                                  M.loop (|
                                    Ty.tuple [],
                                    ltac:(M.monadic
                                      (let~ _ : Ty.tuple [] :=
                                        M.read (|
                                          let~ _ : Ty.tuple [] :=
                                            M.call_closure (|
                                              Ty.tuple [],
                                              M.get_function (|
                                                "core::ptr::swap_nonoverlapping",
                                                [],
                                                [ T ]
                                              |),
                                              [
                                                M.call_closure (|
                                                  Ty.apply (Ty.path "*mut") [] [ T ],
                                                  M.get_associated_function (|
                                                    Ty.apply (Ty.path "*mut") [] [ T ],
                                                    "sub",
                                                    [],
                                                    []
                                                  |),
                                                  [ M.read (| mid |); M.read (| right |) ]
                                                |);
                                                M.read (| mid |);
                                                M.read (| right |)
                                              ]
                                            |) in
                                          let~ _ : Ty.tuple [] :=
                                            M.write (|
                                              mid,
                                              M.call_closure (|
                                                Ty.apply (Ty.path "*mut") [] [ T ],
                                                M.get_associated_function (|
                                                  Ty.apply (Ty.path "*mut") [] [ T ],
                                                  "sub",
                                                  [],
                                                  []
                                                |),
                                                [ M.read (| mid |); M.read (| right |) ]
                                              |)
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                        |) in
                                      let~ _ : Ty.tuple [] :=
                                        let β := left in
                                        M.write (|
                                          β,
                                          M.call_closure (|
                                            Ty.path "usize",
                                            BinOp.Wrap.sub,
                                            [ M.read (| β |); M.read (| right |) ]
                                          |)
                                        |) in
                                      M.alloc (|
                                        Ty.tuple [],
                                        M.match_operator (|
                                          Ty.tuple [],
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                          [
                                            fun γ =>
                                              ltac:(M.monadic
                                                (let γ :=
                                                  M.alloc (|
                                                    Ty.path "bool",
                                                    M.call_closure (|
                                                      Ty.path "bool",
                                                      BinOp.lt,
                                                      [ M.read (| left |); M.read (| right |) ]
                                                    |)
                                                  |) in
                                                let _ :=
                                                  is_constant_or_break_match (|
                                                    M.read (| γ |),
                                                    Value.Bool true
                                                  |) in
                                                M.never_to_any (| M.read (| M.break (||) |) |)));
                                            fun γ => ltac:(M.monadic (Value.Tuple []))
                                          ]
                                        |)
                                      |)))
                                  |)
                                |)));
                            fun γ =>
                              ltac:(M.monadic
                                (M.read (|
                                  M.loop (|
                                    Ty.tuple [],
                                    ltac:(M.monadic
                                      (let~ _ : Ty.tuple [] :=
                                        M.read (|
                                          let~ _ : Ty.tuple [] :=
                                            M.call_closure (|
                                              Ty.tuple [],
                                              M.get_function (|
                                                "core::ptr::swap_nonoverlapping",
                                                [],
                                                [ T ]
                                              |),
                                              [
                                                M.call_closure (|
                                                  Ty.apply (Ty.path "*mut") [] [ T ],
                                                  M.get_associated_function (|
                                                    Ty.apply (Ty.path "*mut") [] [ T ],
                                                    "sub",
                                                    [],
                                                    []
                                                  |),
                                                  [ M.read (| mid |); M.read (| left |) ]
                                                |);
                                                M.read (| mid |);
                                                M.read (| left |)
                                              ]
                                            |) in
                                          let~ _ : Ty.tuple [] :=
                                            M.write (|
                                              mid,
                                              M.call_closure (|
                                                Ty.apply (Ty.path "*mut") [] [ T ],
                                                M.get_associated_function (|
                                                  Ty.apply (Ty.path "*mut") [] [ T ],
                                                  "add",
                                                  [],
                                                  []
                                                |),
                                                [ M.read (| mid |); M.read (| left |) ]
                                              |)
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                        |) in
                                      let~ _ : Ty.tuple [] :=
                                        let β := right in
                                        M.write (|
                                          β,
                                          M.call_closure (|
                                            Ty.path "usize",
                                            BinOp.Wrap.sub,
                                            [ M.read (| β |); M.read (| left |) ]
                                          |)
                                        |) in
                                      M.alloc (|
                                        Ty.tuple [],
                                        M.match_operator (|
                                          Ty.tuple [],
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                          [
                                            fun γ =>
                                              ltac:(M.monadic
                                                (let γ :=
                                                  M.alloc (|
                                                    Ty.path "bool",
                                                    M.call_closure (|
                                                      Ty.path "bool",
                                                      BinOp.lt,
                                                      [ M.read (| right |); M.read (| left |) ]
                                                    |)
                                                  |) in
                                                let _ :=
                                                  is_constant_or_break_match (|
                                                    M.read (| γ |),
                                                    Value.Bool true
                                                  |) in
                                                M.never_to_any (| M.read (| M.break (||) |) |)));
                                            fun γ => ltac:(M.monadic (Value.Tuple []))
                                          ]
                                        |)
                                      |)))
                                  |)
                                |)))
                          ]
                        |) in
                      M.alloc (|
                        Ty.tuple [],
                        M.match_operator (|
                          Ty.tuple [],
                          M.alloc (| Ty.tuple [], Value.Tuple [] |),
                          [
                            fun γ =>
                              ltac:(M.monadic
                                (let γ :=
                                  M.alloc (|
                                    Ty.path "bool",
                                    LogicalOp.or (|
                                      M.call_closure (|
                                        Ty.path "bool",
                                        BinOp.eq,
                                        [ M.read (| right |); Value.Integer IntegerKind.Usize 0 ]
                                      |),
                                      ltac:(M.monadic
                                        (M.call_closure (|
                                          Ty.path "bool",
                                          BinOp.eq,
                                          [ M.read (| left |); Value.Integer IntegerKind.Usize 0 ]
                                        |)))
                                    |)
                                  |) in
                                let _ :=
                                  is_constant_or_break_match (|
                                    M.read (| γ |),
                                    Value.Bool true
                                  |) in
                                M.never_to_any (| M.read (| M.return_ (| Value.Tuple [] |) |) |)));
                            fun γ => ltac:(M.monadic (Value.Tuple []))
                          ]
                        |)
                      |)))
                  |)
                |)
              |)))
          |)))
      | _, _, _ => M.impossible "wrong number of arguments"
      end.
    
    Global Instance Instance_IsFunction_ptr_rotate_swap :
      M.IsFunction.C "core::slice::rotate::ptr_rotate_swap" ptr_rotate_swap.
    Admitted.
    Global Typeclasses Opaque ptr_rotate_swap.
    
    (*
    const fn const_min(left: usize, right: usize) -> usize {
        if right < left { right } else { left }
    }
    *)
    Definition const_min (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
      match ε, τ, α with
      | [], [], [ _ as left; _ as right ] =>
        ltac:(M.monadic
          (let left := M.alloc (| Ty.path "usize", left |) in
          let right := M.alloc (| Ty.path "usize", right |) in
          M.match_operator (|
            Ty.path "usize",
            M.alloc (| Ty.tuple [], Value.Tuple [] |),
            [
              fun γ =>
                ltac:(M.monadic
                  (let γ :=
                    M.alloc (|
                      Ty.path "bool",
                      M.call_closure (|
                        Ty.path "bool",
                        BinOp.lt,
                        [ M.read (| right |); M.read (| left |) ]
                      |)
                    |) in
                  let _ := is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                  M.read (| right |)));
              fun γ => ltac:(M.monadic (M.read (| left |)))
            ]
          |)))
      | _, _, _ => M.impossible "wrong number of arguments"
      end.
    
    Global Instance Instance_IsFunction_const_min :
      M.IsFunction.C "core::slice::rotate::const_min" const_min.
    Admitted.
    Global Typeclasses Opaque const_min.
  End rotate.
End slice.
