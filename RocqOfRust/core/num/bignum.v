(* Generated by rocq-of-rust *)
Require Import RocqOfRust.RocqOfRust.

Module num.
  Module bignum.
    (* Trait *)
    (* Empty module 'FullOps' *)
    
    Module Impl_core_num_bignum_FullOps_for_u8.
      Definition Self : Ty.t := Ty.path "u8".
      
      (*
                      fn full_mul_add(self, other: $ty, other2: $ty, carry: $ty) -> ($ty, $ty) {
                          // This cannot overflow;
                          // the output is between `0` and `2^nbits * (2^nbits - 1)`.
                          let (lo, hi) = self.carrying_mul_add(other, other2, carry);
                          (hi, lo)
                      }
      *)
      Definition full_mul_add (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other; other2; carry ] =>
          ltac:(M.monadic
            (let self := M.alloc (| Ty.path "u8", self |) in
            let other := M.alloc (| Ty.path "u8", other |) in
            let other2 := M.alloc (| Ty.path "u8", other2 |) in
            let carry := M.alloc (| Ty.path "u8", carry |) in
            M.match_operator (|
              Ty.tuple [ Ty.path "u8"; Ty.path "u8" ],
              M.alloc (|
                Ty.tuple [ Ty.path "u8"; Ty.path "u8" ],
                M.call_closure (|
                  Ty.tuple [ Ty.path "u8"; Ty.path "u8" ],
                  M.get_associated_function (| Ty.path "u8", "carrying_mul_add", [], [] |),
                  [ M.read (| self |); M.read (| other |); M.read (| other2 |); M.read (| carry |) ]
                |)
              |),
              [
                fun γ =>
                  ltac:(M.monadic
                    (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                    let γ0_1 := M.SubPointer.get_tuple_field (| γ, 1 |) in
                    let lo := M.copy (| Ty.path "u8", γ0_0 |) in
                    let hi := M.copy (| Ty.path "u8", γ0_1 |) in
                    Value.Tuple [ M.read (| hi |); M.read (| lo |) ]))
              ]
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      (*
                      fn full_div_rem(self, other: $ty, borrow: $ty) -> ($ty, $ty) {
                          debug_assert!(borrow < other);
                          // This cannot overflow; the output is between `0` and `other * (2^nbits - 1)`.
                          let lhs = ((borrow as $bigty) << <$ty>::BITS) | (self as $bigty);
                          let rhs = other as $bigty;
                          ((lhs / rhs) as $ty, (lhs % rhs) as $ty)
                      }
      *)
      Definition full_div_rem (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other; borrow ] =>
          ltac:(M.monadic
            (let self := M.alloc (| Ty.path "u8", self |) in
            let other := M.alloc (| Ty.path "u8", other |) in
            let borrow := M.alloc (| Ty.path "u8", borrow |) in
            M.read (|
              let~ _ : Ty.tuple [] :=
                M.match_operator (|
                  Ty.tuple [],
                  M.alloc (| Ty.tuple [], Value.Tuple [] |),
                  [
                    fun γ =>
                      ltac:(M.monadic
                        (let γ := M.alloc (| Ty.path "bool", Value.Bool true |) in
                        let _ := is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                        M.read (|
                          let~ _ : Ty.tuple [] :=
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (| Ty.tuple [], Value.Tuple [] |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let γ :=
                                      M.alloc (|
                                        Ty.path "bool",
                                        M.call_closure (|
                                          Ty.path "bool",
                                          UnOp.not,
                                          [
                                            M.call_closure (|
                                              Ty.path "bool",
                                              BinOp.lt,
                                              [ M.read (| borrow |); M.read (| other |) ]
                                            |)
                                          ]
                                        |)
                                      |) in
                                    let _ :=
                                      is_constant_or_break_match (|
                                        M.read (| γ |),
                                        Value.Bool true
                                      |) in
                                    M.never_to_any (|
                                      M.call_closure (|
                                        Ty.path "never",
                                        M.get_function (| "core::panicking::panic", [], [] |),
                                        [ mk_str (| "assertion failed: borrow < other" |) ]
                                      |)
                                    |)));
                                fun γ => ltac:(M.monadic (Value.Tuple []))
                              ]
                            |) in
                          M.alloc (| Ty.tuple [], Value.Tuple [] |)
                        |)));
                    fun γ => ltac:(M.monadic (Value.Tuple []))
                  ]
                |) in
              let~ lhs : Ty.path "u16" :=
                M.call_closure (|
                  Ty.path "u16",
                  BinOp.Wrap.bit_or,
                  [
                    M.call_closure (|
                      Ty.path "u16",
                      BinOp.Wrap.shl,
                      [
                        M.cast (Ty.path "u16") (M.read (| borrow |));
                        M.read (|
                          get_associated_constant (| Ty.path "u8", "BITS", Ty.path "u32" |)
                        |)
                      ]
                    |);
                    M.cast (Ty.path "u16") (M.read (| self |))
                  ]
                |) in
              let~ rhs : Ty.path "u16" := M.cast (Ty.path "u16") (M.read (| other |)) in
              M.alloc (|
                Ty.tuple [ Ty.path "u8"; Ty.path "u8" ],
                Value.Tuple
                  [
                    M.cast
                      (Ty.path "u8")
                      (M.call_closure (|
                        Ty.path "u16",
                        BinOp.Wrap.div,
                        [ M.read (| lhs |); M.read (| rhs |) ]
                      |));
                    M.cast
                      (Ty.path "u8")
                      (M.call_closure (|
                        Ty.path "u16",
                        BinOp.Wrap.rem,
                        [ M.read (| lhs |); M.read (| rhs |) ]
                      |))
                  ]
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::num::bignum::FullOps"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *)
          [
            ("full_mul_add", InstanceField.Method full_mul_add);
            ("full_div_rem", InstanceField.Method full_div_rem)
          ].
    End Impl_core_num_bignum_FullOps_for_u8.
    
    Module Impl_core_num_bignum_FullOps_for_u16.
      Definition Self : Ty.t := Ty.path "u16".
      
      (*
                      fn full_mul_add(self, other: $ty, other2: $ty, carry: $ty) -> ($ty, $ty) {
                          // This cannot overflow;
                          // the output is between `0` and `2^nbits * (2^nbits - 1)`.
                          let (lo, hi) = self.carrying_mul_add(other, other2, carry);
                          (hi, lo)
                      }
      *)
      Definition full_mul_add (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other; other2; carry ] =>
          ltac:(M.monadic
            (let self := M.alloc (| Ty.path "u16", self |) in
            let other := M.alloc (| Ty.path "u16", other |) in
            let other2 := M.alloc (| Ty.path "u16", other2 |) in
            let carry := M.alloc (| Ty.path "u16", carry |) in
            M.match_operator (|
              Ty.tuple [ Ty.path "u16"; Ty.path "u16" ],
              M.alloc (|
                Ty.tuple [ Ty.path "u16"; Ty.path "u16" ],
                M.call_closure (|
                  Ty.tuple [ Ty.path "u16"; Ty.path "u16" ],
                  M.get_associated_function (| Ty.path "u16", "carrying_mul_add", [], [] |),
                  [ M.read (| self |); M.read (| other |); M.read (| other2 |); M.read (| carry |) ]
                |)
              |),
              [
                fun γ =>
                  ltac:(M.monadic
                    (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                    let γ0_1 := M.SubPointer.get_tuple_field (| γ, 1 |) in
                    let lo := M.copy (| Ty.path "u16", γ0_0 |) in
                    let hi := M.copy (| Ty.path "u16", γ0_1 |) in
                    Value.Tuple [ M.read (| hi |); M.read (| lo |) ]))
              ]
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      (*
                      fn full_div_rem(self, other: $ty, borrow: $ty) -> ($ty, $ty) {
                          debug_assert!(borrow < other);
                          // This cannot overflow; the output is between `0` and `other * (2^nbits - 1)`.
                          let lhs = ((borrow as $bigty) << <$ty>::BITS) | (self as $bigty);
                          let rhs = other as $bigty;
                          ((lhs / rhs) as $ty, (lhs % rhs) as $ty)
                      }
      *)
      Definition full_div_rem (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other; borrow ] =>
          ltac:(M.monadic
            (let self := M.alloc (| Ty.path "u16", self |) in
            let other := M.alloc (| Ty.path "u16", other |) in
            let borrow := M.alloc (| Ty.path "u16", borrow |) in
            M.read (|
              let~ _ : Ty.tuple [] :=
                M.match_operator (|
                  Ty.tuple [],
                  M.alloc (| Ty.tuple [], Value.Tuple [] |),
                  [
                    fun γ =>
                      ltac:(M.monadic
                        (let γ := M.alloc (| Ty.path "bool", Value.Bool true |) in
                        let _ := is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                        M.read (|
                          let~ _ : Ty.tuple [] :=
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (| Ty.tuple [], Value.Tuple [] |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let γ :=
                                      M.alloc (|
                                        Ty.path "bool",
                                        M.call_closure (|
                                          Ty.path "bool",
                                          UnOp.not,
                                          [
                                            M.call_closure (|
                                              Ty.path "bool",
                                              BinOp.lt,
                                              [ M.read (| borrow |); M.read (| other |) ]
                                            |)
                                          ]
                                        |)
                                      |) in
                                    let _ :=
                                      is_constant_or_break_match (|
                                        M.read (| γ |),
                                        Value.Bool true
                                      |) in
                                    M.never_to_any (|
                                      M.call_closure (|
                                        Ty.path "never",
                                        M.get_function (| "core::panicking::panic", [], [] |),
                                        [ mk_str (| "assertion failed: borrow < other" |) ]
                                      |)
                                    |)));
                                fun γ => ltac:(M.monadic (Value.Tuple []))
                              ]
                            |) in
                          M.alloc (| Ty.tuple [], Value.Tuple [] |)
                        |)));
                    fun γ => ltac:(M.monadic (Value.Tuple []))
                  ]
                |) in
              let~ lhs : Ty.path "u32" :=
                M.call_closure (|
                  Ty.path "u32",
                  BinOp.Wrap.bit_or,
                  [
                    M.call_closure (|
                      Ty.path "u32",
                      BinOp.Wrap.shl,
                      [
                        M.cast (Ty.path "u32") (M.read (| borrow |));
                        M.read (|
                          get_associated_constant (| Ty.path "u16", "BITS", Ty.path "u32" |)
                        |)
                      ]
                    |);
                    M.cast (Ty.path "u32") (M.read (| self |))
                  ]
                |) in
              let~ rhs : Ty.path "u32" := M.cast (Ty.path "u32") (M.read (| other |)) in
              M.alloc (|
                Ty.tuple [ Ty.path "u16"; Ty.path "u16" ],
                Value.Tuple
                  [
                    M.cast
                      (Ty.path "u16")
                      (M.call_closure (|
                        Ty.path "u32",
                        BinOp.Wrap.div,
                        [ M.read (| lhs |); M.read (| rhs |) ]
                      |));
                    M.cast
                      (Ty.path "u16")
                      (M.call_closure (|
                        Ty.path "u32",
                        BinOp.Wrap.rem,
                        [ M.read (| lhs |); M.read (| rhs |) ]
                      |))
                  ]
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::num::bignum::FullOps"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *)
          [
            ("full_mul_add", InstanceField.Method full_mul_add);
            ("full_div_rem", InstanceField.Method full_div_rem)
          ].
    End Impl_core_num_bignum_FullOps_for_u16.
    
    Module Impl_core_num_bignum_FullOps_for_u32.
      Definition Self : Ty.t := Ty.path "u32".
      
      (*
                      fn full_mul_add(self, other: $ty, other2: $ty, carry: $ty) -> ($ty, $ty) {
                          // This cannot overflow;
                          // the output is between `0` and `2^nbits * (2^nbits - 1)`.
                          let (lo, hi) = self.carrying_mul_add(other, other2, carry);
                          (hi, lo)
                      }
      *)
      Definition full_mul_add (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other; other2; carry ] =>
          ltac:(M.monadic
            (let self := M.alloc (| Ty.path "u32", self |) in
            let other := M.alloc (| Ty.path "u32", other |) in
            let other2 := M.alloc (| Ty.path "u32", other2 |) in
            let carry := M.alloc (| Ty.path "u32", carry |) in
            M.match_operator (|
              Ty.tuple [ Ty.path "u32"; Ty.path "u32" ],
              M.alloc (|
                Ty.tuple [ Ty.path "u32"; Ty.path "u32" ],
                M.call_closure (|
                  Ty.tuple [ Ty.path "u32"; Ty.path "u32" ],
                  M.get_associated_function (| Ty.path "u32", "carrying_mul_add", [], [] |),
                  [ M.read (| self |); M.read (| other |); M.read (| other2 |); M.read (| carry |) ]
                |)
              |),
              [
                fun γ =>
                  ltac:(M.monadic
                    (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                    let γ0_1 := M.SubPointer.get_tuple_field (| γ, 1 |) in
                    let lo := M.copy (| Ty.path "u32", γ0_0 |) in
                    let hi := M.copy (| Ty.path "u32", γ0_1 |) in
                    Value.Tuple [ M.read (| hi |); M.read (| lo |) ]))
              ]
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      (*
                      fn full_div_rem(self, other: $ty, borrow: $ty) -> ($ty, $ty) {
                          debug_assert!(borrow < other);
                          // This cannot overflow; the output is between `0` and `other * (2^nbits - 1)`.
                          let lhs = ((borrow as $bigty) << <$ty>::BITS) | (self as $bigty);
                          let rhs = other as $bigty;
                          ((lhs / rhs) as $ty, (lhs % rhs) as $ty)
                      }
      *)
      Definition full_div_rem (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other; borrow ] =>
          ltac:(M.monadic
            (let self := M.alloc (| Ty.path "u32", self |) in
            let other := M.alloc (| Ty.path "u32", other |) in
            let borrow := M.alloc (| Ty.path "u32", borrow |) in
            M.read (|
              let~ _ : Ty.tuple [] :=
                M.match_operator (|
                  Ty.tuple [],
                  M.alloc (| Ty.tuple [], Value.Tuple [] |),
                  [
                    fun γ =>
                      ltac:(M.monadic
                        (let γ := M.alloc (| Ty.path "bool", Value.Bool true |) in
                        let _ := is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                        M.read (|
                          let~ _ : Ty.tuple [] :=
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (| Ty.tuple [], Value.Tuple [] |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let γ :=
                                      M.alloc (|
                                        Ty.path "bool",
                                        M.call_closure (|
                                          Ty.path "bool",
                                          UnOp.not,
                                          [
                                            M.call_closure (|
                                              Ty.path "bool",
                                              BinOp.lt,
                                              [ M.read (| borrow |); M.read (| other |) ]
                                            |)
                                          ]
                                        |)
                                      |) in
                                    let _ :=
                                      is_constant_or_break_match (|
                                        M.read (| γ |),
                                        Value.Bool true
                                      |) in
                                    M.never_to_any (|
                                      M.call_closure (|
                                        Ty.path "never",
                                        M.get_function (| "core::panicking::panic", [], [] |),
                                        [ mk_str (| "assertion failed: borrow < other" |) ]
                                      |)
                                    |)));
                                fun γ => ltac:(M.monadic (Value.Tuple []))
                              ]
                            |) in
                          M.alloc (| Ty.tuple [], Value.Tuple [] |)
                        |)));
                    fun γ => ltac:(M.monadic (Value.Tuple []))
                  ]
                |) in
              let~ lhs : Ty.path "u64" :=
                M.call_closure (|
                  Ty.path "u64",
                  BinOp.Wrap.bit_or,
                  [
                    M.call_closure (|
                      Ty.path "u64",
                      BinOp.Wrap.shl,
                      [
                        M.cast (Ty.path "u64") (M.read (| borrow |));
                        M.read (|
                          get_associated_constant (| Ty.path "u32", "BITS", Ty.path "u32" |)
                        |)
                      ]
                    |);
                    M.cast (Ty.path "u64") (M.read (| self |))
                  ]
                |) in
              let~ rhs : Ty.path "u64" := M.cast (Ty.path "u64") (M.read (| other |)) in
              M.alloc (|
                Ty.tuple [ Ty.path "u32"; Ty.path "u32" ],
                Value.Tuple
                  [
                    M.cast
                      (Ty.path "u32")
                      (M.call_closure (|
                        Ty.path "u64",
                        BinOp.Wrap.div,
                        [ M.read (| lhs |); M.read (| rhs |) ]
                      |));
                    M.cast
                      (Ty.path "u32")
                      (M.call_closure (|
                        Ty.path "u64",
                        BinOp.Wrap.rem,
                        [ M.read (| lhs |); M.read (| rhs |) ]
                      |))
                  ]
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::num::bignum::FullOps"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *)
          [
            ("full_mul_add", InstanceField.Method full_mul_add);
            ("full_div_rem", InstanceField.Method full_div_rem)
          ].
    End Impl_core_num_bignum_FullOps_for_u32.
    
    Module Impl_core_num_bignum_FullOps_for_u64.
      Definition Self : Ty.t := Ty.path "u64".
      
      (*
                      fn full_mul_add(self, other: $ty, other2: $ty, carry: $ty) -> ($ty, $ty) {
                          // This cannot overflow;
                          // the output is between `0` and `2^nbits * (2^nbits - 1)`.
                          let (lo, hi) = self.carrying_mul_add(other, other2, carry);
                          (hi, lo)
                      }
      *)
      Definition full_mul_add (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other; other2; carry ] =>
          ltac:(M.monadic
            (let self := M.alloc (| Ty.path "u64", self |) in
            let other := M.alloc (| Ty.path "u64", other |) in
            let other2 := M.alloc (| Ty.path "u64", other2 |) in
            let carry := M.alloc (| Ty.path "u64", carry |) in
            M.match_operator (|
              Ty.tuple [ Ty.path "u64"; Ty.path "u64" ],
              M.alloc (|
                Ty.tuple [ Ty.path "u64"; Ty.path "u64" ],
                M.call_closure (|
                  Ty.tuple [ Ty.path "u64"; Ty.path "u64" ],
                  M.get_associated_function (| Ty.path "u64", "carrying_mul_add", [], [] |),
                  [ M.read (| self |); M.read (| other |); M.read (| other2 |); M.read (| carry |) ]
                |)
              |),
              [
                fun γ =>
                  ltac:(M.monadic
                    (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                    let γ0_1 := M.SubPointer.get_tuple_field (| γ, 1 |) in
                    let lo := M.copy (| Ty.path "u64", γ0_0 |) in
                    let hi := M.copy (| Ty.path "u64", γ0_1 |) in
                    Value.Tuple [ M.read (| hi |); M.read (| lo |) ]))
              ]
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      (*
                      fn full_div_rem(self, other: $ty, borrow: $ty) -> ($ty, $ty) {
                          debug_assert!(borrow < other);
                          // This cannot overflow; the output is between `0` and `other * (2^nbits - 1)`.
                          let lhs = ((borrow as $bigty) << <$ty>::BITS) | (self as $bigty);
                          let rhs = other as $bigty;
                          ((lhs / rhs) as $ty, (lhs % rhs) as $ty)
                      }
      *)
      Definition full_div_rem (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other; borrow ] =>
          ltac:(M.monadic
            (let self := M.alloc (| Ty.path "u64", self |) in
            let other := M.alloc (| Ty.path "u64", other |) in
            let borrow := M.alloc (| Ty.path "u64", borrow |) in
            M.read (|
              let~ _ : Ty.tuple [] :=
                M.match_operator (|
                  Ty.tuple [],
                  M.alloc (| Ty.tuple [], Value.Tuple [] |),
                  [
                    fun γ =>
                      ltac:(M.monadic
                        (let γ := M.alloc (| Ty.path "bool", Value.Bool true |) in
                        let _ := is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                        M.read (|
                          let~ _ : Ty.tuple [] :=
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (| Ty.tuple [], Value.Tuple [] |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let γ :=
                                      M.alloc (|
                                        Ty.path "bool",
                                        M.call_closure (|
                                          Ty.path "bool",
                                          UnOp.not,
                                          [
                                            M.call_closure (|
                                              Ty.path "bool",
                                              BinOp.lt,
                                              [ M.read (| borrow |); M.read (| other |) ]
                                            |)
                                          ]
                                        |)
                                      |) in
                                    let _ :=
                                      is_constant_or_break_match (|
                                        M.read (| γ |),
                                        Value.Bool true
                                      |) in
                                    M.never_to_any (|
                                      M.call_closure (|
                                        Ty.path "never",
                                        M.get_function (| "core::panicking::panic", [], [] |),
                                        [ mk_str (| "assertion failed: borrow < other" |) ]
                                      |)
                                    |)));
                                fun γ => ltac:(M.monadic (Value.Tuple []))
                              ]
                            |) in
                          M.alloc (| Ty.tuple [], Value.Tuple [] |)
                        |)));
                    fun γ => ltac:(M.monadic (Value.Tuple []))
                  ]
                |) in
              let~ lhs : Ty.path "u128" :=
                M.call_closure (|
                  Ty.path "u128",
                  BinOp.Wrap.bit_or,
                  [
                    M.call_closure (|
                      Ty.path "u128",
                      BinOp.Wrap.shl,
                      [
                        M.cast (Ty.path "u128") (M.read (| borrow |));
                        M.read (|
                          get_associated_constant (| Ty.path "u64", "BITS", Ty.path "u32" |)
                        |)
                      ]
                    |);
                    M.cast (Ty.path "u128") (M.read (| self |))
                  ]
                |) in
              let~ rhs : Ty.path "u128" := M.cast (Ty.path "u128") (M.read (| other |)) in
              M.alloc (|
                Ty.tuple [ Ty.path "u64"; Ty.path "u64" ],
                Value.Tuple
                  [
                    M.cast
                      (Ty.path "u64")
                      (M.call_closure (|
                        Ty.path "u128",
                        BinOp.Wrap.div,
                        [ M.read (| lhs |); M.read (| rhs |) ]
                      |));
                    M.cast
                      (Ty.path "u64")
                      (M.call_closure (|
                        Ty.path "u128",
                        BinOp.Wrap.rem,
                        [ M.read (| lhs |); M.read (| rhs |) ]
                      |))
                  ]
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::num::bignum::FullOps"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *)
          [
            ("full_mul_add", InstanceField.Method full_mul_add);
            ("full_div_rem", InstanceField.Method full_div_rem)
          ].
    End Impl_core_num_bignum_FullOps_for_u64.
    
    Definition value_SMALL_POW5 (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
      ltac:(M.monadic
        (M.alloc (|
          Ty.apply
            (Ty.path "array")
            [ Value.Integer IntegerKind.Usize 3 ]
            [ Ty.tuple [ Ty.path "u64"; Ty.path "usize" ] ],
          Value.Array
            [
              Value.Tuple [ Value.Integer IntegerKind.U64 125; Value.Integer IntegerKind.Usize 3 ];
              Value.Tuple
                [ Value.Integer IntegerKind.U64 15625; Value.Integer IntegerKind.Usize 6 ];
              Value.Tuple
                [ Value.Integer IntegerKind.U64 1220703125; Value.Integer IntegerKind.Usize 13 ]
            ]
        |))).
    
    Global Instance Instance_IsConstant_value_SMALL_POW5 :
      M.IsFunction.C "core::num::bignum::SMALL_POW5" value_SMALL_POW5.
    Admitted.
    Global Typeclasses Opaque value_SMALL_POW5.
    
    Axiom Digit32 : (Ty.path "core::num::bignum::Digit32") = (Ty.path "u32").
    
    (* StructRecord
      {
        name := "Big32x40";
        const_params := [];
        ty_params := [];
        fields :=
          [
            ("size", Ty.path "usize");
            ("base",
              Ty.apply (Ty.path "array") [ Value.Integer IntegerKind.Usize 40 ] [ Ty.path "u32" ])
          ];
      } *)
    
    Module Impl_core_num_bignum_Big32x40.
      Definition Self : Ty.t := Ty.path "core::num::bignum::Big32x40".
      
      (*
                  pub fn from_small(v: $ty) -> $name {
                      let mut base = [0; $n];
                      base[0] = v;
                      $name { size: 1, base }
                  }
      *)
      Definition from_small (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ v ] =>
          ltac:(M.monadic
            (let v := M.alloc (| Ty.path "u32", v |) in
            M.read (|
              let~ base :
                  Ty.apply
                    (Ty.path "array")
                    [ Value.Integer IntegerKind.Usize 40 ]
                    [ Ty.path "u32" ] :=
                lib.repeat (|
                  Value.Integer IntegerKind.U32 0,
                  Value.Integer IntegerKind.Usize 40
                |) in
              let~ _ : Ty.tuple [] :=
                M.write (|
                  M.SubPointer.get_array_field (| base, Value.Integer IntegerKind.Usize 0 |),
                  M.read (| v |)
                |) in
              M.alloc (|
                Ty.path "core::num::bignum::Big32x40",
                Value.mkStructRecord
                  "core::num::bignum::Big32x40"
                  []
                  []
                  [ ("size", Value.Integer IntegerKind.Usize 1); ("base", M.read (| base |)) ]
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_from_small :
        M.IsAssociatedFunction.C Self "from_small" from_small.
      Admitted.
      Global Typeclasses Opaque from_small.
      
      (*
                  pub fn from_u64(mut v: u64) -> $name {
                      let mut base = [0; $n];
                      let mut sz = 0;
                      while v > 0 {
                          base[sz] = v as $ty;
                          v >>= <$ty>::BITS;
                          sz += 1;
                      }
                      $name { size: sz, base }
                  }
      *)
      Definition from_u64 (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ v ] =>
          ltac:(M.monadic
            (let v := M.alloc (| Ty.path "u64", v |) in
            M.read (|
              let~ base :
                  Ty.apply
                    (Ty.path "array")
                    [ Value.Integer IntegerKind.Usize 40 ]
                    [ Ty.path "u32" ] :=
                lib.repeat (|
                  Value.Integer IntegerKind.U32 0,
                  Value.Integer IntegerKind.Usize 40
                |) in
              let~ sz : Ty.path "usize" := Value.Integer IntegerKind.Usize 0 in
              let~ _ : Ty.tuple [] :=
                M.read (|
                  M.loop (|
                    Ty.tuple [],
                    ltac:(M.monadic
                      (M.alloc (|
                        Ty.tuple [],
                        M.match_operator (|
                          Ty.tuple [],
                          M.alloc (| Ty.tuple [], Value.Tuple [] |),
                          [
                            fun γ =>
                              ltac:(M.monadic
                                (let γ :=
                                  M.alloc (|
                                    Ty.path "bool",
                                    M.call_closure (|
                                      Ty.path "bool",
                                      BinOp.gt,
                                      [ M.read (| v |); Value.Integer IntegerKind.U64 0 ]
                                    |)
                                  |) in
                                let _ :=
                                  is_constant_or_break_match (|
                                    M.read (| γ |),
                                    Value.Bool true
                                  |) in
                                M.read (|
                                  let~ _ : Ty.tuple [] :=
                                    M.write (|
                                      M.SubPointer.get_array_field (| base, M.read (| sz |) |),
                                      M.cast (Ty.path "u32") (M.read (| v |))
                                    |) in
                                  let~ _ : Ty.tuple [] :=
                                    let β := v in
                                    M.write (|
                                      β,
                                      M.call_closure (|
                                        Ty.path "u64",
                                        BinOp.Wrap.shr,
                                        [
                                          M.read (| β |);
                                          M.read (|
                                            get_associated_constant (|
                                              Ty.path "u32",
                                              "BITS",
                                              Ty.path "u32"
                                            |)
                                          |)
                                        ]
                                      |)
                                    |) in
                                  let~ _ : Ty.tuple [] :=
                                    let β := sz in
                                    M.write (|
                                      β,
                                      M.call_closure (|
                                        Ty.path "usize",
                                        BinOp.Wrap.add,
                                        [ M.read (| β |); Value.Integer IntegerKind.Usize 1 ]
                                      |)
                                    |) in
                                  M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                |)));
                            fun γ =>
                              ltac:(M.monadic
                                (M.never_to_any (|
                                  M.read (|
                                    let~ _ : Ty.tuple [] :=
                                      M.never_to_any (| M.read (| M.break (||) |) |) in
                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                  |)
                                |)))
                          ]
                        |)
                      |)))
                  |)
                |) in
              M.alloc (|
                Ty.path "core::num::bignum::Big32x40",
                Value.mkStructRecord
                  "core::num::bignum::Big32x40"
                  []
                  []
                  [ ("size", M.read (| sz |)); ("base", M.read (| base |)) ]
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_from_u64 :
        M.IsAssociatedFunction.C Self "from_u64" from_u64.
      Admitted.
      Global Typeclasses Opaque from_u64.
      
      (*
                  pub fn digits(&self) -> &[$ty] {
                      &self.base[..self.size]
                  }
      *)
      Definition digits (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            M.borrow (|
              Pointer.Kind.Ref,
              M.deref (|
                M.borrow (|
                  Pointer.Kind.Ref,
                  M.deref (|
                    M.call_closure (|
                      Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                      M.get_trait_method (|
                        "core::ops::index::Index",
                        Ty.apply
                          (Ty.path "array")
                          [ Value.Integer IntegerKind.Usize 40 ]
                          [ Ty.path "u32" ],
                        [],
                        [ Ty.apply (Ty.path "core::ops::range::RangeTo") [] [ Ty.path "usize" ] ],
                        "index",
                        [],
                        []
                      |),
                      [
                        M.borrow (|
                          Pointer.Kind.Ref,
                          M.SubPointer.get_struct_record_field (|
                            M.deref (| M.read (| self |) |),
                            "core::num::bignum::Big32x40",
                            "base"
                          |)
                        |);
                        Value.mkStructRecord
                          "core::ops::range::RangeTo"
                          []
                          [ Ty.path "usize" ]
                          [
                            ("end_",
                              M.read (|
                                M.SubPointer.get_struct_record_field (|
                                  M.deref (| M.read (| self |) |),
                                  "core::num::bignum::Big32x40",
                                  "size"
                                |)
                              |))
                          ]
                      ]
                    |)
                  |)
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_digits : M.IsAssociatedFunction.C Self "digits" digits.
      Admitted.
      Global Typeclasses Opaque digits.
      
      (*
                  pub fn get_bit(&self, i: usize) -> u8 {
                      let digitbits = <$ty>::BITS as usize;
                      let d = i / digitbits;
                      let b = i % digitbits;
                      ((self.base[d] >> b) & 1) as u8
                  }
      *)
      Definition get_bit (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; i ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let i := M.alloc (| Ty.path "usize", i |) in
            M.read (|
              let~ digitbits : Ty.path "usize" :=
                M.cast
                  (Ty.path "usize")
                  (M.read (|
                    get_associated_constant (| Ty.path "u32", "BITS", Ty.path "u32" |)
                  |)) in
              let~ d : Ty.path "usize" :=
                M.call_closure (|
                  Ty.path "usize",
                  BinOp.Wrap.div,
                  [ M.read (| i |); M.read (| digitbits |) ]
                |) in
              let~ b : Ty.path "usize" :=
                M.call_closure (|
                  Ty.path "usize",
                  BinOp.Wrap.rem,
                  [ M.read (| i |); M.read (| digitbits |) ]
                |) in
              M.alloc (|
                Ty.path "u8",
                M.cast
                  (Ty.path "u8")
                  (M.call_closure (|
                    Ty.path "u32",
                    BinOp.Wrap.bit_and,
                    [
                      M.call_closure (|
                        Ty.path "u32",
                        BinOp.Wrap.shr,
                        [
                          M.read (|
                            M.SubPointer.get_array_field (|
                              M.SubPointer.get_struct_record_field (|
                                M.deref (| M.read (| self |) |),
                                "core::num::bignum::Big32x40",
                                "base"
                              |),
                              M.read (| d |)
                            |)
                          |);
                          M.read (| b |)
                        ]
                      |);
                      Value.Integer IntegerKind.U32 1
                    ]
                  |))
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_get_bit : M.IsAssociatedFunction.C Self "get_bit" get_bit.
      Admitted.
      Global Typeclasses Opaque get_bit.
      
      (*
                  pub fn is_zero(&self) -> bool {
                      self.digits().iter().all(|&v| v == 0)
                  }
      *)
      Definition is_zero (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            M.call_closure (|
              Ty.path "bool",
              M.get_trait_method (|
                "core::iter::traits::iterator::Iterator",
                Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                [],
                [],
                "all",
                [],
                [ Ty.function [ Ty.apply (Ty.path "&") [] [ Ty.path "u32" ] ] (Ty.path "bool") ]
              |),
              [
                M.borrow (|
                  Pointer.Kind.MutRef,
                  M.alloc (|
                    Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                    M.call_closure (|
                      Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                      M.get_associated_function (|
                        Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ],
                        "iter",
                        [],
                        []
                      |),
                      [
                        M.borrow (|
                          Pointer.Kind.Ref,
                          M.deref (|
                            M.call_closure (|
                              Ty.apply
                                (Ty.path "&")
                                []
                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                              M.get_associated_function (|
                                Ty.path "core::num::bignum::Big32x40",
                                "digits",
                                [],
                                []
                              |),
                              [ M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| self |) |) |) ]
                            |)
                          |)
                        |)
                      ]
                    |)
                  |)
                |);
                M.closure
                  (fun γ =>
                    ltac:(M.monadic
                      match γ with
                      | [ α0 ] =>
                        ltac:(M.monadic
                          (M.match_operator (|
                            Ty.path "bool",
                            M.alloc (| Ty.apply (Ty.path "&") [] [ Ty.path "u32" ], α0 |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let γ := M.deref (| M.read (| γ |) |) in
                                  let v := M.copy (| Ty.path "u32", γ |) in
                                  M.call_closure (|
                                    Ty.path "bool",
                                    BinOp.eq,
                                    [ M.read (| v |); Value.Integer IntegerKind.U32 0 ]
                                  |)))
                            ]
                          |)))
                      | _ => M.impossible "wrong number of arguments"
                      end))
              ]
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_is_zero : M.IsAssociatedFunction.C Self "is_zero" is_zero.
      Admitted.
      Global Typeclasses Opaque is_zero.
      
      (*
                  pub fn bit_length(&self) -> usize {
                      let digitbits = <$ty>::BITS as usize;
                      let digits = self.digits();
                      // Find the most significant non-zero digit.
                      let msd = digits.iter().rposition(|&x| x != 0);
                      match msd {
                          Some(msd) => msd * digitbits + digits[msd].ilog2() as usize + 1,
                          // There are no non-zero digits, i.e., the number is zero.
                          _ => 0,
                      }
                  }
      *)
      Definition bit_length (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            M.read (|
              let~ digitbits : Ty.path "usize" :=
                M.cast
                  (Ty.path "usize")
                  (M.read (|
                    get_associated_constant (| Ty.path "u32", "BITS", Ty.path "u32" |)
                  |)) in
              let~ digits :
                  Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ] :=
                M.call_closure (|
                  Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                  M.get_associated_function (|
                    Ty.path "core::num::bignum::Big32x40",
                    "digits",
                    [],
                    []
                  |),
                  [ M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| self |) |) |) ]
                |) in
              let~ msd : Ty.apply (Ty.path "core::option::Option") [] [ Ty.path "usize" ] :=
                M.call_closure (|
                  Ty.apply (Ty.path "core::option::Option") [] [ Ty.path "usize" ],
                  M.get_trait_method (|
                    "core::iter::traits::iterator::Iterator",
                    Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                    [],
                    [],
                    "rposition",
                    [],
                    [ Ty.function [ Ty.apply (Ty.path "&") [] [ Ty.path "u32" ] ] (Ty.path "bool") ]
                  |),
                  [
                    M.borrow (|
                      Pointer.Kind.MutRef,
                      M.alloc (|
                        Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                        M.call_closure (|
                          Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                          M.get_associated_function (|
                            Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ],
                            "iter",
                            [],
                            []
                          |),
                          [ M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| digits |) |) |) ]
                        |)
                      |)
                    |);
                    M.closure
                      (fun γ =>
                        ltac:(M.monadic
                          match γ with
                          | [ α0 ] =>
                            ltac:(M.monadic
                              (M.match_operator (|
                                Ty.path "bool",
                                M.alloc (| Ty.apply (Ty.path "&") [] [ Ty.path "u32" ], α0 |),
                                [
                                  fun γ =>
                                    ltac:(M.monadic
                                      (let γ := M.deref (| M.read (| γ |) |) in
                                      let x := M.copy (| Ty.path "u32", γ |) in
                                      M.call_closure (|
                                        Ty.path "bool",
                                        BinOp.ne,
                                        [ M.read (| x |); Value.Integer IntegerKind.U32 0 ]
                                      |)))
                                ]
                              |)))
                          | _ => M.impossible "wrong number of arguments"
                          end))
                  ]
                |) in
              M.alloc (|
                Ty.path "usize",
                M.match_operator (|
                  Ty.path "usize",
                  msd,
                  [
                    fun γ =>
                      ltac:(M.monadic
                        (let γ0_0 :=
                          M.SubPointer.get_struct_tuple_field (|
                            γ,
                            "core::option::Option::Some",
                            0
                          |) in
                        let msd := M.copy (| Ty.path "usize", γ0_0 |) in
                        M.call_closure (|
                          Ty.path "usize",
                          BinOp.Wrap.add,
                          [
                            M.call_closure (|
                              Ty.path "usize",
                              BinOp.Wrap.add,
                              [
                                M.call_closure (|
                                  Ty.path "usize",
                                  BinOp.Wrap.mul,
                                  [ M.read (| msd |); M.read (| digitbits |) ]
                                |);
                                M.cast
                                  (Ty.path "usize")
                                  (M.call_closure (|
                                    Ty.path "u32",
                                    M.get_associated_function (| Ty.path "u32", "ilog2", [], [] |),
                                    [
                                      M.read (|
                                        M.SubPointer.get_array_field (|
                                          M.deref (| M.read (| digits |) |),
                                          M.read (| msd |)
                                        |)
                                      |)
                                    ]
                                  |))
                              ]
                            |);
                            Value.Integer IntegerKind.Usize 1
                          ]
                        |)));
                    fun γ => ltac:(M.monadic (Value.Integer IntegerKind.Usize 0))
                  ]
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_bit_length :
        M.IsAssociatedFunction.C Self "bit_length" bit_length.
      Admitted.
      Global Typeclasses Opaque bit_length.
      
      (*
                  pub fn add<'a>(&'a mut self, other: &$name) -> &'a mut $name {
                      use crate::{cmp, iter};
      
                      let mut sz = cmp::max(self.size, other.size);
                      let mut carry = false;
                      for (a, b) in iter::zip(&mut self.base[..sz], &other.base[..sz]) {
                          let (v, c) = ( *a).carrying_add( *b, carry);
                          *a = v;
                          carry = c;
                      }
                      if carry {
                          self.base[sz] = 1;
                          sz += 1;
                      }
                      self.size = sz;
                      self
                  }
      *)
      Definition add (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                other
              |) in
            M.borrow (|
              Pointer.Kind.MutRef,
              M.deref (|
                M.read (|
                  let~ sz : Ty.path "usize" :=
                    M.call_closure (|
                      Ty.path "usize",
                      M.get_function (| "core::cmp::max", [], [ Ty.path "usize" ] |),
                      [
                        M.read (|
                          M.SubPointer.get_struct_record_field (|
                            M.deref (| M.read (| self |) |),
                            "core::num::bignum::Big32x40",
                            "size"
                          |)
                        |);
                        M.read (|
                          M.SubPointer.get_struct_record_field (|
                            M.deref (| M.read (| other |) |),
                            "core::num::bignum::Big32x40",
                            "size"
                          |)
                        |)
                      ]
                    |) in
                  let~ carry : Ty.path "bool" := Value.Bool false in
                  let~ _ : Ty.tuple [] :=
                    M.read (|
                      M.use
                        (M.alloc (|
                          Ty.tuple [],
                          M.match_operator (|
                            Ty.tuple [],
                            M.alloc (|
                              Ty.apply
                                (Ty.path "core::iter::adapters::zip::Zip")
                                []
                                [
                                  Ty.apply
                                    (Ty.path "core::slice::iter::IterMut")
                                    []
                                    [ Ty.path "u32" ];
                                  Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ]
                                ],
                              M.call_closure (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::zip::Zip")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::slice::iter::IterMut")
                                      []
                                      [ Ty.path "u32" ];
                                    Ty.apply
                                      (Ty.path "core::slice::iter::Iter")
                                      []
                                      [ Ty.path "u32" ]
                                  ],
                                M.get_trait_method (|
                                  "core::iter::traits::collect::IntoIterator",
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::zip::Zip")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::slice::iter::IterMut")
                                        []
                                        [ Ty.path "u32" ];
                                      Ty.apply
                                        (Ty.path "core::slice::iter::Iter")
                                        []
                                        [ Ty.path "u32" ]
                                    ],
                                  [],
                                  [],
                                  "into_iter",
                                  [],
                                  []
                                |),
                                [
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::zip::Zip")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::slice::iter::IterMut")
                                          []
                                          [ Ty.path "u32" ];
                                        Ty.apply
                                          (Ty.path "core::slice::iter::Iter")
                                          []
                                          [ Ty.path "u32" ]
                                      ],
                                    M.get_function (|
                                      "core::iter::adapters::zip::zip",
                                      [],
                                      [
                                        Ty.apply
                                          (Ty.path "&mut")
                                          []
                                          [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ];
                                        Ty.apply
                                          (Ty.path "&")
                                          []
                                          [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ]
                                      ]
                                    |),
                                    [
                                      M.borrow (|
                                        Pointer.Kind.MutRef,
                                        M.deref (|
                                          M.call_closure (|
                                            Ty.apply
                                              (Ty.path "&mut")
                                              []
                                              [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                            M.get_trait_method (|
                                              "core::ops::index::IndexMut",
                                              Ty.apply
                                                (Ty.path "array")
                                                [ Value.Integer IntegerKind.Usize 40 ]
                                                [ Ty.path "u32" ],
                                              [],
                                              [
                                                Ty.apply
                                                  (Ty.path "core::ops::range::RangeTo")
                                                  []
                                                  [ Ty.path "usize" ]
                                              ],
                                              "index_mut",
                                              [],
                                              []
                                            |),
                                            [
                                              M.borrow (|
                                                Pointer.Kind.MutRef,
                                                M.SubPointer.get_struct_record_field (|
                                                  M.deref (| M.read (| self |) |),
                                                  "core::num::bignum::Big32x40",
                                                  "base"
                                                |)
                                              |);
                                              Value.mkStructRecord
                                                "core::ops::range::RangeTo"
                                                []
                                                [ Ty.path "usize" ]
                                                [ ("end_", M.read (| sz |)) ]
                                            ]
                                          |)
                                        |)
                                      |);
                                      M.borrow (|
                                        Pointer.Kind.Ref,
                                        M.deref (|
                                          M.call_closure (|
                                            Ty.apply
                                              (Ty.path "&")
                                              []
                                              [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                            M.get_trait_method (|
                                              "core::ops::index::Index",
                                              Ty.apply
                                                (Ty.path "array")
                                                [ Value.Integer IntegerKind.Usize 40 ]
                                                [ Ty.path "u32" ],
                                              [],
                                              [
                                                Ty.apply
                                                  (Ty.path "core::ops::range::RangeTo")
                                                  []
                                                  [ Ty.path "usize" ]
                                              ],
                                              "index",
                                              [],
                                              []
                                            |),
                                            [
                                              M.borrow (|
                                                Pointer.Kind.Ref,
                                                M.SubPointer.get_struct_record_field (|
                                                  M.deref (| M.read (| other |) |),
                                                  "core::num::bignum::Big32x40",
                                                  "base"
                                                |)
                                              |);
                                              Value.mkStructRecord
                                                "core::ops::range::RangeTo"
                                                []
                                                [ Ty.path "usize" ]
                                                [ ("end_", M.read (| sz |)) ]
                                            ]
                                          |)
                                        |)
                                      |)
                                    ]
                                  |)
                                ]
                              |)
                            |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let~ iter :
                                      Ty.apply
                                        (Ty.path "core::iter::adapters::zip::Zip")
                                        []
                                        [
                                          Ty.apply
                                            (Ty.path "core::slice::iter::IterMut")
                                            []
                                            [ Ty.path "u32" ];
                                          Ty.apply
                                            (Ty.path "core::slice::iter::Iter")
                                            []
                                            [ Ty.path "u32" ]
                                        ] :=
                                    M.read (| γ |) in
                                  M.read (|
                                    M.loop (|
                                      Ty.tuple [],
                                      ltac:(M.monadic
                                        (let~ _ : Ty.tuple [] :=
                                          M.match_operator (|
                                            Ty.tuple [],
                                            M.alloc (|
                                              Ty.apply
                                                (Ty.path "core::option::Option")
                                                []
                                                [
                                                  Ty.tuple
                                                    [
                                                      Ty.apply
                                                        (Ty.path "&mut")
                                                        []
                                                        [ Ty.path "u32" ];
                                                      Ty.apply (Ty.path "&") [] [ Ty.path "u32" ]
                                                    ]
                                                ],
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [
                                                    Ty.tuple
                                                      [
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [ Ty.path "u32" ];
                                                        Ty.apply (Ty.path "&") [] [ Ty.path "u32" ]
                                                      ]
                                                  ],
                                                M.get_trait_method (|
                                                  "core::iter::traits::iterator::Iterator",
                                                  Ty.apply
                                                    (Ty.path "core::iter::adapters::zip::Zip")
                                                    []
                                                    [
                                                      Ty.apply
                                                        (Ty.path "core::slice::iter::IterMut")
                                                        []
                                                        [ Ty.path "u32" ];
                                                      Ty.apply
                                                        (Ty.path "core::slice::iter::Iter")
                                                        []
                                                        [ Ty.path "u32" ]
                                                    ],
                                                  [],
                                                  [],
                                                  "next",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.MutRef,
                                                    M.deref (|
                                                      M.borrow (| Pointer.Kind.MutRef, iter |)
                                                    |)
                                                  |)
                                                ]
                                              |)
                                            |),
                                            [
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let _ :=
                                                    M.is_struct_tuple (|
                                                      γ,
                                                      "core::option::Option::None"
                                                    |) in
                                                  M.never_to_any (| M.read (| M.break (||) |) |)));
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let γ0_0 :=
                                                    M.SubPointer.get_struct_tuple_field (|
                                                      γ,
                                                      "core::option::Option::Some",
                                                      0
                                                    |) in
                                                  let γ1_0 :=
                                                    M.SubPointer.get_tuple_field (| γ0_0, 0 |) in
                                                  let γ1_1 :=
                                                    M.SubPointer.get_tuple_field (| γ0_0, 1 |) in
                                                  let a :=
                                                    M.copy (|
                                                      Ty.apply
                                                        (Ty.path "&mut")
                                                        []
                                                        [ Ty.path "u32" ],
                                                      γ1_0
                                                    |) in
                                                  let b :=
                                                    M.copy (|
                                                      Ty.apply (Ty.path "&") [] [ Ty.path "u32" ],
                                                      γ1_1
                                                    |) in
                                                  M.match_operator (|
                                                    Ty.tuple [],
                                                    M.alloc (|
                                                      Ty.tuple [ Ty.path "u32"; Ty.path "bool" ],
                                                      M.call_closure (|
                                                        Ty.tuple [ Ty.path "u32"; Ty.path "bool" ],
                                                        M.get_associated_function (|
                                                          Ty.path "u32",
                                                          "carrying_add",
                                                          [],
                                                          []
                                                        |),
                                                        [
                                                          M.read (| M.deref (| M.read (| a |) |) |);
                                                          M.read (| M.deref (| M.read (| b |) |) |);
                                                          M.read (| carry |)
                                                        ]
                                                      |)
                                                    |),
                                                    [
                                                      fun γ =>
                                                        ltac:(M.monadic
                                                          (let γ0_0 :=
                                                            M.SubPointer.get_tuple_field (|
                                                              γ,
                                                              0
                                                            |) in
                                                          let γ0_1 :=
                                                            M.SubPointer.get_tuple_field (|
                                                              γ,
                                                              1
                                                            |) in
                                                          let v :=
                                                            M.copy (| Ty.path "u32", γ0_0 |) in
                                                          let c :=
                                                            M.copy (| Ty.path "bool", γ0_1 |) in
                                                          M.read (|
                                                            let~ _ : Ty.tuple [] :=
                                                              M.write (|
                                                                M.deref (| M.read (| a |) |),
                                                                M.read (| v |)
                                                              |) in
                                                            let~ _ : Ty.tuple [] :=
                                                              M.write (| carry, M.read (| c |) |) in
                                                            M.alloc (|
                                                              Ty.tuple [],
                                                              Value.Tuple []
                                                            |)
                                                          |)))
                                                    ]
                                                  |)))
                                            ]
                                          |) in
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                    |)
                                  |)))
                            ]
                          |)
                        |))
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.match_operator (|
                      Ty.tuple [],
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ := carry in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            M.read (|
                              let~ _ : Ty.tuple [] :=
                                M.write (|
                                  M.SubPointer.get_array_field (|
                                    M.SubPointer.get_struct_record_field (|
                                      M.deref (| M.read (| self |) |),
                                      "core::num::bignum::Big32x40",
                                      "base"
                                    |),
                                    M.read (| sz |)
                                  |),
                                  Value.Integer IntegerKind.U32 1
                                |) in
                              let~ _ : Ty.tuple [] :=
                                let β := sz in
                                M.write (|
                                  β,
                                  M.call_closure (|
                                    Ty.path "usize",
                                    BinOp.Wrap.add,
                                    [ M.read (| β |); Value.Integer IntegerKind.Usize 1 ]
                                  |)
                                |) in
                              M.alloc (| Ty.tuple [], Value.Tuple [] |)
                            |)));
                        fun γ => ltac:(M.monadic (Value.Tuple []))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.write (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::Big32x40",
                        "size"
                      |),
                      M.read (| sz |)
                    |) in
                  M.alloc (|
                    Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                    M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                  |)
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_add : M.IsAssociatedFunction.C Self "add" add.
      Admitted.
      Global Typeclasses Opaque add.
      
      (*
                  pub fn add_small(&mut self, other: $ty) -> &mut $name {
                      let (v, mut carry) = self.base[0].carrying_add(other, false);
                      self.base[0] = v;
                      let mut i = 1;
                      while carry {
                          let (v, c) = self.base[i].carrying_add(0, carry);
                          self.base[i] = v;
                          carry = c;
                          i += 1;
                      }
                      if i > self.size {
                          self.size = i;
                      }
                      self
                  }
      *)
      Definition add_small (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other := M.alloc (| Ty.path "u32", other |) in
            M.borrow (|
              Pointer.Kind.MutRef,
              M.deref (|
                M.match_operator (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                  M.alloc (|
                    Ty.tuple [ Ty.path "u32"; Ty.path "bool" ],
                    M.call_closure (|
                      Ty.tuple [ Ty.path "u32"; Ty.path "bool" ],
                      M.get_associated_function (| Ty.path "u32", "carrying_add", [], [] |),
                      [
                        M.read (|
                          M.SubPointer.get_array_field (|
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| self |) |),
                              "core::num::bignum::Big32x40",
                              "base"
                            |),
                            Value.Integer IntegerKind.Usize 0
                          |)
                        |);
                        M.read (| other |);
                        Value.Bool false
                      ]
                    |)
                  |),
                  [
                    fun γ =>
                      ltac:(M.monadic
                        (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                        let γ0_1 := M.SubPointer.get_tuple_field (| γ, 1 |) in
                        let v := M.copy (| Ty.path "u32", γ0_0 |) in
                        let~ carry : Ty.path "bool" := M.read (| γ0_1 |) in
                        M.read (|
                          let~ _ : Ty.tuple [] :=
                            M.write (|
                              M.SubPointer.get_array_field (|
                                M.SubPointer.get_struct_record_field (|
                                  M.deref (| M.read (| self |) |),
                                  "core::num::bignum::Big32x40",
                                  "base"
                                |),
                                Value.Integer IntegerKind.Usize 0
                              |),
                              M.read (| v |)
                            |) in
                          let~ i : Ty.path "usize" := Value.Integer IntegerKind.Usize 1 in
                          let~ _ : Ty.tuple [] :=
                            M.read (|
                              M.loop (|
                                Ty.tuple [],
                                ltac:(M.monadic
                                  (M.alloc (|
                                    Ty.tuple [],
                                    M.match_operator (|
                                      Ty.tuple [],
                                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                      [
                                        fun γ =>
                                          ltac:(M.monadic
                                            (let γ := carry in
                                            let _ :=
                                              is_constant_or_break_match (|
                                                M.read (| γ |),
                                                Value.Bool true
                                              |) in
                                            M.match_operator (|
                                              Ty.tuple [],
                                              M.alloc (|
                                                Ty.tuple [ Ty.path "u32"; Ty.path "bool" ],
                                                M.call_closure (|
                                                  Ty.tuple [ Ty.path "u32"; Ty.path "bool" ],
                                                  M.get_associated_function (|
                                                    Ty.path "u32",
                                                    "carrying_add",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    M.read (|
                                                      M.SubPointer.get_array_field (|
                                                        M.SubPointer.get_struct_record_field (|
                                                          M.deref (| M.read (| self |) |),
                                                          "core::num::bignum::Big32x40",
                                                          "base"
                                                        |),
                                                        M.read (| i |)
                                                      |)
                                                    |);
                                                    Value.Integer IntegerKind.U32 0;
                                                    M.read (| carry |)
                                                  ]
                                                |)
                                              |),
                                              [
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let γ0_0 :=
                                                      M.SubPointer.get_tuple_field (| γ, 0 |) in
                                                    let γ0_1 :=
                                                      M.SubPointer.get_tuple_field (| γ, 1 |) in
                                                    let v := M.copy (| Ty.path "u32", γ0_0 |) in
                                                    let c := M.copy (| Ty.path "bool", γ0_1 |) in
                                                    M.read (|
                                                      let~ _ : Ty.tuple [] :=
                                                        M.write (|
                                                          M.SubPointer.get_array_field (|
                                                            M.SubPointer.get_struct_record_field (|
                                                              M.deref (| M.read (| self |) |),
                                                              "core::num::bignum::Big32x40",
                                                              "base"
                                                            |),
                                                            M.read (| i |)
                                                          |),
                                                          M.read (| v |)
                                                        |) in
                                                      let~ _ : Ty.tuple [] :=
                                                        M.write (| carry, M.read (| c |) |) in
                                                      let~ _ : Ty.tuple [] :=
                                                        let β := i in
                                                        M.write (|
                                                          β,
                                                          M.call_closure (|
                                                            Ty.path "usize",
                                                            BinOp.Wrap.add,
                                                            [
                                                              M.read (| β |);
                                                              Value.Integer IntegerKind.Usize 1
                                                            ]
                                                          |)
                                                        |) in
                                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                    |)))
                                              ]
                                            |)));
                                        fun γ =>
                                          ltac:(M.monadic
                                            (M.never_to_any (|
                                              M.read (|
                                                let~ _ : Ty.tuple [] :=
                                                  M.never_to_any (| M.read (| M.break (||) |) |) in
                                                M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                              |)
                                            |)))
                                      ]
                                    |)
                                  |)))
                              |)
                            |) in
                          let~ _ : Ty.tuple [] :=
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (| Ty.tuple [], Value.Tuple [] |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let γ :=
                                      M.alloc (|
                                        Ty.path "bool",
                                        M.call_closure (|
                                          Ty.path "bool",
                                          BinOp.gt,
                                          [
                                            M.read (| i |);
                                            M.read (|
                                              M.SubPointer.get_struct_record_field (|
                                                M.deref (| M.read (| self |) |),
                                                "core::num::bignum::Big32x40",
                                                "size"
                                              |)
                                            |)
                                          ]
                                        |)
                                      |) in
                                    let _ :=
                                      is_constant_or_break_match (|
                                        M.read (| γ |),
                                        Value.Bool true
                                      |) in
                                    M.read (|
                                      let~ _ : Ty.tuple [] :=
                                        M.write (|
                                          M.SubPointer.get_struct_record_field (|
                                            M.deref (| M.read (| self |) |),
                                            "core::num::bignum::Big32x40",
                                            "size"
                                          |),
                                          M.read (| i |)
                                        |) in
                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                    |)));
                                fun γ => ltac:(M.monadic (Value.Tuple []))
                              ]
                            |) in
                          M.alloc (|
                            Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                            M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                          |)
                        |)))
                  ]
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_add_small :
        M.IsAssociatedFunction.C Self "add_small" add_small.
      Admitted.
      Global Typeclasses Opaque add_small.
      
      (*
                  pub fn sub<'a>(&'a mut self, other: &$name) -> &'a mut $name {
                      use crate::{cmp, iter};
      
                      let sz = cmp::max(self.size, other.size);
                      let mut noborrow = true;
                      for (a, b) in iter::zip(&mut self.base[..sz], &other.base[..sz]) {
                          let (v, c) = ( *a).carrying_add(!*b, noborrow);
                          *a = v;
                          noborrow = c;
                      }
                      assert!(noborrow);
                      self.size = sz;
                      self
                  }
      *)
      Definition sub (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                other
              |) in
            M.borrow (|
              Pointer.Kind.MutRef,
              M.deref (|
                M.read (|
                  let~ sz : Ty.path "usize" :=
                    M.call_closure (|
                      Ty.path "usize",
                      M.get_function (| "core::cmp::max", [], [ Ty.path "usize" ] |),
                      [
                        M.read (|
                          M.SubPointer.get_struct_record_field (|
                            M.deref (| M.read (| self |) |),
                            "core::num::bignum::Big32x40",
                            "size"
                          |)
                        |);
                        M.read (|
                          M.SubPointer.get_struct_record_field (|
                            M.deref (| M.read (| other |) |),
                            "core::num::bignum::Big32x40",
                            "size"
                          |)
                        |)
                      ]
                    |) in
                  let~ noborrow : Ty.path "bool" := Value.Bool true in
                  let~ _ : Ty.tuple [] :=
                    M.read (|
                      M.use
                        (M.alloc (|
                          Ty.tuple [],
                          M.match_operator (|
                            Ty.tuple [],
                            M.alloc (|
                              Ty.apply
                                (Ty.path "core::iter::adapters::zip::Zip")
                                []
                                [
                                  Ty.apply
                                    (Ty.path "core::slice::iter::IterMut")
                                    []
                                    [ Ty.path "u32" ];
                                  Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ]
                                ],
                              M.call_closure (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::zip::Zip")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::slice::iter::IterMut")
                                      []
                                      [ Ty.path "u32" ];
                                    Ty.apply
                                      (Ty.path "core::slice::iter::Iter")
                                      []
                                      [ Ty.path "u32" ]
                                  ],
                                M.get_trait_method (|
                                  "core::iter::traits::collect::IntoIterator",
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::zip::Zip")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::slice::iter::IterMut")
                                        []
                                        [ Ty.path "u32" ];
                                      Ty.apply
                                        (Ty.path "core::slice::iter::Iter")
                                        []
                                        [ Ty.path "u32" ]
                                    ],
                                  [],
                                  [],
                                  "into_iter",
                                  [],
                                  []
                                |),
                                [
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::zip::Zip")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::slice::iter::IterMut")
                                          []
                                          [ Ty.path "u32" ];
                                        Ty.apply
                                          (Ty.path "core::slice::iter::Iter")
                                          []
                                          [ Ty.path "u32" ]
                                      ],
                                    M.get_function (|
                                      "core::iter::adapters::zip::zip",
                                      [],
                                      [
                                        Ty.apply
                                          (Ty.path "&mut")
                                          []
                                          [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ];
                                        Ty.apply
                                          (Ty.path "&")
                                          []
                                          [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ]
                                      ]
                                    |),
                                    [
                                      M.borrow (|
                                        Pointer.Kind.MutRef,
                                        M.deref (|
                                          M.call_closure (|
                                            Ty.apply
                                              (Ty.path "&mut")
                                              []
                                              [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                            M.get_trait_method (|
                                              "core::ops::index::IndexMut",
                                              Ty.apply
                                                (Ty.path "array")
                                                [ Value.Integer IntegerKind.Usize 40 ]
                                                [ Ty.path "u32" ],
                                              [],
                                              [
                                                Ty.apply
                                                  (Ty.path "core::ops::range::RangeTo")
                                                  []
                                                  [ Ty.path "usize" ]
                                              ],
                                              "index_mut",
                                              [],
                                              []
                                            |),
                                            [
                                              M.borrow (|
                                                Pointer.Kind.MutRef,
                                                M.SubPointer.get_struct_record_field (|
                                                  M.deref (| M.read (| self |) |),
                                                  "core::num::bignum::Big32x40",
                                                  "base"
                                                |)
                                              |);
                                              Value.mkStructRecord
                                                "core::ops::range::RangeTo"
                                                []
                                                [ Ty.path "usize" ]
                                                [ ("end_", M.read (| sz |)) ]
                                            ]
                                          |)
                                        |)
                                      |);
                                      M.borrow (|
                                        Pointer.Kind.Ref,
                                        M.deref (|
                                          M.call_closure (|
                                            Ty.apply
                                              (Ty.path "&")
                                              []
                                              [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                            M.get_trait_method (|
                                              "core::ops::index::Index",
                                              Ty.apply
                                                (Ty.path "array")
                                                [ Value.Integer IntegerKind.Usize 40 ]
                                                [ Ty.path "u32" ],
                                              [],
                                              [
                                                Ty.apply
                                                  (Ty.path "core::ops::range::RangeTo")
                                                  []
                                                  [ Ty.path "usize" ]
                                              ],
                                              "index",
                                              [],
                                              []
                                            |),
                                            [
                                              M.borrow (|
                                                Pointer.Kind.Ref,
                                                M.SubPointer.get_struct_record_field (|
                                                  M.deref (| M.read (| other |) |),
                                                  "core::num::bignum::Big32x40",
                                                  "base"
                                                |)
                                              |);
                                              Value.mkStructRecord
                                                "core::ops::range::RangeTo"
                                                []
                                                [ Ty.path "usize" ]
                                                [ ("end_", M.read (| sz |)) ]
                                            ]
                                          |)
                                        |)
                                      |)
                                    ]
                                  |)
                                ]
                              |)
                            |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let~ iter :
                                      Ty.apply
                                        (Ty.path "core::iter::adapters::zip::Zip")
                                        []
                                        [
                                          Ty.apply
                                            (Ty.path "core::slice::iter::IterMut")
                                            []
                                            [ Ty.path "u32" ];
                                          Ty.apply
                                            (Ty.path "core::slice::iter::Iter")
                                            []
                                            [ Ty.path "u32" ]
                                        ] :=
                                    M.read (| γ |) in
                                  M.read (|
                                    M.loop (|
                                      Ty.tuple [],
                                      ltac:(M.monadic
                                        (let~ _ : Ty.tuple [] :=
                                          M.match_operator (|
                                            Ty.tuple [],
                                            M.alloc (|
                                              Ty.apply
                                                (Ty.path "core::option::Option")
                                                []
                                                [
                                                  Ty.tuple
                                                    [
                                                      Ty.apply
                                                        (Ty.path "&mut")
                                                        []
                                                        [ Ty.path "u32" ];
                                                      Ty.apply (Ty.path "&") [] [ Ty.path "u32" ]
                                                    ]
                                                ],
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [
                                                    Ty.tuple
                                                      [
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [ Ty.path "u32" ];
                                                        Ty.apply (Ty.path "&") [] [ Ty.path "u32" ]
                                                      ]
                                                  ],
                                                M.get_trait_method (|
                                                  "core::iter::traits::iterator::Iterator",
                                                  Ty.apply
                                                    (Ty.path "core::iter::adapters::zip::Zip")
                                                    []
                                                    [
                                                      Ty.apply
                                                        (Ty.path "core::slice::iter::IterMut")
                                                        []
                                                        [ Ty.path "u32" ];
                                                      Ty.apply
                                                        (Ty.path "core::slice::iter::Iter")
                                                        []
                                                        [ Ty.path "u32" ]
                                                    ],
                                                  [],
                                                  [],
                                                  "next",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.MutRef,
                                                    M.deref (|
                                                      M.borrow (| Pointer.Kind.MutRef, iter |)
                                                    |)
                                                  |)
                                                ]
                                              |)
                                            |),
                                            [
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let _ :=
                                                    M.is_struct_tuple (|
                                                      γ,
                                                      "core::option::Option::None"
                                                    |) in
                                                  M.never_to_any (| M.read (| M.break (||) |) |)));
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let γ0_0 :=
                                                    M.SubPointer.get_struct_tuple_field (|
                                                      γ,
                                                      "core::option::Option::Some",
                                                      0
                                                    |) in
                                                  let γ1_0 :=
                                                    M.SubPointer.get_tuple_field (| γ0_0, 0 |) in
                                                  let γ1_1 :=
                                                    M.SubPointer.get_tuple_field (| γ0_0, 1 |) in
                                                  let a :=
                                                    M.copy (|
                                                      Ty.apply
                                                        (Ty.path "&mut")
                                                        []
                                                        [ Ty.path "u32" ],
                                                      γ1_0
                                                    |) in
                                                  let b :=
                                                    M.copy (|
                                                      Ty.apply (Ty.path "&") [] [ Ty.path "u32" ],
                                                      γ1_1
                                                    |) in
                                                  M.match_operator (|
                                                    Ty.tuple [],
                                                    M.alloc (|
                                                      Ty.tuple [ Ty.path "u32"; Ty.path "bool" ],
                                                      M.call_closure (|
                                                        Ty.tuple [ Ty.path "u32"; Ty.path "bool" ],
                                                        M.get_associated_function (|
                                                          Ty.path "u32",
                                                          "carrying_add",
                                                          [],
                                                          []
                                                        |),
                                                        [
                                                          M.read (| M.deref (| M.read (| a |) |) |);
                                                          M.call_closure (|
                                                            Ty.path "u32",
                                                            UnOp.not,
                                                            [
                                                              M.read (|
                                                                M.deref (| M.read (| b |) |)
                                                              |)
                                                            ]
                                                          |);
                                                          M.read (| noborrow |)
                                                        ]
                                                      |)
                                                    |),
                                                    [
                                                      fun γ =>
                                                        ltac:(M.monadic
                                                          (let γ0_0 :=
                                                            M.SubPointer.get_tuple_field (|
                                                              γ,
                                                              0
                                                            |) in
                                                          let γ0_1 :=
                                                            M.SubPointer.get_tuple_field (|
                                                              γ,
                                                              1
                                                            |) in
                                                          let v :=
                                                            M.copy (| Ty.path "u32", γ0_0 |) in
                                                          let c :=
                                                            M.copy (| Ty.path "bool", γ0_1 |) in
                                                          M.read (|
                                                            let~ _ : Ty.tuple [] :=
                                                              M.write (|
                                                                M.deref (| M.read (| a |) |),
                                                                M.read (| v |)
                                                              |) in
                                                            let~ _ : Ty.tuple [] :=
                                                              M.write (|
                                                                noborrow,
                                                                M.read (| c |)
                                                              |) in
                                                            M.alloc (|
                                                              Ty.tuple [],
                                                              Value.Tuple []
                                                            |)
                                                          |)))
                                                    ]
                                                  |)))
                                            ]
                                          |) in
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                    |)
                                  |)))
                            ]
                          |)
                        |))
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.match_operator (|
                      Ty.tuple [],
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ :=
                              M.alloc (|
                                Ty.path "bool",
                                M.call_closure (|
                                  Ty.path "bool",
                                  UnOp.not,
                                  [ M.read (| noborrow |) ]
                                |)
                              |) in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            M.never_to_any (|
                              M.call_closure (|
                                Ty.path "never",
                                M.get_function (| "core::panicking::panic", [], [] |),
                                [ mk_str (| "assertion failed: noborrow" |) ]
                              |)
                            |)));
                        fun γ => ltac:(M.monadic (Value.Tuple []))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.write (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::Big32x40",
                        "size"
                      |),
                      M.read (| sz |)
                    |) in
                  M.alloc (|
                    Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                    M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                  |)
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_sub : M.IsAssociatedFunction.C Self "sub" sub.
      Admitted.
      Global Typeclasses Opaque sub.
      
      (*
                  pub fn mul_small(&mut self, other: $ty) -> &mut $name {
                      let mut sz = self.size;
                      let mut carry = 0;
                      for a in &mut self.base[..sz] {
                          let (v, c) = ( *a).carrying_mul(other, carry);
                          *a = v;
                          carry = c;
                      }
                      if carry > 0 {
                          self.base[sz] = carry;
                          sz += 1;
                      }
                      self.size = sz;
                      self
                  }
      *)
      Definition mul_small (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other := M.alloc (| Ty.path "u32", other |) in
            M.borrow (|
              Pointer.Kind.MutRef,
              M.deref (|
                M.read (|
                  let~ sz : Ty.path "usize" :=
                    M.read (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::Big32x40",
                        "size"
                      |)
                    |) in
                  let~ carry : Ty.path "u32" := Value.Integer IntegerKind.U32 0 in
                  let~ _ : Ty.tuple [] :=
                    M.read (|
                      M.use
                        (M.alloc (|
                          Ty.tuple [],
                          M.match_operator (|
                            Ty.tuple [],
                            M.alloc (|
                              Ty.apply (Ty.path "core::slice::iter::IterMut") [] [ Ty.path "u32" ],
                              M.call_closure (|
                                Ty.apply
                                  (Ty.path "core::slice::iter::IterMut")
                                  []
                                  [ Ty.path "u32" ],
                                M.get_trait_method (|
                                  "core::iter::traits::collect::IntoIterator",
                                  Ty.apply
                                    (Ty.path "&mut")
                                    []
                                    [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                  [],
                                  [],
                                  "into_iter",
                                  [],
                                  []
                                |),
                                [
                                  M.borrow (|
                                    Pointer.Kind.MutRef,
                                    M.deref (|
                                      M.call_closure (|
                                        Ty.apply
                                          (Ty.path "&mut")
                                          []
                                          [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                        M.get_trait_method (|
                                          "core::ops::index::IndexMut",
                                          Ty.apply
                                            (Ty.path "array")
                                            [ Value.Integer IntegerKind.Usize 40 ]
                                            [ Ty.path "u32" ],
                                          [],
                                          [
                                            Ty.apply
                                              (Ty.path "core::ops::range::RangeTo")
                                              []
                                              [ Ty.path "usize" ]
                                          ],
                                          "index_mut",
                                          [],
                                          []
                                        |),
                                        [
                                          M.borrow (|
                                            Pointer.Kind.MutRef,
                                            M.SubPointer.get_struct_record_field (|
                                              M.deref (| M.read (| self |) |),
                                              "core::num::bignum::Big32x40",
                                              "base"
                                            |)
                                          |);
                                          Value.mkStructRecord
                                            "core::ops::range::RangeTo"
                                            []
                                            [ Ty.path "usize" ]
                                            [ ("end_", M.read (| sz |)) ]
                                        ]
                                      |)
                                    |)
                                  |)
                                ]
                              |)
                            |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let~ iter :
                                      Ty.apply
                                        (Ty.path "core::slice::iter::IterMut")
                                        []
                                        [ Ty.path "u32" ] :=
                                    M.read (| γ |) in
                                  M.read (|
                                    M.loop (|
                                      Ty.tuple [],
                                      ltac:(M.monadic
                                        (let~ _ : Ty.tuple [] :=
                                          M.match_operator (|
                                            Ty.tuple [],
                                            M.alloc (|
                                              Ty.apply
                                                (Ty.path "core::option::Option")
                                                []
                                                [ Ty.apply (Ty.path "&mut") [] [ Ty.path "u32" ] ],
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [ Ty.apply (Ty.path "&mut") [] [ Ty.path "u32" ]
                                                  ],
                                                M.get_trait_method (|
                                                  "core::iter::traits::iterator::Iterator",
                                                  Ty.apply
                                                    (Ty.path "core::slice::iter::IterMut")
                                                    []
                                                    [ Ty.path "u32" ],
                                                  [],
                                                  [],
                                                  "next",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.MutRef,
                                                    M.deref (|
                                                      M.borrow (| Pointer.Kind.MutRef, iter |)
                                                    |)
                                                  |)
                                                ]
                                              |)
                                            |),
                                            [
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let _ :=
                                                    M.is_struct_tuple (|
                                                      γ,
                                                      "core::option::Option::None"
                                                    |) in
                                                  M.never_to_any (| M.read (| M.break (||) |) |)));
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let γ0_0 :=
                                                    M.SubPointer.get_struct_tuple_field (|
                                                      γ,
                                                      "core::option::Option::Some",
                                                      0
                                                    |) in
                                                  let a :=
                                                    M.copy (|
                                                      Ty.apply
                                                        (Ty.path "&mut")
                                                        []
                                                        [ Ty.path "u32" ],
                                                      γ0_0
                                                    |) in
                                                  M.match_operator (|
                                                    Ty.tuple [],
                                                    M.alloc (|
                                                      Ty.tuple [ Ty.path "u32"; Ty.path "u32" ],
                                                      M.call_closure (|
                                                        Ty.tuple [ Ty.path "u32"; Ty.path "u32" ],
                                                        M.get_associated_function (|
                                                          Ty.path "u32",
                                                          "carrying_mul",
                                                          [],
                                                          []
                                                        |),
                                                        [
                                                          M.read (| M.deref (| M.read (| a |) |) |);
                                                          M.read (| other |);
                                                          M.read (| carry |)
                                                        ]
                                                      |)
                                                    |),
                                                    [
                                                      fun γ =>
                                                        ltac:(M.monadic
                                                          (let γ0_0 :=
                                                            M.SubPointer.get_tuple_field (|
                                                              γ,
                                                              0
                                                            |) in
                                                          let γ0_1 :=
                                                            M.SubPointer.get_tuple_field (|
                                                              γ,
                                                              1
                                                            |) in
                                                          let v :=
                                                            M.copy (| Ty.path "u32", γ0_0 |) in
                                                          let c :=
                                                            M.copy (| Ty.path "u32", γ0_1 |) in
                                                          M.read (|
                                                            let~ _ : Ty.tuple [] :=
                                                              M.write (|
                                                                M.deref (| M.read (| a |) |),
                                                                M.read (| v |)
                                                              |) in
                                                            let~ _ : Ty.tuple [] :=
                                                              M.write (| carry, M.read (| c |) |) in
                                                            M.alloc (|
                                                              Ty.tuple [],
                                                              Value.Tuple []
                                                            |)
                                                          |)))
                                                    ]
                                                  |)))
                                            ]
                                          |) in
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                    |)
                                  |)))
                            ]
                          |)
                        |))
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.match_operator (|
                      Ty.tuple [],
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ :=
                              M.alloc (|
                                Ty.path "bool",
                                M.call_closure (|
                                  Ty.path "bool",
                                  BinOp.gt,
                                  [ M.read (| carry |); Value.Integer IntegerKind.U32 0 ]
                                |)
                              |) in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            M.read (|
                              let~ _ : Ty.tuple [] :=
                                M.write (|
                                  M.SubPointer.get_array_field (|
                                    M.SubPointer.get_struct_record_field (|
                                      M.deref (| M.read (| self |) |),
                                      "core::num::bignum::Big32x40",
                                      "base"
                                    |),
                                    M.read (| sz |)
                                  |),
                                  M.read (| carry |)
                                |) in
                              let~ _ : Ty.tuple [] :=
                                let β := sz in
                                M.write (|
                                  β,
                                  M.call_closure (|
                                    Ty.path "usize",
                                    BinOp.Wrap.add,
                                    [ M.read (| β |); Value.Integer IntegerKind.Usize 1 ]
                                  |)
                                |) in
                              M.alloc (| Ty.tuple [], Value.Tuple [] |)
                            |)));
                        fun γ => ltac:(M.monadic (Value.Tuple []))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.write (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::Big32x40",
                        "size"
                      |),
                      M.read (| sz |)
                    |) in
                  M.alloc (|
                    Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                    M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                  |)
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_mul_small :
        M.IsAssociatedFunction.C Self "mul_small" mul_small.
      Admitted.
      Global Typeclasses Opaque mul_small.
      
      (*
                  pub fn mul_pow2(&mut self, bits: usize) -> &mut $name {
                      let digitbits = <$ty>::BITS as usize;
                      let digits = bits / digitbits;
                      let bits = bits % digitbits;
      
                      assert!(digits < $n);
                      debug_assert!(self.base[$n - digits..].iter().all(|&v| v == 0));
                      debug_assert!(bits == 0 || (self.base[$n - digits - 1] >> (digitbits - bits)) == 0);
      
                      // shift by `digits * digitbits` bits
                      for i in (0..self.size).rev() {
                          self.base[i + digits] = self.base[i];
                      }
                      for i in 0..digits {
                          self.base[i] = 0;
                      }
      
                      // shift by `bits` bits
                      let mut sz = self.size + digits;
                      if bits > 0 {
                          let last = sz;
                          let overflow = self.base[last - 1] >> (digitbits - bits);
                          if overflow > 0 {
                              self.base[last] = overflow;
                              sz += 1;
                          }
                          for i in (digits + 1..last).rev() {
                              self.base[i] =
                                  (self.base[i] << bits) | (self.base[i - 1] >> (digitbits - bits));
                          }
                          self.base[digits] <<= bits;
                          // self.base[..digits] is zero, no need to shift
                      }
      
                      self.size = sz;
                      self
                  }
      *)
      Definition mul_pow2 (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; bits ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let bits := M.alloc (| Ty.path "usize", bits |) in
            M.borrow (|
              Pointer.Kind.MutRef,
              M.deref (|
                M.read (|
                  let~ digitbits : Ty.path "usize" :=
                    M.cast
                      (Ty.path "usize")
                      (M.read (|
                        get_associated_constant (| Ty.path "u32", "BITS", Ty.path "u32" |)
                      |)) in
                  let~ digits : Ty.path "usize" :=
                    M.call_closure (|
                      Ty.path "usize",
                      BinOp.Wrap.div,
                      [ M.read (| bits |); M.read (| digitbits |) ]
                    |) in
                  let~ bits : Ty.path "usize" :=
                    M.call_closure (|
                      Ty.path "usize",
                      BinOp.Wrap.rem,
                      [ M.read (| bits |); M.read (| digitbits |) ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.match_operator (|
                      Ty.tuple [],
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ :=
                              M.alloc (|
                                Ty.path "bool",
                                M.call_closure (|
                                  Ty.path "bool",
                                  UnOp.not,
                                  [
                                    M.call_closure (|
                                      Ty.path "bool",
                                      BinOp.lt,
                                      [ M.read (| digits |); Value.Integer IntegerKind.Usize 40 ]
                                    |)
                                  ]
                                |)
                              |) in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            M.never_to_any (|
                              M.call_closure (|
                                Ty.path "never",
                                M.get_function (| "core::panicking::panic", [], [] |),
                                [ mk_str (| "assertion failed: digits < 40" |) ]
                              |)
                            |)));
                        fun γ => ltac:(M.monadic (Value.Tuple []))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.match_operator (|
                      Ty.tuple [],
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ := M.alloc (| Ty.path "bool", Value.Bool true |) in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            M.read (|
                              let~ _ : Ty.tuple [] :=
                                M.match_operator (|
                                  Ty.tuple [],
                                  M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                  [
                                    fun γ =>
                                      ltac:(M.monadic
                                        (let γ :=
                                          M.alloc (|
                                            Ty.path "bool",
                                            M.call_closure (|
                                              Ty.path "bool",
                                              UnOp.not,
                                              [
                                                M.call_closure (|
                                                  Ty.path "bool",
                                                  M.get_trait_method (|
                                                    "core::iter::traits::iterator::Iterator",
                                                    Ty.apply
                                                      (Ty.path "core::slice::iter::Iter")
                                                      []
                                                      [ Ty.path "u32" ],
                                                    [],
                                                    [],
                                                    "all",
                                                    [],
                                                    [
                                                      Ty.function
                                                        [
                                                          Ty.apply
                                                            (Ty.path "&")
                                                            []
                                                            [ Ty.path "u32" ]
                                                        ]
                                                        (Ty.path "bool")
                                                    ]
                                                  |),
                                                  [
                                                    M.borrow (|
                                                      Pointer.Kind.MutRef,
                                                      M.alloc (|
                                                        Ty.apply
                                                          (Ty.path "core::slice::iter::Iter")
                                                          []
                                                          [ Ty.path "u32" ],
                                                        M.call_closure (|
                                                          Ty.apply
                                                            (Ty.path "core::slice::iter::Iter")
                                                            []
                                                            [ Ty.path "u32" ],
                                                          M.get_associated_function (|
                                                            Ty.apply
                                                              (Ty.path "slice")
                                                              []
                                                              [ Ty.path "u32" ],
                                                            "iter",
                                                            [],
                                                            []
                                                          |),
                                                          [
                                                            M.borrow (|
                                                              Pointer.Kind.Ref,
                                                              M.deref (|
                                                                M.call_closure (|
                                                                  Ty.apply
                                                                    (Ty.path "&")
                                                                    []
                                                                    [
                                                                      Ty.apply
                                                                        (Ty.path "slice")
                                                                        []
                                                                        [ Ty.path "u32" ]
                                                                    ],
                                                                  M.get_trait_method (|
                                                                    "core::ops::index::Index",
                                                                    Ty.apply
                                                                      (Ty.path "array")
                                                                      [
                                                                        Value.Integer
                                                                          IntegerKind.Usize
                                                                          40
                                                                      ]
                                                                      [ Ty.path "u32" ],
                                                                    [],
                                                                    [
                                                                      Ty.apply
                                                                        (Ty.path
                                                                          "core::ops::range::RangeFrom")
                                                                        []
                                                                        [ Ty.path "usize" ]
                                                                    ],
                                                                    "index",
                                                                    [],
                                                                    []
                                                                  |),
                                                                  [
                                                                    M.borrow (|
                                                                      Pointer.Kind.Ref,
                                                                      M.SubPointer.get_struct_record_field (|
                                                                        M.deref (|
                                                                          M.read (| self |)
                                                                        |),
                                                                        "core::num::bignum::Big32x40",
                                                                        "base"
                                                                      |)
                                                                    |);
                                                                    Value.mkStructRecord
                                                                      "core::ops::range::RangeFrom"
                                                                      []
                                                                      [ Ty.path "usize" ]
                                                                      [
                                                                        ("start",
                                                                          M.call_closure (|
                                                                            Ty.path "usize",
                                                                            BinOp.Wrap.sub,
                                                                            [
                                                                              Value.Integer
                                                                                IntegerKind.Usize
                                                                                40;
                                                                              M.read (| digits |)
                                                                            ]
                                                                          |))
                                                                      ]
                                                                  ]
                                                                |)
                                                              |)
                                                            |)
                                                          ]
                                                        |)
                                                      |)
                                                    |);
                                                    M.closure
                                                      (fun γ =>
                                                        ltac:(M.monadic
                                                          match γ with
                                                          | [ α0 ] =>
                                                            ltac:(M.monadic
                                                              (M.match_operator (|
                                                                Ty.path "bool",
                                                                M.alloc (|
                                                                  Ty.apply
                                                                    (Ty.path "&")
                                                                    []
                                                                    [ Ty.path "u32" ],
                                                                  α0
                                                                |),
                                                                [
                                                                  fun γ =>
                                                                    ltac:(M.monadic
                                                                      (let γ :=
                                                                        M.deref (|
                                                                          M.read (| γ |)
                                                                        |) in
                                                                      let v :=
                                                                        M.copy (|
                                                                          Ty.path "u32",
                                                                          γ
                                                                        |) in
                                                                      M.call_closure (|
                                                                        Ty.path "bool",
                                                                        BinOp.eq,
                                                                        [
                                                                          M.read (| v |);
                                                                          Value.Integer
                                                                            IntegerKind.U32
                                                                            0
                                                                        ]
                                                                      |)))
                                                                ]
                                                              |)))
                                                          | _ =>
                                                            M.impossible "wrong number of arguments"
                                                          end))
                                                  ]
                                                |)
                                              ]
                                            |)
                                          |) in
                                        let _ :=
                                          is_constant_or_break_match (|
                                            M.read (| γ |),
                                            Value.Bool true
                                          |) in
                                        M.never_to_any (|
                                          M.call_closure (|
                                            Ty.path "never",
                                            M.get_function (| "core::panicking::panic", [], [] |),
                                            [
                                              mk_str (|
                                                "assertion failed: self.base[40 - digits..].iter().all(|&v| v == 0)"
                                              |)
                                            ]
                                          |)
                                        |)));
                                    fun γ => ltac:(M.monadic (Value.Tuple []))
                                  ]
                                |) in
                              M.alloc (| Ty.tuple [], Value.Tuple [] |)
                            |)));
                        fun γ => ltac:(M.monadic (Value.Tuple []))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.match_operator (|
                      Ty.tuple [],
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ := M.alloc (| Ty.path "bool", Value.Bool true |) in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            M.read (|
                              let~ _ : Ty.tuple [] :=
                                M.match_operator (|
                                  Ty.tuple [],
                                  M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                  [
                                    fun γ =>
                                      ltac:(M.monadic
                                        (let γ :=
                                          M.alloc (|
                                            Ty.path "bool",
                                            M.call_closure (|
                                              Ty.path "bool",
                                              UnOp.not,
                                              [
                                                LogicalOp.or (|
                                                  M.call_closure (|
                                                    Ty.path "bool",
                                                    BinOp.eq,
                                                    [
                                                      M.read (| bits |);
                                                      Value.Integer IntegerKind.Usize 0
                                                    ]
                                                  |),
                                                  ltac:(M.monadic
                                                    (M.call_closure (|
                                                      Ty.path "bool",
                                                      BinOp.eq,
                                                      [
                                                        M.call_closure (|
                                                          Ty.path "u32",
                                                          BinOp.Wrap.shr,
                                                          [
                                                            M.read (|
                                                              M.SubPointer.get_array_field (|
                                                                M.SubPointer.get_struct_record_field (|
                                                                  M.deref (| M.read (| self |) |),
                                                                  "core::num::bignum::Big32x40",
                                                                  "base"
                                                                |),
                                                                M.call_closure (|
                                                                  Ty.path "usize",
                                                                  BinOp.Wrap.sub,
                                                                  [
                                                                    M.call_closure (|
                                                                      Ty.path "usize",
                                                                      BinOp.Wrap.sub,
                                                                      [
                                                                        Value.Integer
                                                                          IntegerKind.Usize
                                                                          40;
                                                                        M.read (| digits |)
                                                                      ]
                                                                    |);
                                                                    Value.Integer
                                                                      IntegerKind.Usize
                                                                      1
                                                                  ]
                                                                |)
                                                              |)
                                                            |);
                                                            M.call_closure (|
                                                              Ty.path "usize",
                                                              BinOp.Wrap.sub,
                                                              [
                                                                M.read (| digitbits |);
                                                                M.read (| bits |)
                                                              ]
                                                            |)
                                                          ]
                                                        |);
                                                        Value.Integer IntegerKind.U32 0
                                                      ]
                                                    |)))
                                                |)
                                              ]
                                            |)
                                          |) in
                                        let _ :=
                                          is_constant_or_break_match (|
                                            M.read (| γ |),
                                            Value.Bool true
                                          |) in
                                        M.never_to_any (|
                                          M.call_closure (|
                                            Ty.path "never",
                                            M.get_function (| "core::panicking::panic", [], [] |),
                                            [
                                              mk_str (|
                                                "assertion failed: bits == 0 || (self.base[40 - digits - 1] >> (digitbits - bits)) == 0"
                                              |)
                                            ]
                                          |)
                                        |)));
                                    fun γ => ltac:(M.monadic (Value.Tuple []))
                                  ]
                                |) in
                              M.alloc (| Ty.tuple [], Value.Tuple [] |)
                            |)));
                        fun γ => ltac:(M.monadic (Value.Tuple []))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.read (|
                      M.use
                        (M.alloc (|
                          Ty.tuple [],
                          M.match_operator (|
                            Ty.tuple [],
                            M.alloc (|
                              Ty.apply
                                (Ty.path "core::iter::adapters::rev::Rev")
                                []
                                [
                                  Ty.apply
                                    (Ty.path "core::ops::range::Range")
                                    []
                                    [ Ty.path "usize" ]
                                ],
                              M.call_closure (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::rev::Rev")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::ops::range::Range")
                                      []
                                      [ Ty.path "usize" ]
                                  ],
                                M.get_trait_method (|
                                  "core::iter::traits::collect::IntoIterator",
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::rev::Rev")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::ops::range::Range")
                                        []
                                        [ Ty.path "usize" ]
                                    ],
                                  [],
                                  [],
                                  "into_iter",
                                  [],
                                  []
                                |),
                                [
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::rev::Rev")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::ops::range::Range")
                                          []
                                          [ Ty.path "usize" ]
                                      ],
                                    M.get_trait_method (|
                                      "core::iter::traits::iterator::Iterator",
                                      Ty.apply
                                        (Ty.path "core::ops::range::Range")
                                        []
                                        [ Ty.path "usize" ],
                                      [],
                                      [],
                                      "rev",
                                      [],
                                      []
                                    |),
                                    [
                                      Value.mkStructRecord
                                        "core::ops::range::Range"
                                        []
                                        [ Ty.path "usize" ]
                                        [
                                          ("start", Value.Integer IntegerKind.Usize 0);
                                          ("end_",
                                            M.read (|
                                              M.SubPointer.get_struct_record_field (|
                                                M.deref (| M.read (| self |) |),
                                                "core::num::bignum::Big32x40",
                                                "size"
                                              |)
                                            |))
                                        ]
                                    ]
                                  |)
                                ]
                              |)
                            |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let~ iter :
                                      Ty.apply
                                        (Ty.path "core::iter::adapters::rev::Rev")
                                        []
                                        [
                                          Ty.apply
                                            (Ty.path "core::ops::range::Range")
                                            []
                                            [ Ty.path "usize" ]
                                        ] :=
                                    M.read (| γ |) in
                                  M.read (|
                                    M.loop (|
                                      Ty.tuple [],
                                      ltac:(M.monadic
                                        (let~ _ : Ty.tuple [] :=
                                          M.match_operator (|
                                            Ty.tuple [],
                                            M.alloc (|
                                              Ty.apply
                                                (Ty.path "core::option::Option")
                                                []
                                                [ Ty.path "usize" ],
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [ Ty.path "usize" ],
                                                M.get_trait_method (|
                                                  "core::iter::traits::iterator::Iterator",
                                                  Ty.apply
                                                    (Ty.path "core::iter::adapters::rev::Rev")
                                                    []
                                                    [
                                                      Ty.apply
                                                        (Ty.path "core::ops::range::Range")
                                                        []
                                                        [ Ty.path "usize" ]
                                                    ],
                                                  [],
                                                  [],
                                                  "next",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.MutRef,
                                                    M.deref (|
                                                      M.borrow (| Pointer.Kind.MutRef, iter |)
                                                    |)
                                                  |)
                                                ]
                                              |)
                                            |),
                                            [
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let _ :=
                                                    M.is_struct_tuple (|
                                                      γ,
                                                      "core::option::Option::None"
                                                    |) in
                                                  M.never_to_any (| M.read (| M.break (||) |) |)));
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let γ0_0 :=
                                                    M.SubPointer.get_struct_tuple_field (|
                                                      γ,
                                                      "core::option::Option::Some",
                                                      0
                                                    |) in
                                                  let i := M.copy (| Ty.path "usize", γ0_0 |) in
                                                  M.read (|
                                                    let~ _ : Ty.tuple [] :=
                                                      M.write (|
                                                        M.SubPointer.get_array_field (|
                                                          M.SubPointer.get_struct_record_field (|
                                                            M.deref (| M.read (| self |) |),
                                                            "core::num::bignum::Big32x40",
                                                            "base"
                                                          |),
                                                          M.call_closure (|
                                                            Ty.path "usize",
                                                            BinOp.Wrap.add,
                                                            [ M.read (| i |); M.read (| digits |) ]
                                                          |)
                                                        |),
                                                        M.read (|
                                                          M.SubPointer.get_array_field (|
                                                            M.SubPointer.get_struct_record_field (|
                                                              M.deref (| M.read (| self |) |),
                                                              "core::num::bignum::Big32x40",
                                                              "base"
                                                            |),
                                                            M.read (| i |)
                                                          |)
                                                        |)
                                                      |) in
                                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                  |)))
                                            ]
                                          |) in
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                    |)
                                  |)))
                            ]
                          |)
                        |))
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.read (|
                      M.use
                        (M.alloc (|
                          Ty.tuple [],
                          M.match_operator (|
                            Ty.tuple [],
                            M.alloc (|
                              Ty.apply (Ty.path "core::ops::range::Range") [] [ Ty.path "usize" ],
                              M.call_closure (|
                                Ty.apply (Ty.path "core::ops::range::Range") [] [ Ty.path "usize" ],
                                M.get_trait_method (|
                                  "core::iter::traits::collect::IntoIterator",
                                  Ty.apply
                                    (Ty.path "core::ops::range::Range")
                                    []
                                    [ Ty.path "usize" ],
                                  [],
                                  [],
                                  "into_iter",
                                  [],
                                  []
                                |),
                                [
                                  Value.mkStructRecord
                                    "core::ops::range::Range"
                                    []
                                    [ Ty.path "usize" ]
                                    [
                                      ("start", Value.Integer IntegerKind.Usize 0);
                                      ("end_", M.read (| digits |))
                                    ]
                                ]
                              |)
                            |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let~ iter :
                                      Ty.apply
                                        (Ty.path "core::ops::range::Range")
                                        []
                                        [ Ty.path "usize" ] :=
                                    M.read (| γ |) in
                                  M.read (|
                                    M.loop (|
                                      Ty.tuple [],
                                      ltac:(M.monadic
                                        (let~ _ : Ty.tuple [] :=
                                          M.match_operator (|
                                            Ty.tuple [],
                                            M.alloc (|
                                              Ty.apply
                                                (Ty.path "core::option::Option")
                                                []
                                                [ Ty.path "usize" ],
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [ Ty.path "usize" ],
                                                M.get_trait_method (|
                                                  "core::iter::traits::iterator::Iterator",
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::Range")
                                                    []
                                                    [ Ty.path "usize" ],
                                                  [],
                                                  [],
                                                  "next",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.MutRef,
                                                    M.deref (|
                                                      M.borrow (| Pointer.Kind.MutRef, iter |)
                                                    |)
                                                  |)
                                                ]
                                              |)
                                            |),
                                            [
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let _ :=
                                                    M.is_struct_tuple (|
                                                      γ,
                                                      "core::option::Option::None"
                                                    |) in
                                                  M.never_to_any (| M.read (| M.break (||) |) |)));
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let γ0_0 :=
                                                    M.SubPointer.get_struct_tuple_field (|
                                                      γ,
                                                      "core::option::Option::Some",
                                                      0
                                                    |) in
                                                  let i := M.copy (| Ty.path "usize", γ0_0 |) in
                                                  M.read (|
                                                    let~ _ : Ty.tuple [] :=
                                                      M.write (|
                                                        M.SubPointer.get_array_field (|
                                                          M.SubPointer.get_struct_record_field (|
                                                            M.deref (| M.read (| self |) |),
                                                            "core::num::bignum::Big32x40",
                                                            "base"
                                                          |),
                                                          M.read (| i |)
                                                        |),
                                                        Value.Integer IntegerKind.U32 0
                                                      |) in
                                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                  |)))
                                            ]
                                          |) in
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                    |)
                                  |)))
                            ]
                          |)
                        |))
                    |) in
                  let~ sz : Ty.path "usize" :=
                    M.call_closure (|
                      Ty.path "usize",
                      BinOp.Wrap.add,
                      [
                        M.read (|
                          M.SubPointer.get_struct_record_field (|
                            M.deref (| M.read (| self |) |),
                            "core::num::bignum::Big32x40",
                            "size"
                          |)
                        |);
                        M.read (| digits |)
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.match_operator (|
                      Ty.tuple [],
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ :=
                              M.alloc (|
                                Ty.path "bool",
                                M.call_closure (|
                                  Ty.path "bool",
                                  BinOp.gt,
                                  [ M.read (| bits |); Value.Integer IntegerKind.Usize 0 ]
                                |)
                              |) in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            M.read (|
                              let~ last : Ty.path "usize" := M.read (| sz |) in
                              let~ overflow : Ty.path "u32" :=
                                M.call_closure (|
                                  Ty.path "u32",
                                  BinOp.Wrap.shr,
                                  [
                                    M.read (|
                                      M.SubPointer.get_array_field (|
                                        M.SubPointer.get_struct_record_field (|
                                          M.deref (| M.read (| self |) |),
                                          "core::num::bignum::Big32x40",
                                          "base"
                                        |),
                                        M.call_closure (|
                                          Ty.path "usize",
                                          BinOp.Wrap.sub,
                                          [ M.read (| last |); Value.Integer IntegerKind.Usize 1 ]
                                        |)
                                      |)
                                    |);
                                    M.call_closure (|
                                      Ty.path "usize",
                                      BinOp.Wrap.sub,
                                      [ M.read (| digitbits |); M.read (| bits |) ]
                                    |)
                                  ]
                                |) in
                              let~ _ : Ty.tuple [] :=
                                M.match_operator (|
                                  Ty.tuple [],
                                  M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                  [
                                    fun γ =>
                                      ltac:(M.monadic
                                        (let γ :=
                                          M.alloc (|
                                            Ty.path "bool",
                                            M.call_closure (|
                                              Ty.path "bool",
                                              BinOp.gt,
                                              [
                                                M.read (| overflow |);
                                                Value.Integer IntegerKind.U32 0
                                              ]
                                            |)
                                          |) in
                                        let _ :=
                                          is_constant_or_break_match (|
                                            M.read (| γ |),
                                            Value.Bool true
                                          |) in
                                        M.read (|
                                          let~ _ : Ty.tuple [] :=
                                            M.write (|
                                              M.SubPointer.get_array_field (|
                                                M.SubPointer.get_struct_record_field (|
                                                  M.deref (| M.read (| self |) |),
                                                  "core::num::bignum::Big32x40",
                                                  "base"
                                                |),
                                                M.read (| last |)
                                              |),
                                              M.read (| overflow |)
                                            |) in
                                          let~ _ : Ty.tuple [] :=
                                            let β := sz in
                                            M.write (|
                                              β,
                                              M.call_closure (|
                                                Ty.path "usize",
                                                BinOp.Wrap.add,
                                                [ M.read (| β |); Value.Integer IntegerKind.Usize 1
                                                ]
                                              |)
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                        |)));
                                    fun γ => ltac:(M.monadic (Value.Tuple []))
                                  ]
                                |) in
                              let~ _ : Ty.tuple [] :=
                                M.read (|
                                  M.use
                                    (M.alloc (|
                                      Ty.tuple [],
                                      M.match_operator (|
                                        Ty.tuple [],
                                        M.alloc (|
                                          Ty.apply
                                            (Ty.path "core::iter::adapters::rev::Rev")
                                            []
                                            [
                                              Ty.apply
                                                (Ty.path "core::ops::range::Range")
                                                []
                                                [ Ty.path "usize" ]
                                            ],
                                          M.call_closure (|
                                            Ty.apply
                                              (Ty.path "core::iter::adapters::rev::Rev")
                                              []
                                              [
                                                Ty.apply
                                                  (Ty.path "core::ops::range::Range")
                                                  []
                                                  [ Ty.path "usize" ]
                                              ],
                                            M.get_trait_method (|
                                              "core::iter::traits::collect::IntoIterator",
                                              Ty.apply
                                                (Ty.path "core::iter::adapters::rev::Rev")
                                                []
                                                [
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::Range")
                                                    []
                                                    [ Ty.path "usize" ]
                                                ],
                                              [],
                                              [],
                                              "into_iter",
                                              [],
                                              []
                                            |),
                                            [
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "core::iter::adapters::rev::Rev")
                                                  []
                                                  [
                                                    Ty.apply
                                                      (Ty.path "core::ops::range::Range")
                                                      []
                                                      [ Ty.path "usize" ]
                                                  ],
                                                M.get_trait_method (|
                                                  "core::iter::traits::iterator::Iterator",
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::Range")
                                                    []
                                                    [ Ty.path "usize" ],
                                                  [],
                                                  [],
                                                  "rev",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  Value.mkStructRecord
                                                    "core::ops::range::Range"
                                                    []
                                                    [ Ty.path "usize" ]
                                                    [
                                                      ("start",
                                                        M.call_closure (|
                                                          Ty.path "usize",
                                                          BinOp.Wrap.add,
                                                          [
                                                            M.read (| digits |);
                                                            Value.Integer IntegerKind.Usize 1
                                                          ]
                                                        |));
                                                      ("end_", M.read (| last |))
                                                    ]
                                                ]
                                              |)
                                            ]
                                          |)
                                        |),
                                        [
                                          fun γ =>
                                            ltac:(M.monadic
                                              (let~ iter :
                                                  Ty.apply
                                                    (Ty.path "core::iter::adapters::rev::Rev")
                                                    []
                                                    [
                                                      Ty.apply
                                                        (Ty.path "core::ops::range::Range")
                                                        []
                                                        [ Ty.path "usize" ]
                                                    ] :=
                                                M.read (| γ |) in
                                              M.read (|
                                                M.loop (|
                                                  Ty.tuple [],
                                                  ltac:(M.monadic
                                                    (let~ _ : Ty.tuple [] :=
                                                      M.match_operator (|
                                                        Ty.tuple [],
                                                        M.alloc (|
                                                          Ty.apply
                                                            (Ty.path "core::option::Option")
                                                            []
                                                            [ Ty.path "usize" ],
                                                          M.call_closure (|
                                                            Ty.apply
                                                              (Ty.path "core::option::Option")
                                                              []
                                                              [ Ty.path "usize" ],
                                                            M.get_trait_method (|
                                                              "core::iter::traits::iterator::Iterator",
                                                              Ty.apply
                                                                (Ty.path
                                                                  "core::iter::adapters::rev::Rev")
                                                                []
                                                                [
                                                                  Ty.apply
                                                                    (Ty.path
                                                                      "core::ops::range::Range")
                                                                    []
                                                                    [ Ty.path "usize" ]
                                                                ],
                                                              [],
                                                              [],
                                                              "next",
                                                              [],
                                                              []
                                                            |),
                                                            [
                                                              M.borrow (|
                                                                Pointer.Kind.MutRef,
                                                                M.deref (|
                                                                  M.borrow (|
                                                                    Pointer.Kind.MutRef,
                                                                    iter
                                                                  |)
                                                                |)
                                                              |)
                                                            ]
                                                          |)
                                                        |),
                                                        [
                                                          fun γ =>
                                                            ltac:(M.monadic
                                                              (let _ :=
                                                                M.is_struct_tuple (|
                                                                  γ,
                                                                  "core::option::Option::None"
                                                                |) in
                                                              M.never_to_any (|
                                                                M.read (| M.break (||) |)
                                                              |)));
                                                          fun γ =>
                                                            ltac:(M.monadic
                                                              (let γ0_0 :=
                                                                M.SubPointer.get_struct_tuple_field (|
                                                                  γ,
                                                                  "core::option::Option::Some",
                                                                  0
                                                                |) in
                                                              let i :=
                                                                M.copy (|
                                                                  Ty.path "usize",
                                                                  γ0_0
                                                                |) in
                                                              M.read (|
                                                                let~ _ : Ty.tuple [] :=
                                                                  M.write (|
                                                                    M.SubPointer.get_array_field (|
                                                                      M.SubPointer.get_struct_record_field (|
                                                                        M.deref (|
                                                                          M.read (| self |)
                                                                        |),
                                                                        "core::num::bignum::Big32x40",
                                                                        "base"
                                                                      |),
                                                                      M.read (| i |)
                                                                    |),
                                                                    M.call_closure (|
                                                                      Ty.path "u32",
                                                                      BinOp.Wrap.bit_or,
                                                                      [
                                                                        M.call_closure (|
                                                                          Ty.path "u32",
                                                                          BinOp.Wrap.shl,
                                                                          [
                                                                            M.read (|
                                                                              M.SubPointer.get_array_field (|
                                                                                M.SubPointer.get_struct_record_field (|
                                                                                  M.deref (|
                                                                                    M.read (|
                                                                                      self
                                                                                    |)
                                                                                  |),
                                                                                  "core::num::bignum::Big32x40",
                                                                                  "base"
                                                                                |),
                                                                                M.read (| i |)
                                                                              |)
                                                                            |);
                                                                            M.read (| bits |)
                                                                          ]
                                                                        |);
                                                                        M.call_closure (|
                                                                          Ty.path "u32",
                                                                          BinOp.Wrap.shr,
                                                                          [
                                                                            M.read (|
                                                                              M.SubPointer.get_array_field (|
                                                                                M.SubPointer.get_struct_record_field (|
                                                                                  M.deref (|
                                                                                    M.read (|
                                                                                      self
                                                                                    |)
                                                                                  |),
                                                                                  "core::num::bignum::Big32x40",
                                                                                  "base"
                                                                                |),
                                                                                M.call_closure (|
                                                                                  Ty.path "usize",
                                                                                  BinOp.Wrap.sub,
                                                                                  [
                                                                                    M.read (| i |);
                                                                                    Value.Integer
                                                                                      IntegerKind.Usize
                                                                                      1
                                                                                  ]
                                                                                |)
                                                                              |)
                                                                            |);
                                                                            M.call_closure (|
                                                                              Ty.path "usize",
                                                                              BinOp.Wrap.sub,
                                                                              [
                                                                                M.read (|
                                                                                  digitbits
                                                                                |);
                                                                                M.read (| bits |)
                                                                              ]
                                                                            |)
                                                                          ]
                                                                        |)
                                                                      ]
                                                                    |)
                                                                  |) in
                                                                M.alloc (|
                                                                  Ty.tuple [],
                                                                  Value.Tuple []
                                                                |)
                                                              |)))
                                                        ]
                                                      |) in
                                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                                |)
                                              |)))
                                        ]
                                      |)
                                    |))
                                |) in
                              let~ _ : Ty.tuple [] :=
                                let β :=
                                  M.SubPointer.get_array_field (|
                                    M.SubPointer.get_struct_record_field (|
                                      M.deref (| M.read (| self |) |),
                                      "core::num::bignum::Big32x40",
                                      "base"
                                    |),
                                    M.read (| digits |)
                                  |) in
                                M.write (|
                                  β,
                                  M.call_closure (|
                                    Ty.path "u32",
                                    BinOp.Wrap.shl,
                                    [ M.read (| β |); M.read (| bits |) ]
                                  |)
                                |) in
                              M.alloc (| Ty.tuple [], Value.Tuple [] |)
                            |)));
                        fun γ => ltac:(M.monadic (Value.Tuple []))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.write (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::Big32x40",
                        "size"
                      |),
                      M.read (| sz |)
                    |) in
                  M.alloc (|
                    Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                    M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                  |)
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_mul_pow2 :
        M.IsAssociatedFunction.C Self "mul_pow2" mul_pow2.
      Admitted.
      Global Typeclasses Opaque mul_pow2.
      
      (*
                  pub fn mul_pow5(&mut self, mut e: usize) -> &mut $name {
                      use crate::num::bignum::SMALL_POW5;
      
                      // There are exactly n trailing zeros on 2^n, and the only relevant digit sizes
                      // are consecutive powers of two, so this is well suited index for the table.
                      let table_index = size_of::<$ty>().trailing_zeros() as usize;
                      let (small_power, small_e) = SMALL_POW5[table_index];
                      let small_power = small_power as $ty;
      
                      // Multiply with the largest single-digit power as long as possible ...
                      while e >= small_e {
                          self.mul_small(small_power);
                          e -= small_e;
                      }
      
                      // ... then finish off the remainder.
                      let mut rest_power = 1;
                      for _ in 0..e {
                          rest_power *= 5;
                      }
                      self.mul_small(rest_power);
      
                      self
                  }
      *)
      Definition mul_pow5 (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; e ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let e := M.alloc (| Ty.path "usize", e |) in
            M.borrow (|
              Pointer.Kind.MutRef,
              M.deref (|
                M.read (|
                  let~ table_index : Ty.path "usize" :=
                    M.cast
                      (Ty.path "usize")
                      (M.call_closure (|
                        Ty.path "u32",
                        M.get_associated_function (| Ty.path "usize", "trailing_zeros", [], [] |),
                        [
                          M.call_closure (|
                            Ty.path "usize",
                            M.get_function (| "core::mem::size_of", [], [ Ty.path "u32" ] |),
                            []
                          |)
                        ]
                      |)) in
                  M.alloc (|
                    Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                    M.match_operator (|
                      Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                      M.SubPointer.get_array_field (|
                        get_constant (|
                          "core::num::bignum::SMALL_POW5",
                          Ty.apply
                            (Ty.path "array")
                            [ Value.Integer IntegerKind.Usize 3 ]
                            [ Ty.tuple [ Ty.path "u64"; Ty.path "usize" ] ]
                        |),
                        M.read (| table_index |)
                      |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                            let γ0_1 := M.SubPointer.get_tuple_field (| γ, 1 |) in
                            let small_power := M.copy (| Ty.path "u64", γ0_0 |) in
                            let small_e := M.copy (| Ty.path "usize", γ0_1 |) in
                            M.read (|
                              let~ small_power : Ty.path "u32" :=
                                M.cast (Ty.path "u32") (M.read (| small_power |)) in
                              let~ _ : Ty.tuple [] :=
                                M.read (|
                                  M.loop (|
                                    Ty.tuple [],
                                    ltac:(M.monadic
                                      (M.alloc (|
                                        Ty.tuple [],
                                        M.match_operator (|
                                          Ty.tuple [],
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                          [
                                            fun γ =>
                                              ltac:(M.monadic
                                                (let γ :=
                                                  M.alloc (|
                                                    Ty.path "bool",
                                                    M.call_closure (|
                                                      Ty.path "bool",
                                                      BinOp.ge,
                                                      [ M.read (| e |); M.read (| small_e |) ]
                                                    |)
                                                  |) in
                                                let _ :=
                                                  is_constant_or_break_match (|
                                                    M.read (| γ |),
                                                    Value.Bool true
                                                  |) in
                                                M.read (|
                                                  let~ _ :
                                                      Ty.apply
                                                        (Ty.path "&mut")
                                                        []
                                                        [ Ty.path "core::num::bignum::Big32x40" ] :=
                                                    M.call_closure (|
                                                      Ty.apply
                                                        (Ty.path "&mut")
                                                        []
                                                        [ Ty.path "core::num::bignum::Big32x40" ],
                                                      M.get_associated_function (|
                                                        Ty.path "core::num::bignum::Big32x40",
                                                        "mul_small",
                                                        [],
                                                        []
                                                      |),
                                                      [
                                                        M.borrow (|
                                                          Pointer.Kind.MutRef,
                                                          M.deref (| M.read (| self |) |)
                                                        |);
                                                        M.read (| small_power |)
                                                      ]
                                                    |) in
                                                  let~ _ : Ty.tuple [] :=
                                                    let β := e in
                                                    M.write (|
                                                      β,
                                                      M.call_closure (|
                                                        Ty.path "usize",
                                                        BinOp.Wrap.sub,
                                                        [ M.read (| β |); M.read (| small_e |) ]
                                                      |)
                                                    |) in
                                                  M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                |)));
                                            fun γ =>
                                              ltac:(M.monadic
                                                (M.never_to_any (|
                                                  M.read (|
                                                    let~ _ : Ty.tuple [] :=
                                                      M.never_to_any (|
                                                        M.read (| M.break (||) |)
                                                      |) in
                                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                  |)
                                                |)))
                                          ]
                                        |)
                                      |)))
                                  |)
                                |) in
                              let~ rest_power : Ty.path "u32" := Value.Integer IntegerKind.U32 1 in
                              let~ _ : Ty.tuple [] :=
                                M.read (|
                                  M.use
                                    (M.alloc (|
                                      Ty.tuple [],
                                      M.match_operator (|
                                        Ty.tuple [],
                                        M.alloc (|
                                          Ty.apply
                                            (Ty.path "core::ops::range::Range")
                                            []
                                            [ Ty.path "usize" ],
                                          M.call_closure (|
                                            Ty.apply
                                              (Ty.path "core::ops::range::Range")
                                              []
                                              [ Ty.path "usize" ],
                                            M.get_trait_method (|
                                              "core::iter::traits::collect::IntoIterator",
                                              Ty.apply
                                                (Ty.path "core::ops::range::Range")
                                                []
                                                [ Ty.path "usize" ],
                                              [],
                                              [],
                                              "into_iter",
                                              [],
                                              []
                                            |),
                                            [
                                              Value.mkStructRecord
                                                "core::ops::range::Range"
                                                []
                                                [ Ty.path "usize" ]
                                                [
                                                  ("start", Value.Integer IntegerKind.Usize 0);
                                                  ("end_", M.read (| e |))
                                                ]
                                            ]
                                          |)
                                        |),
                                        [
                                          fun γ =>
                                            ltac:(M.monadic
                                              (let~ iter :
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::Range")
                                                    []
                                                    [ Ty.path "usize" ] :=
                                                M.read (| γ |) in
                                              M.read (|
                                                M.loop (|
                                                  Ty.tuple [],
                                                  ltac:(M.monadic
                                                    (let~ _ : Ty.tuple [] :=
                                                      M.match_operator (|
                                                        Ty.tuple [],
                                                        M.alloc (|
                                                          Ty.apply
                                                            (Ty.path "core::option::Option")
                                                            []
                                                            [ Ty.path "usize" ],
                                                          M.call_closure (|
                                                            Ty.apply
                                                              (Ty.path "core::option::Option")
                                                              []
                                                              [ Ty.path "usize" ],
                                                            M.get_trait_method (|
                                                              "core::iter::traits::iterator::Iterator",
                                                              Ty.apply
                                                                (Ty.path "core::ops::range::Range")
                                                                []
                                                                [ Ty.path "usize" ],
                                                              [],
                                                              [],
                                                              "next",
                                                              [],
                                                              []
                                                            |),
                                                            [
                                                              M.borrow (|
                                                                Pointer.Kind.MutRef,
                                                                M.deref (|
                                                                  M.borrow (|
                                                                    Pointer.Kind.MutRef,
                                                                    iter
                                                                  |)
                                                                |)
                                                              |)
                                                            ]
                                                          |)
                                                        |),
                                                        [
                                                          fun γ =>
                                                            ltac:(M.monadic
                                                              (let _ :=
                                                                M.is_struct_tuple (|
                                                                  γ,
                                                                  "core::option::Option::None"
                                                                |) in
                                                              M.never_to_any (|
                                                                M.read (| M.break (||) |)
                                                              |)));
                                                          fun γ =>
                                                            ltac:(M.monadic
                                                              (let γ0_0 :=
                                                                M.SubPointer.get_struct_tuple_field (|
                                                                  γ,
                                                                  "core::option::Option::Some",
                                                                  0
                                                                |) in
                                                              M.read (|
                                                                let~ _ : Ty.tuple [] :=
                                                                  let β := rest_power in
                                                                  M.write (|
                                                                    β,
                                                                    M.call_closure (|
                                                                      Ty.path "u32",
                                                                      BinOp.Wrap.mul,
                                                                      [
                                                                        M.read (| β |);
                                                                        Value.Integer
                                                                          IntegerKind.U32
                                                                          5
                                                                      ]
                                                                    |)
                                                                  |) in
                                                                M.alloc (|
                                                                  Ty.tuple [],
                                                                  Value.Tuple []
                                                                |)
                                                              |)))
                                                        ]
                                                      |) in
                                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                                |)
                                              |)))
                                        ]
                                      |)
                                    |))
                                |) in
                              let~ _ :
                                  Ty.apply
                                    (Ty.path "&mut")
                                    []
                                    [ Ty.path "core::num::bignum::Big32x40" ] :=
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "&mut")
                                    []
                                    [ Ty.path "core::num::bignum::Big32x40" ],
                                  M.get_associated_function (|
                                    Ty.path "core::num::bignum::Big32x40",
                                    "mul_small",
                                    [],
                                    []
                                  |),
                                  [
                                    M.borrow (|
                                      Pointer.Kind.MutRef,
                                      M.deref (| M.read (| self |) |)
                                    |);
                                    M.read (| rest_power |)
                                  ]
                                |) in
                              M.alloc (|
                                Ty.apply
                                  (Ty.path "&mut")
                                  []
                                  [ Ty.path "core::num::bignum::Big32x40" ],
                                M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                              |)
                            |)))
                      ]
                    |)
                  |)
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_mul_pow5 :
        M.IsAssociatedFunction.C Self "mul_pow5" mul_pow5.
      Admitted.
      Global Typeclasses Opaque mul_pow5.
      
      (*
                  pub fn mul_digits<'a>(&'a mut self, other: &[$ty]) -> &'a mut $name {
                      // the internal routine. works best when aa.len() <= bb.len().
                      fn mul_inner(ret: &mut [$ty; $n], aa: &[$ty], bb: &[$ty]) -> usize {
                          use crate::num::bignum::FullOps;
      
                          let mut retsz = 0;
                          for (i, &a) in aa.iter().enumerate() {
                              if a == 0 {
                                  continue;
                              }
                              let mut sz = bb.len();
                              let mut carry = 0;
                              for (j, &b) in bb.iter().enumerate() {
                                  let (c, v) = a.full_mul_add(b, ret[i + j], carry);
                                  ret[i + j] = v;
                                  carry = c;
                              }
                              if carry > 0 {
                                  ret[i + sz] = carry;
                                  sz += 1;
                              }
                              if retsz < i + sz {
                                  retsz = i + sz;
                              }
                          }
                          retsz
                      }
      
                      let mut ret = [0; $n];
                      let retsz = if self.size < other.len() {
                          mul_inner(&mut ret, &self.digits(), other)
                      } else {
                          mul_inner(&mut ret, other, &self.digits())
                      };
                      self.base = ret;
                      self.size = retsz;
                      self
                  }
      *)
      Definition mul_digits (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                other
              |) in
            M.borrow (|
              Pointer.Kind.MutRef,
              M.deref (|
                M.read (|
                  let~ ret :
                      Ty.apply
                        (Ty.path "array")
                        [ Value.Integer IntegerKind.Usize 40 ]
                        [ Ty.path "u32" ] :=
                    lib.repeat (|
                      Value.Integer IntegerKind.U32 0,
                      Value.Integer IntegerKind.Usize 40
                    |) in
                  let~ retsz : Ty.path "usize" :=
                    M.match_operator (|
                      Ty.path "usize",
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ :=
                              M.alloc (|
                                Ty.path "bool",
                                M.call_closure (|
                                  Ty.path "bool",
                                  BinOp.lt,
                                  [
                                    M.read (|
                                      M.SubPointer.get_struct_record_field (|
                                        M.deref (| M.read (| self |) |),
                                        "core::num::bignum::Big32x40",
                                        "size"
                                      |)
                                    |);
                                    M.call_closure (|
                                      Ty.path "usize",
                                      M.get_associated_function (|
                                        Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ],
                                        "len",
                                        [],
                                        []
                                      |),
                                      [
                                        M.borrow (|
                                          Pointer.Kind.Ref,
                                          M.deref (| M.read (| other |) |)
                                        |)
                                      ]
                                    |)
                                  ]
                                |)
                              |) in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            M.call_closure (|
                              Ty.path "usize",
                              M.get_associated_function (| Self, "mul_inner.mul_digits", [], [] |),
                              [
                                M.borrow (|
                                  Pointer.Kind.MutRef,
                                  M.deref (| M.borrow (| Pointer.Kind.MutRef, ret |) |)
                                |);
                                M.borrow (|
                                  Pointer.Kind.Ref,
                                  M.deref (|
                                    M.read (|
                                      M.deref (|
                                        M.borrow (|
                                          Pointer.Kind.Ref,
                                          M.alloc (|
                                            Ty.apply
                                              (Ty.path "&")
                                              []
                                              [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "&")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                              M.get_associated_function (|
                                                Ty.path "core::num::bignum::Big32x40",
                                                "digits",
                                                [],
                                                []
                                              |),
                                              [
                                                M.borrow (|
                                                  Pointer.Kind.Ref,
                                                  M.deref (| M.read (| self |) |)
                                                |)
                                              ]
                                            |)
                                          |)
                                        |)
                                      |)
                                    |)
                                  |)
                                |);
                                M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| other |) |) |)
                              ]
                            |)));
                        fun γ =>
                          ltac:(M.monadic
                            (M.call_closure (|
                              Ty.path "usize",
                              M.get_associated_function (| Self, "mul_inner.mul_digits", [], [] |),
                              [
                                M.borrow (|
                                  Pointer.Kind.MutRef,
                                  M.deref (| M.borrow (| Pointer.Kind.MutRef, ret |) |)
                                |);
                                M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| other |) |) |);
                                M.borrow (|
                                  Pointer.Kind.Ref,
                                  M.deref (|
                                    M.read (|
                                      M.deref (|
                                        M.borrow (|
                                          Pointer.Kind.Ref,
                                          M.alloc (|
                                            Ty.apply
                                              (Ty.path "&")
                                              []
                                              [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "&")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                              M.get_associated_function (|
                                                Ty.path "core::num::bignum::Big32x40",
                                                "digits",
                                                [],
                                                []
                                              |),
                                              [
                                                M.borrow (|
                                                  Pointer.Kind.Ref,
                                                  M.deref (| M.read (| self |) |)
                                                |)
                                              ]
                                            |)
                                          |)
                                        |)
                                      |)
                                    |)
                                  |)
                                |)
                              ]
                            |)))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.write (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::Big32x40",
                        "base"
                      |),
                      M.read (| ret |)
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.write (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::Big32x40",
                        "size"
                      |),
                      M.read (| retsz |)
                    |) in
                  M.alloc (|
                    Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                    M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                  |)
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_mul_digits :
        M.IsAssociatedFunction.C Self "mul_digits" mul_digits.
      Admitted.
      Global Typeclasses Opaque mul_digits.
      
      (*
                  pub fn div_rem_small(&mut self, other: $ty) -> (&mut $name, $ty) {
                      use crate::num::bignum::FullOps;
      
                      assert!(other > 0);
      
                      let sz = self.size;
                      let mut borrow = 0;
                      for a in self.base[..sz].iter_mut().rev() {
                          let (q, r) = ( *a).full_div_rem(other, borrow);
                          *a = q;
                          borrow = r;
                      }
                      (self, borrow)
                  }
      *)
      Definition div_rem_small (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other := M.alloc (| Ty.path "u32", other |) in
            M.read (|
              let~ _ : Ty.tuple [] :=
                M.match_operator (|
                  Ty.tuple [],
                  M.alloc (| Ty.tuple [], Value.Tuple [] |),
                  [
                    fun γ =>
                      ltac:(M.monadic
                        (let γ :=
                          M.alloc (|
                            Ty.path "bool",
                            M.call_closure (|
                              Ty.path "bool",
                              UnOp.not,
                              [
                                M.call_closure (|
                                  Ty.path "bool",
                                  BinOp.gt,
                                  [ M.read (| other |); Value.Integer IntegerKind.U32 0 ]
                                |)
                              ]
                            |)
                          |) in
                        let _ := is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                        M.never_to_any (|
                          M.call_closure (|
                            Ty.path "never",
                            M.get_function (| "core::panicking::panic", [], [] |),
                            [ mk_str (| "assertion failed: other > 0" |) ]
                          |)
                        |)));
                    fun γ => ltac:(M.monadic (Value.Tuple []))
                  ]
                |) in
              let~ sz : Ty.path "usize" :=
                M.read (|
                  M.SubPointer.get_struct_record_field (|
                    M.deref (| M.read (| self |) |),
                    "core::num::bignum::Big32x40",
                    "size"
                  |)
                |) in
              let~ borrow : Ty.path "u32" := Value.Integer IntegerKind.U32 0 in
              let~ _ : Ty.tuple [] :=
                M.read (|
                  M.use
                    (M.alloc (|
                      Ty.tuple [],
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (|
                          Ty.apply
                            (Ty.path "core::iter::adapters::rev::Rev")
                            []
                            [ Ty.apply (Ty.path "core::slice::iter::IterMut") [] [ Ty.path "u32" ]
                            ],
                          M.call_closure (|
                            Ty.apply
                              (Ty.path "core::iter::adapters::rev::Rev")
                              []
                              [ Ty.apply (Ty.path "core::slice::iter::IterMut") [] [ Ty.path "u32" ]
                              ],
                            M.get_trait_method (|
                              "core::iter::traits::collect::IntoIterator",
                              Ty.apply
                                (Ty.path "core::iter::adapters::rev::Rev")
                                []
                                [
                                  Ty.apply
                                    (Ty.path "core::slice::iter::IterMut")
                                    []
                                    [ Ty.path "u32" ]
                                ],
                              [],
                              [],
                              "into_iter",
                              [],
                              []
                            |),
                            [
                              M.call_closure (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::rev::Rev")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::slice::iter::IterMut")
                                      []
                                      [ Ty.path "u32" ]
                                  ],
                                M.get_trait_method (|
                                  "core::iter::traits::iterator::Iterator",
                                  Ty.apply
                                    (Ty.path "core::slice::iter::IterMut")
                                    []
                                    [ Ty.path "u32" ],
                                  [],
                                  [],
                                  "rev",
                                  [],
                                  []
                                |),
                                [
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "core::slice::iter::IterMut")
                                      []
                                      [ Ty.path "u32" ],
                                    M.get_associated_function (|
                                      Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ],
                                      "iter_mut",
                                      [],
                                      []
                                    |),
                                    [
                                      M.borrow (|
                                        Pointer.Kind.MutRef,
                                        M.deref (|
                                          M.call_closure (|
                                            Ty.apply
                                              (Ty.path "&mut")
                                              []
                                              [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                            M.get_trait_method (|
                                              "core::ops::index::IndexMut",
                                              Ty.apply
                                                (Ty.path "array")
                                                [ Value.Integer IntegerKind.Usize 40 ]
                                                [ Ty.path "u32" ],
                                              [],
                                              [
                                                Ty.apply
                                                  (Ty.path "core::ops::range::RangeTo")
                                                  []
                                                  [ Ty.path "usize" ]
                                              ],
                                              "index_mut",
                                              [],
                                              []
                                            |),
                                            [
                                              M.borrow (|
                                                Pointer.Kind.MutRef,
                                                M.SubPointer.get_struct_record_field (|
                                                  M.deref (| M.read (| self |) |),
                                                  "core::num::bignum::Big32x40",
                                                  "base"
                                                |)
                                              |);
                                              Value.mkStructRecord
                                                "core::ops::range::RangeTo"
                                                []
                                                [ Ty.path "usize" ]
                                                [ ("end_", M.read (| sz |)) ]
                                            ]
                                          |)
                                        |)
                                      |)
                                    ]
                                  |)
                                ]
                              |)
                            ]
                          |)
                        |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let~ iter :
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::rev::Rev")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::slice::iter::IterMut")
                                        []
                                        [ Ty.path "u32" ]
                                    ] :=
                                M.read (| γ |) in
                              M.read (|
                                M.loop (|
                                  Ty.tuple [],
                                  ltac:(M.monadic
                                    (let~ _ : Ty.tuple [] :=
                                      M.match_operator (|
                                        Ty.tuple [],
                                        M.alloc (|
                                          Ty.apply
                                            (Ty.path "core::option::Option")
                                            []
                                            [ Ty.apply (Ty.path "&mut") [] [ Ty.path "u32" ] ],
                                          M.call_closure (|
                                            Ty.apply
                                              (Ty.path "core::option::Option")
                                              []
                                              [ Ty.apply (Ty.path "&mut") [] [ Ty.path "u32" ] ],
                                            M.get_trait_method (|
                                              "core::iter::traits::iterator::Iterator",
                                              Ty.apply
                                                (Ty.path "core::iter::adapters::rev::Rev")
                                                []
                                                [
                                                  Ty.apply
                                                    (Ty.path "core::slice::iter::IterMut")
                                                    []
                                                    [ Ty.path "u32" ]
                                                ],
                                              [],
                                              [],
                                              "next",
                                              [],
                                              []
                                            |),
                                            [
                                              M.borrow (|
                                                Pointer.Kind.MutRef,
                                                M.deref (|
                                                  M.borrow (| Pointer.Kind.MutRef, iter |)
                                                |)
                                              |)
                                            ]
                                          |)
                                        |),
                                        [
                                          fun γ =>
                                            ltac:(M.monadic
                                              (let _ :=
                                                M.is_struct_tuple (|
                                                  γ,
                                                  "core::option::Option::None"
                                                |) in
                                              M.never_to_any (| M.read (| M.break (||) |) |)));
                                          fun γ =>
                                            ltac:(M.monadic
                                              (let γ0_0 :=
                                                M.SubPointer.get_struct_tuple_field (|
                                                  γ,
                                                  "core::option::Option::Some",
                                                  0
                                                |) in
                                              let a :=
                                                M.copy (|
                                                  Ty.apply (Ty.path "&mut") [] [ Ty.path "u32" ],
                                                  γ0_0
                                                |) in
                                              M.match_operator (|
                                                Ty.tuple [],
                                                M.alloc (|
                                                  Ty.tuple [ Ty.path "u32"; Ty.path "u32" ],
                                                  M.call_closure (|
                                                    Ty.tuple [ Ty.path "u32"; Ty.path "u32" ],
                                                    M.get_trait_method (|
                                                      "core::num::bignum::FullOps",
                                                      Ty.path "u32",
                                                      [],
                                                      [],
                                                      "full_div_rem",
                                                      [],
                                                      []
                                                    |),
                                                    [
                                                      M.read (| M.deref (| M.read (| a |) |) |);
                                                      M.read (| other |);
                                                      M.read (| borrow |)
                                                    ]
                                                  |)
                                                |),
                                                [
                                                  fun γ =>
                                                    ltac:(M.monadic
                                                      (let γ0_0 :=
                                                        M.SubPointer.get_tuple_field (| γ, 0 |) in
                                                      let γ0_1 :=
                                                        M.SubPointer.get_tuple_field (| γ, 1 |) in
                                                      let q := M.copy (| Ty.path "u32", γ0_0 |) in
                                                      let r := M.copy (| Ty.path "u32", γ0_1 |) in
                                                      M.read (|
                                                        let~ _ : Ty.tuple [] :=
                                                          M.write (|
                                                            M.deref (| M.read (| a |) |),
                                                            M.read (| q |)
                                                          |) in
                                                        let~ _ : Ty.tuple [] :=
                                                          M.write (| borrow, M.read (| r |) |) in
                                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                      |)))
                                                ]
                                              |)))
                                        ]
                                      |) in
                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                |)
                              |)))
                        ]
                      |)
                    |))
                |) in
              M.alloc (|
                Ty.tuple
                  [
                    Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::Big32x40" ];
                    Ty.path "u32"
                  ],
                Value.Tuple
                  [
                    M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |);
                    M.read (| borrow |)
                  ]
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Global Instance AssociatedFunction_div_rem_small :
        M.IsAssociatedFunction.C Self "div_rem_small" div_rem_small.
      Admitted.
      Global Typeclasses Opaque div_rem_small.
    End Impl_core_num_bignum_Big32x40.
    
    Module Impl_core_cmp_PartialEq_core_num_bignum_Big32x40_for_core_num_bignum_Big32x40.
      Definition Self : Ty.t := Ty.path "core::num::bignum::Big32x40".
      
      (*
                  fn eq(&self, other: &$name) -> bool {
                      self.base[..] == other.base[..]
                  }
      *)
      Definition eq (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                other
              |) in
            M.call_closure (|
              Ty.path "bool",
              M.get_trait_method (|
                "core::cmp::PartialEq",
                Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ],
                [],
                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                "eq",
                [],
                []
              |),
              [
                M.borrow (|
                  Pointer.Kind.Ref,
                  M.deref (|
                    M.call_closure (|
                      Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                      M.get_trait_method (|
                        "core::ops::index::Index",
                        Ty.apply
                          (Ty.path "array")
                          [ Value.Integer IntegerKind.Usize 40 ]
                          [ Ty.path "u32" ],
                        [],
                        [ Ty.path "core::ops::range::RangeFull" ],
                        "index",
                        [],
                        []
                      |),
                      [
                        M.borrow (|
                          Pointer.Kind.Ref,
                          M.SubPointer.get_struct_record_field (|
                            M.deref (| M.read (| self |) |),
                            "core::num::bignum::Big32x40",
                            "base"
                          |)
                        |);
                        Value.StructTuple "core::ops::range::RangeFull" [] [] []
                      ]
                    |)
                  |)
                |);
                M.borrow (|
                  Pointer.Kind.Ref,
                  M.deref (|
                    M.call_closure (|
                      Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                      M.get_trait_method (|
                        "core::ops::index::Index",
                        Ty.apply
                          (Ty.path "array")
                          [ Value.Integer IntegerKind.Usize 40 ]
                          [ Ty.path "u32" ],
                        [],
                        [ Ty.path "core::ops::range::RangeFull" ],
                        "index",
                        [],
                        []
                      |),
                      [
                        M.borrow (|
                          Pointer.Kind.Ref,
                          M.SubPointer.get_struct_record_field (|
                            M.deref (| M.read (| other |) |),
                            "core::num::bignum::Big32x40",
                            "base"
                          |)
                        |);
                        Value.StructTuple "core::ops::range::RangeFull" [] [] []
                      ]
                    |)
                  |)
                |)
              ]
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::cmp::PartialEq"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) [ Ty.path "core::num::bignum::Big32x40" ]
          Self
          (* Instance *) [ ("eq", InstanceField.Method eq) ].
    End Impl_core_cmp_PartialEq_core_num_bignum_Big32x40_for_core_num_bignum_Big32x40.
    
    Module Impl_core_cmp_Eq_for_core_num_bignum_Big32x40.
      Definition Self : Ty.t := Ty.path "core::num::bignum::Big32x40".
      
      Axiom Implements :
        M.IsTraitInstance
          "core::cmp::Eq"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *) [].
    End Impl_core_cmp_Eq_for_core_num_bignum_Big32x40.
    
    Module Impl_core_cmp_PartialOrd_core_num_bignum_Big32x40_for_core_num_bignum_Big32x40.
      Definition Self : Ty.t := Ty.path "core::num::bignum::Big32x40".
      
      (*
                  fn partial_cmp(&self, other: &$name) -> crate::option::Option<crate::cmp::Ordering> {
                      crate::option::Option::Some(self.cmp(other))
                  }
      *)
      Definition partial_cmp (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                other
              |) in
            Value.StructTuple
              "core::option::Option::Some"
              []
              [ Ty.path "core::cmp::Ordering" ]
              [
                M.call_closure (|
                  Ty.path "core::cmp::Ordering",
                  M.get_trait_method (|
                    "core::cmp::Ord",
                    Ty.path "core::num::bignum::Big32x40",
                    [],
                    [],
                    "cmp",
                    [],
                    []
                  |),
                  [
                    M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| self |) |) |);
                    M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| other |) |) |)
                  ]
                |)
              ]))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::cmp::PartialOrd"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) [ Ty.path "core::num::bignum::Big32x40" ]
          Self
          (* Instance *) [ ("partial_cmp", InstanceField.Method partial_cmp) ].
    End Impl_core_cmp_PartialOrd_core_num_bignum_Big32x40_for_core_num_bignum_Big32x40.
    
    Module Impl_core_cmp_Ord_for_core_num_bignum_Big32x40.
      Definition Self : Ty.t := Ty.path "core::num::bignum::Big32x40".
      
      (*
                  fn cmp(&self, other: &$name) -> crate::cmp::Ordering {
                      use crate::cmp::max;
                      let sz = max(self.size, other.size);
                      let lhs = self.base[..sz].iter().cloned().rev();
                      let rhs = other.base[..sz].iter().cloned().rev();
                      lhs.cmp(rhs)
                  }
      *)
      Definition cmp (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; other ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let other :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                other
              |) in
            M.read (|
              let~ sz : Ty.path "usize" :=
                M.call_closure (|
                  Ty.path "usize",
                  M.get_function (| "core::cmp::max", [], [ Ty.path "usize" ] |),
                  [
                    M.read (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::Big32x40",
                        "size"
                      |)
                    |);
                    M.read (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| other |) |),
                        "core::num::bignum::Big32x40",
                        "size"
                      |)
                    |)
                  ]
                |) in
              let~ lhs :
                  Ty.apply
                    (Ty.path "core::iter::adapters::rev::Rev")
                    []
                    [
                      Ty.apply
                        (Ty.path "core::iter::adapters::cloned::Cloned")
                        []
                        [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ]
                    ] :=
                M.call_closure (|
                  Ty.apply
                    (Ty.path "core::iter::adapters::rev::Rev")
                    []
                    [
                      Ty.apply
                        (Ty.path "core::iter::adapters::cloned::Cloned")
                        []
                        [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ]
                    ],
                  M.get_trait_method (|
                    "core::iter::traits::iterator::Iterator",
                    Ty.apply
                      (Ty.path "core::iter::adapters::cloned::Cloned")
                      []
                      [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ],
                    [],
                    [],
                    "rev",
                    [],
                    []
                  |),
                  [
                    M.call_closure (|
                      Ty.apply
                        (Ty.path "core::iter::adapters::cloned::Cloned")
                        []
                        [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ],
                      M.get_trait_method (|
                        "core::iter::traits::iterator::Iterator",
                        Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                        [],
                        [],
                        "cloned",
                        [],
                        [ Ty.path "u32" ]
                      |),
                      [
                        M.call_closure (|
                          Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                          M.get_associated_function (|
                            Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ],
                            "iter",
                            [],
                            []
                          |),
                          [
                            M.borrow (|
                              Pointer.Kind.Ref,
                              M.deref (|
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "&")
                                    []
                                    [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                  M.get_trait_method (|
                                    "core::ops::index::Index",
                                    Ty.apply
                                      (Ty.path "array")
                                      [ Value.Integer IntegerKind.Usize 40 ]
                                      [ Ty.path "u32" ],
                                    [],
                                    [
                                      Ty.apply
                                        (Ty.path "core::ops::range::RangeTo")
                                        []
                                        [ Ty.path "usize" ]
                                    ],
                                    "index",
                                    [],
                                    []
                                  |),
                                  [
                                    M.borrow (|
                                      Pointer.Kind.Ref,
                                      M.SubPointer.get_struct_record_field (|
                                        M.deref (| M.read (| self |) |),
                                        "core::num::bignum::Big32x40",
                                        "base"
                                      |)
                                    |);
                                    Value.mkStructRecord
                                      "core::ops::range::RangeTo"
                                      []
                                      [ Ty.path "usize" ]
                                      [ ("end_", M.read (| sz |)) ]
                                  ]
                                |)
                              |)
                            |)
                          ]
                        |)
                      ]
                    |)
                  ]
                |) in
              let~ rhs :
                  Ty.apply
                    (Ty.path "core::iter::adapters::rev::Rev")
                    []
                    [
                      Ty.apply
                        (Ty.path "core::iter::adapters::cloned::Cloned")
                        []
                        [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ]
                    ] :=
                M.call_closure (|
                  Ty.apply
                    (Ty.path "core::iter::adapters::rev::Rev")
                    []
                    [
                      Ty.apply
                        (Ty.path "core::iter::adapters::cloned::Cloned")
                        []
                        [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ]
                    ],
                  M.get_trait_method (|
                    "core::iter::traits::iterator::Iterator",
                    Ty.apply
                      (Ty.path "core::iter::adapters::cloned::Cloned")
                      []
                      [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ],
                    [],
                    [],
                    "rev",
                    [],
                    []
                  |),
                  [
                    M.call_closure (|
                      Ty.apply
                        (Ty.path "core::iter::adapters::cloned::Cloned")
                        []
                        [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ],
                      M.get_trait_method (|
                        "core::iter::traits::iterator::Iterator",
                        Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                        [],
                        [],
                        "cloned",
                        [],
                        [ Ty.path "u32" ]
                      |),
                      [
                        M.call_closure (|
                          Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ],
                          M.get_associated_function (|
                            Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ],
                            "iter",
                            [],
                            []
                          |),
                          [
                            M.borrow (|
                              Pointer.Kind.Ref,
                              M.deref (|
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "&")
                                    []
                                    [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ] ],
                                  M.get_trait_method (|
                                    "core::ops::index::Index",
                                    Ty.apply
                                      (Ty.path "array")
                                      [ Value.Integer IntegerKind.Usize 40 ]
                                      [ Ty.path "u32" ],
                                    [],
                                    [
                                      Ty.apply
                                        (Ty.path "core::ops::range::RangeTo")
                                        []
                                        [ Ty.path "usize" ]
                                    ],
                                    "index",
                                    [],
                                    []
                                  |),
                                  [
                                    M.borrow (|
                                      Pointer.Kind.Ref,
                                      M.SubPointer.get_struct_record_field (|
                                        M.deref (| M.read (| other |) |),
                                        "core::num::bignum::Big32x40",
                                        "base"
                                      |)
                                    |);
                                    Value.mkStructRecord
                                      "core::ops::range::RangeTo"
                                      []
                                      [ Ty.path "usize" ]
                                      [ ("end_", M.read (| sz |)) ]
                                  ]
                                |)
                              |)
                            |)
                          ]
                        |)
                      ]
                    |)
                  ]
                |) in
              M.alloc (|
                Ty.path "core::cmp::Ordering",
                M.call_closure (|
                  Ty.path "core::cmp::Ordering",
                  M.get_trait_method (|
                    "core::iter::traits::iterator::Iterator",
                    Ty.apply
                      (Ty.path "core::iter::adapters::rev::Rev")
                      []
                      [
                        Ty.apply
                          (Ty.path "core::iter::adapters::cloned::Cloned")
                          []
                          [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ]
                      ],
                    [],
                    [],
                    "cmp",
                    [],
                    [
                      Ty.apply
                        (Ty.path "core::iter::adapters::rev::Rev")
                        []
                        [
                          Ty.apply
                            (Ty.path "core::iter::adapters::cloned::Cloned")
                            []
                            [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ] ]
                        ]
                    ]
                  |),
                  [ M.read (| lhs |); M.read (| rhs |) ]
                |)
              |)
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::cmp::Ord"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *) [ ("cmp", InstanceField.Method cmp) ].
    End Impl_core_cmp_Ord_for_core_num_bignum_Big32x40.
    
    Module Impl_core_clone_Clone_for_core_num_bignum_Big32x40.
      Definition Self : Ty.t := Ty.path "core::num::bignum::Big32x40".
      
      (*
                  fn clone(&self) -> Self {
                      Self { size: self.size, base: self.base }
                  }
      *)
      Definition clone (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            Value.mkStructRecord
              "core::num::bignum::Big32x40"
              []
              []
              [
                ("size",
                  M.read (|
                    M.SubPointer.get_struct_record_field (|
                      M.deref (| M.read (| self |) |),
                      "core::num::bignum::Big32x40",
                      "size"
                    |)
                  |));
                ("base",
                  M.read (|
                    M.SubPointer.get_struct_record_field (|
                      M.deref (| M.read (| self |) |),
                      "core::num::bignum::Big32x40",
                      "base"
                    |)
                  |))
              ]))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::clone::Clone"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *) [ ("clone", InstanceField.Method clone) ].
    End Impl_core_clone_Clone_for_core_num_bignum_Big32x40.
    
    Module Impl_core_clone_UseCloned_for_core_num_bignum_Big32x40.
      Definition Self : Ty.t := Ty.path "core::num::bignum::Big32x40".
      
      Axiom Implements :
        M.IsTraitInstance
          "core::clone::UseCloned"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *) [].
    End Impl_core_clone_UseCloned_for_core_num_bignum_Big32x40.
    
    Module Impl_core_fmt_Debug_for_core_num_bignum_Big32x40.
      Definition Self : Ty.t := Ty.path "core::num::bignum::Big32x40".
      
      (*
                  fn fmt(&self, f: &mut crate::fmt::Formatter<'_>) -> crate::fmt::Result {
                      let sz = if self.size < 1 { 1 } else { self.size };
                      let digitlen = <$ty>::BITS as usize / 4;
      
                      write!(f, "{:#x}", self.base[sz - 1])?;
                      for &v in self.base[..sz - 1].iter().rev() {
                          write!(f, "_{:01$x}", v, digitlen)?;
                      }
                      crate::result::Result::Ok(())
                  }
      *)
      Definition fmt (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
        match ε, τ, α with
        | [], [], [ self; f ] =>
          ltac:(M.monadic
            (let self :=
              M.alloc (|
                Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::Big32x40" ],
                self
              |) in
            let f :=
              M.alloc (| Ty.apply (Ty.path "&mut") [] [ Ty.path "core::fmt::Formatter" ], f |) in
            M.catch_return
              (Ty.apply
                (Ty.path "core::result::Result")
                []
                [ Ty.tuple []; Ty.path "core::fmt::Error" ]) (|
              ltac:(M.monadic
                (M.read (|
                  let~ sz : Ty.path "usize" :=
                    M.match_operator (|
                      Ty.path "usize",
                      M.alloc (| Ty.tuple [], Value.Tuple [] |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ :=
                              M.alloc (|
                                Ty.path "bool",
                                M.call_closure (|
                                  Ty.path "bool",
                                  BinOp.lt,
                                  [
                                    M.read (|
                                      M.SubPointer.get_struct_record_field (|
                                        M.deref (| M.read (| self |) |),
                                        "core::num::bignum::Big32x40",
                                        "size"
                                      |)
                                    |);
                                    Value.Integer IntegerKind.Usize 1
                                  ]
                                |)
                              |) in
                            let _ :=
                              is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                            Value.Integer IntegerKind.Usize 1));
                        fun γ =>
                          ltac:(M.monadic
                            (M.read (|
                              M.SubPointer.get_struct_record_field (|
                                M.deref (| M.read (| self |) |),
                                "core::num::bignum::Big32x40",
                                "size"
                              |)
                            |)))
                      ]
                    |) in
                  let~ digitlen : Ty.path "usize" :=
                    M.call_closure (|
                      Ty.path "usize",
                      BinOp.Wrap.div,
                      [
                        M.cast
                          (Ty.path "usize")
                          (M.read (|
                            get_associated_constant (| Ty.path "u32", "BITS", Ty.path "u32" |)
                          |));
                        Value.Integer IntegerKind.Usize 4
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.match_operator (|
                      Ty.tuple [],
                      M.alloc (|
                        Ty.apply
                          (Ty.path "core::ops::control_flow::ControlFlow")
                          []
                          [
                            Ty.apply
                              (Ty.path "core::result::Result")
                              []
                              [ Ty.path "core::convert::Infallible"; Ty.path "core::fmt::Error" ];
                            Ty.tuple []
                          ],
                        M.call_closure (|
                          Ty.apply
                            (Ty.path "core::ops::control_flow::ControlFlow")
                            []
                            [
                              Ty.apply
                                (Ty.path "core::result::Result")
                                []
                                [ Ty.path "core::convert::Infallible"; Ty.path "core::fmt::Error" ];
                              Ty.tuple []
                            ],
                          M.get_trait_method (|
                            "core::ops::try_trait::Try",
                            Ty.apply
                              (Ty.path "core::result::Result")
                              []
                              [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                            [],
                            [],
                            "branch",
                            [],
                            []
                          |),
                          [
                            M.call_closure (|
                              Ty.apply
                                (Ty.path "core::result::Result")
                                []
                                [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                              M.get_associated_function (|
                                Ty.path "core::fmt::Formatter",
                                "write_fmt",
                                [],
                                []
                              |),
                              [
                                M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| f |) |) |);
                                M.read (|
                                  let~ args :
                                      Ty.tuple [ Ty.apply (Ty.path "&") [] [ Ty.path "u32" ] ] :=
                                    Value.Tuple
                                      [
                                        M.borrow (|
                                          Pointer.Kind.Ref,
                                          M.SubPointer.get_array_field (|
                                            M.SubPointer.get_struct_record_field (|
                                              M.deref (| M.read (| self |) |),
                                              "core::num::bignum::Big32x40",
                                              "base"
                                            |),
                                            M.call_closure (|
                                              Ty.path "usize",
                                              BinOp.Wrap.sub,
                                              [ M.read (| sz |); Value.Integer IntegerKind.Usize 1 ]
                                            |)
                                          |)
                                        |)
                                      ] in
                                  let~ args :
                                      Ty.apply
                                        (Ty.path "array")
                                        [ Value.Integer IntegerKind.Usize 1 ]
                                        [ Ty.path "core::fmt::rt::Argument" ] :=
                                    Value.Array
                                      [
                                        M.call_closure (|
                                          Ty.path "core::fmt::rt::Argument",
                                          M.get_associated_function (|
                                            Ty.path "core::fmt::rt::Argument",
                                            "new_lower_hex",
                                            [],
                                            [ Ty.path "u32" ]
                                          |),
                                          [
                                            M.borrow (|
                                              Pointer.Kind.Ref,
                                              M.deref (|
                                                M.read (|
                                                  M.SubPointer.get_tuple_field (| args, 0 |)
                                                |)
                                              |)
                                            |)
                                          ]
                                        |)
                                      ] in
                                  M.alloc (|
                                    Ty.path "core::fmt::Arguments",
                                    M.call_closure (|
                                      Ty.path "core::fmt::Arguments",
                                      M.get_associated_function (|
                                        Ty.path "core::fmt::Arguments",
                                        "new",
                                        [
                                          Value.Integer IntegerKind.Usize 6;
                                          Value.Integer IntegerKind.Usize 1
                                        ],
                                        []
                                      |),
                                      [
                                        M.borrow (|
                                          Pointer.Kind.Ref,
                                          M.deref (|
                                            M.mk_byte_str_ref 6 [ 193; 32; 0; 128; 96; 0 ]
                                          |)
                                        |);
                                        M.borrow (|
                                          Pointer.Kind.Ref,
                                          M.deref (| M.borrow (| Pointer.Kind.Ref, args |) |)
                                        |)
                                      ]
                                    |)
                                  |)
                                |)
                              ]
                            |)
                          ]
                        |)
                      |),
                      [
                        fun γ =>
                          ltac:(M.monadic
                            (let γ0_0 :=
                              M.SubPointer.get_struct_tuple_field (|
                                γ,
                                "core::ops::control_flow::ControlFlow::Break",
                                0
                              |) in
                            let residual :=
                              M.copy (|
                                Ty.apply
                                  (Ty.path "core::result::Result")
                                  []
                                  [ Ty.path "core::convert::Infallible"; Ty.path "core::fmt::Error"
                                  ],
                                γ0_0
                              |) in
                            M.never_to_any (|
                              M.read (|
                                M.return_ (|
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "core::result::Result")
                                      []
                                      [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                                    M.get_trait_method (|
                                      "core::ops::try_trait::FromResidual",
                                      Ty.apply
                                        (Ty.path "core::result::Result")
                                        []
                                        [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                                      [],
                                      [
                                        Ty.apply
                                          (Ty.path "core::result::Result")
                                          []
                                          [
                                            Ty.path "core::convert::Infallible";
                                            Ty.path "core::fmt::Error"
                                          ]
                                      ],
                                      "from_residual",
                                      [],
                                      []
                                    |),
                                    [ M.read (| residual |) ]
                                  |)
                                |)
                              |)
                            |)));
                        fun γ =>
                          ltac:(M.monadic
                            (let γ0_0 :=
                              M.SubPointer.get_struct_tuple_field (|
                                γ,
                                "core::ops::control_flow::ControlFlow::Continue",
                                0
                              |) in
                            let val := M.copy (| Ty.tuple [], γ0_0 |) in
                            M.read (| val |)))
                      ]
                    |) in
                  let~ _ : Ty.tuple [] :=
                    M.read (|
                      M.use
                        (M.alloc (|
                          Ty.tuple [],
                          M.match_operator (|
                            Ty.tuple [],
                            M.alloc (|
                              Ty.apply
                                (Ty.path "core::iter::adapters::rev::Rev")
                                []
                                [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u32" ]
                                ],
                              M.call_closure (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::rev::Rev")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::slice::iter::Iter")
                                      []
                                      [ Ty.path "u32" ]
                                  ],
                                M.get_trait_method (|
                                  "core::iter::traits::collect::IntoIterator",
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::rev::Rev")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::slice::iter::Iter")
                                        []
                                        [ Ty.path "u32" ]
                                    ],
                                  [],
                                  [],
                                  "into_iter",
                                  [],
                                  []
                                |),
                                [
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::rev::Rev")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::slice::iter::Iter")
                                          []
                                          [ Ty.path "u32" ]
                                      ],
                                    M.get_trait_method (|
                                      "core::iter::traits::iterator::Iterator",
                                      Ty.apply
                                        (Ty.path "core::slice::iter::Iter")
                                        []
                                        [ Ty.path "u32" ],
                                      [],
                                      [],
                                      "rev",
                                      [],
                                      []
                                    |),
                                    [
                                      M.call_closure (|
                                        Ty.apply
                                          (Ty.path "core::slice::iter::Iter")
                                          []
                                          [ Ty.path "u32" ],
                                        M.get_associated_function (|
                                          Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ],
                                          "iter",
                                          [],
                                          []
                                        |),
                                        [
                                          M.borrow (|
                                            Pointer.Kind.Ref,
                                            M.deref (|
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "&")
                                                  []
                                                  [ Ty.apply (Ty.path "slice") [] [ Ty.path "u32" ]
                                                  ],
                                                M.get_trait_method (|
                                                  "core::ops::index::Index",
                                                  Ty.apply
                                                    (Ty.path "array")
                                                    [ Value.Integer IntegerKind.Usize 40 ]
                                                    [ Ty.path "u32" ],
                                                  [],
                                                  [
                                                    Ty.apply
                                                      (Ty.path "core::ops::range::RangeTo")
                                                      []
                                                      [ Ty.path "usize" ]
                                                  ],
                                                  "index",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.Ref,
                                                    M.SubPointer.get_struct_record_field (|
                                                      M.deref (| M.read (| self |) |),
                                                      "core::num::bignum::Big32x40",
                                                      "base"
                                                    |)
                                                  |);
                                                  Value.mkStructRecord
                                                    "core::ops::range::RangeTo"
                                                    []
                                                    [ Ty.path "usize" ]
                                                    [
                                                      ("end_",
                                                        M.call_closure (|
                                                          Ty.path "usize",
                                                          BinOp.Wrap.sub,
                                                          [
                                                            M.read (| sz |);
                                                            Value.Integer IntegerKind.Usize 1
                                                          ]
                                                        |))
                                                    ]
                                                ]
                                              |)
                                            |)
                                          |)
                                        ]
                                      |)
                                    ]
                                  |)
                                ]
                              |)
                            |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let~ iter :
                                      Ty.apply
                                        (Ty.path "core::iter::adapters::rev::Rev")
                                        []
                                        [
                                          Ty.apply
                                            (Ty.path "core::slice::iter::Iter")
                                            []
                                            [ Ty.path "u32" ]
                                        ] :=
                                    M.read (| γ |) in
                                  M.read (|
                                    M.loop (|
                                      Ty.tuple [],
                                      ltac:(M.monadic
                                        (let~ _ : Ty.tuple [] :=
                                          M.match_operator (|
                                            Ty.tuple [],
                                            M.alloc (|
                                              Ty.apply
                                                (Ty.path "core::option::Option")
                                                []
                                                [ Ty.apply (Ty.path "&") [] [ Ty.path "u32" ] ],
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [ Ty.apply (Ty.path "&") [] [ Ty.path "u32" ] ],
                                                M.get_trait_method (|
                                                  "core::iter::traits::iterator::Iterator",
                                                  Ty.apply
                                                    (Ty.path "core::iter::adapters::rev::Rev")
                                                    []
                                                    [
                                                      Ty.apply
                                                        (Ty.path "core::slice::iter::Iter")
                                                        []
                                                        [ Ty.path "u32" ]
                                                    ],
                                                  [],
                                                  [],
                                                  "next",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.MutRef,
                                                    M.deref (|
                                                      M.borrow (| Pointer.Kind.MutRef, iter |)
                                                    |)
                                                  |)
                                                ]
                                              |)
                                            |),
                                            [
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let _ :=
                                                    M.is_struct_tuple (|
                                                      γ,
                                                      "core::option::Option::None"
                                                    |) in
                                                  M.never_to_any (| M.read (| M.break (||) |) |)));
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let γ0_0 :=
                                                    M.SubPointer.get_struct_tuple_field (|
                                                      γ,
                                                      "core::option::Option::Some",
                                                      0
                                                    |) in
                                                  let γ0_0 := M.deref (| M.read (| γ0_0 |) |) in
                                                  let v := M.copy (| Ty.path "u32", γ0_0 |) in
                                                  M.read (|
                                                    let~ _ : Ty.tuple [] :=
                                                      M.match_operator (|
                                                        Ty.tuple [],
                                                        M.alloc (|
                                                          Ty.apply
                                                            (Ty.path
                                                              "core::ops::control_flow::ControlFlow")
                                                            []
                                                            [
                                                              Ty.apply
                                                                (Ty.path "core::result::Result")
                                                                []
                                                                [
                                                                  Ty.path
                                                                    "core::convert::Infallible";
                                                                  Ty.path "core::fmt::Error"
                                                                ];
                                                              Ty.tuple []
                                                            ],
                                                          M.call_closure (|
                                                            Ty.apply
                                                              (Ty.path
                                                                "core::ops::control_flow::ControlFlow")
                                                              []
                                                              [
                                                                Ty.apply
                                                                  (Ty.path "core::result::Result")
                                                                  []
                                                                  [
                                                                    Ty.path
                                                                      "core::convert::Infallible";
                                                                    Ty.path "core::fmt::Error"
                                                                  ];
                                                                Ty.tuple []
                                                              ],
                                                            M.get_trait_method (|
                                                              "core::ops::try_trait::Try",
                                                              Ty.apply
                                                                (Ty.path "core::result::Result")
                                                                []
                                                                [
                                                                  Ty.tuple [];
                                                                  Ty.path "core::fmt::Error"
                                                                ],
                                                              [],
                                                              [],
                                                              "branch",
                                                              [],
                                                              []
                                                            |),
                                                            [
                                                              M.call_closure (|
                                                                Ty.apply
                                                                  (Ty.path "core::result::Result")
                                                                  []
                                                                  [
                                                                    Ty.tuple [];
                                                                    Ty.path "core::fmt::Error"
                                                                  ],
                                                                M.get_associated_function (|
                                                                  Ty.path "core::fmt::Formatter",
                                                                  "write_fmt",
                                                                  [],
                                                                  []
                                                                |),
                                                                [
                                                                  M.borrow (|
                                                                    Pointer.Kind.MutRef,
                                                                    M.deref (| M.read (| f |) |)
                                                                  |);
                                                                  M.read (|
                                                                    let~ args :
                                                                        Ty.tuple
                                                                          [
                                                                            Ty.apply
                                                                              (Ty.path "&")
                                                                              []
                                                                              [ Ty.path "u32" ];
                                                                            Ty.apply
                                                                              (Ty.path "&")
                                                                              []
                                                                              [ Ty.path "usize" ]
                                                                          ] :=
                                                                      Value.Tuple
                                                                        [
                                                                          M.borrow (|
                                                                            Pointer.Kind.Ref,
                                                                            v
                                                                          |);
                                                                          M.borrow (|
                                                                            Pointer.Kind.Ref,
                                                                            digitlen
                                                                          |)
                                                                        ] in
                                                                    let~ args :
                                                                        Ty.apply
                                                                          (Ty.path "array")
                                                                          [
                                                                            Value.Integer
                                                                              IntegerKind.Usize
                                                                              2
                                                                          ]
                                                                          [
                                                                            Ty.path
                                                                              "core::fmt::rt::Argument"
                                                                          ] :=
                                                                      Value.Array
                                                                        [
                                                                          M.call_closure (|
                                                                            Ty.path
                                                                              "core::fmt::rt::Argument",
                                                                            M.get_associated_function (|
                                                                              Ty.path
                                                                                "core::fmt::rt::Argument",
                                                                              "new_lower_hex",
                                                                              [],
                                                                              [ Ty.path "u32" ]
                                                                            |),
                                                                            [
                                                                              M.borrow (|
                                                                                Pointer.Kind.Ref,
                                                                                M.deref (|
                                                                                  M.read (|
                                                                                    M.SubPointer.get_tuple_field (|
                                                                                      args,
                                                                                      0
                                                                                    |)
                                                                                  |)
                                                                                |)
                                                                              |)
                                                                            ]
                                                                          |);
                                                                          M.call_closure (|
                                                                            Ty.path
                                                                              "core::fmt::rt::Argument",
                                                                            M.get_associated_function (|
                                                                              Ty.path
                                                                                "core::fmt::rt::Argument",
                                                                              "from_usize",
                                                                              [],
                                                                              []
                                                                            |),
                                                                            [
                                                                              M.borrow (|
                                                                                Pointer.Kind.Ref,
                                                                                M.deref (|
                                                                                  M.read (|
                                                                                    M.SubPointer.get_tuple_field (|
                                                                                      args,
                                                                                      1
                                                                                    |)
                                                                                  |)
                                                                                |)
                                                                              |)
                                                                            ]
                                                                          |)
                                                                        ] in
                                                                    M.alloc (|
                                                                      Ty.path
                                                                        "core::fmt::Arguments",
                                                                      M.call_closure (|
                                                                        Ty.path
                                                                          "core::fmt::Arguments",
                                                                        M.get_associated_function (|
                                                                          Ty.path
                                                                            "core::fmt::Arguments",
                                                                          "new",
                                                                          [
                                                                            Value.Integer
                                                                              IntegerKind.Usize
                                                                              10;
                                                                            Value.Integer
                                                                              IntegerKind.Usize
                                                                              2
                                                                          ],
                                                                          []
                                                                        |),
                                                                        [
                                                                          M.borrow (|
                                                                            Pointer.Kind.Ref,
                                                                            M.deref (|
                                                                              M.mk_byte_str_ref
                                                                                10
                                                                                [
                                                                                  1;
                                                                                  95;
                                                                                  211;
                                                                                  32;
                                                                                  0;
                                                                                  0;
                                                                                  105;
                                                                                  1;
                                                                                  0;
                                                                                  0
                                                                                ]
                                                                            |)
                                                                          |);
                                                                          M.borrow (|
                                                                            Pointer.Kind.Ref,
                                                                            M.deref (|
                                                                              M.borrow (|
                                                                                Pointer.Kind.Ref,
                                                                                args
                                                                              |)
                                                                            |)
                                                                          |)
                                                                        ]
                                                                      |)
                                                                    |)
                                                                  |)
                                                                ]
                                                              |)
                                                            ]
                                                          |)
                                                        |),
                                                        [
                                                          fun γ =>
                                                            ltac:(M.monadic
                                                              (let γ0_0 :=
                                                                M.SubPointer.get_struct_tuple_field (|
                                                                  γ,
                                                                  "core::ops::control_flow::ControlFlow::Break",
                                                                  0
                                                                |) in
                                                              let residual :=
                                                                M.copy (|
                                                                  Ty.apply
                                                                    (Ty.path "core::result::Result")
                                                                    []
                                                                    [
                                                                      Ty.path
                                                                        "core::convert::Infallible";
                                                                      Ty.path "core::fmt::Error"
                                                                    ],
                                                                  γ0_0
                                                                |) in
                                                              M.never_to_any (|
                                                                M.read (|
                                                                  M.return_ (|
                                                                    M.call_closure (|
                                                                      Ty.apply
                                                                        (Ty.path
                                                                          "core::result::Result")
                                                                        []
                                                                        [
                                                                          Ty.tuple [];
                                                                          Ty.path "core::fmt::Error"
                                                                        ],
                                                                      M.get_trait_method (|
                                                                        "core::ops::try_trait::FromResidual",
                                                                        Ty.apply
                                                                          (Ty.path
                                                                            "core::result::Result")
                                                                          []
                                                                          [
                                                                            Ty.tuple [];
                                                                            Ty.path
                                                                              "core::fmt::Error"
                                                                          ],
                                                                        [],
                                                                        [
                                                                          Ty.apply
                                                                            (Ty.path
                                                                              "core::result::Result")
                                                                            []
                                                                            [
                                                                              Ty.path
                                                                                "core::convert::Infallible";
                                                                              Ty.path
                                                                                "core::fmt::Error"
                                                                            ]
                                                                        ],
                                                                        "from_residual",
                                                                        [],
                                                                        []
                                                                      |),
                                                                      [ M.read (| residual |) ]
                                                                    |)
                                                                  |)
                                                                |)
                                                              |)));
                                                          fun γ =>
                                                            ltac:(M.monadic
                                                              (let γ0_0 :=
                                                                M.SubPointer.get_struct_tuple_field (|
                                                                  γ,
                                                                  "core::ops::control_flow::ControlFlow::Continue",
                                                                  0
                                                                |) in
                                                              let val :=
                                                                M.copy (| Ty.tuple [], γ0_0 |) in
                                                              M.read (| val |)))
                                                        ]
                                                      |) in
                                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                  |)))
                                            ]
                                          |) in
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                    |)
                                  |)))
                            ]
                          |)
                        |))
                    |) in
                  M.alloc (|
                    Ty.apply
                      (Ty.path "core::result::Result")
                      []
                      [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                    Value.StructTuple
                      "core::result::Result::Ok"
                      []
                      [ Ty.tuple []; Ty.path "core::fmt::Error" ]
                      [ Value.Tuple [] ]
                  |)
                |)))
            |)))
        | _, _, _ => M.impossible "wrong number of arguments"
        end.
      
      Axiom Implements :
        M.IsTraitInstance
          "core::fmt::Debug"
          (* Trait polymorphic consts *) []
          (* Trait polymorphic types *) []
          Self
          (* Instance *) [ ("fmt", InstanceField.Method fmt) ].
    End Impl_core_fmt_Debug_for_core_num_bignum_Big32x40.
    
    Module tests.
      (* StructRecord
        {
          name := "Big8x3";
          const_params := [];
          ty_params := [];
          fields :=
            [
              ("size", Ty.path "usize");
              ("base",
                Ty.apply (Ty.path "array") [ Value.Integer IntegerKind.Usize 3 ] [ Ty.path "u8" ])
            ];
        } *)
      
      Module Impl_core_num_bignum_tests_Big8x3.
        Definition Self : Ty.t := Ty.path "core::num::bignum::tests::Big8x3".
        
        (*
                    pub fn from_small(v: $ty) -> $name {
                        let mut base = [0; $n];
                        base[0] = v;
                        $name { size: 1, base }
                    }
        *)
        Definition from_small (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ v ] =>
            ltac:(M.monadic
              (let v := M.alloc (| Ty.path "u8", v |) in
              M.read (|
                let~ base :
                    Ty.apply
                      (Ty.path "array")
                      [ Value.Integer IntegerKind.Usize 3 ]
                      [ Ty.path "u8" ] :=
                  lib.repeat (|
                    Value.Integer IntegerKind.U8 0,
                    Value.Integer IntegerKind.Usize 3
                  |) in
                let~ _ : Ty.tuple [] :=
                  M.write (|
                    M.SubPointer.get_array_field (| base, Value.Integer IntegerKind.Usize 0 |),
                    M.read (| v |)
                  |) in
                M.alloc (|
                  Ty.path "core::num::bignum::tests::Big8x3",
                  Value.mkStructRecord
                    "core::num::bignum::tests::Big8x3"
                    []
                    []
                    [ ("size", Value.Integer IntegerKind.Usize 1); ("base", M.read (| base |)) ]
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_from_small :
          M.IsAssociatedFunction.C Self "from_small" from_small.
        Admitted.
        Global Typeclasses Opaque from_small.
        
        (*
                    pub fn from_u64(mut v: u64) -> $name {
                        let mut base = [0; $n];
                        let mut sz = 0;
                        while v > 0 {
                            base[sz] = v as $ty;
                            v >>= <$ty>::BITS;
                            sz += 1;
                        }
                        $name { size: sz, base }
                    }
        *)
        Definition from_u64 (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ v ] =>
            ltac:(M.monadic
              (let v := M.alloc (| Ty.path "u64", v |) in
              M.read (|
                let~ base :
                    Ty.apply
                      (Ty.path "array")
                      [ Value.Integer IntegerKind.Usize 3 ]
                      [ Ty.path "u8" ] :=
                  lib.repeat (|
                    Value.Integer IntegerKind.U8 0,
                    Value.Integer IntegerKind.Usize 3
                  |) in
                let~ sz : Ty.path "usize" := Value.Integer IntegerKind.Usize 0 in
                let~ _ : Ty.tuple [] :=
                  M.read (|
                    M.loop (|
                      Ty.tuple [],
                      ltac:(M.monadic
                        (M.alloc (|
                          Ty.tuple [],
                          M.match_operator (|
                            Ty.tuple [],
                            M.alloc (| Ty.tuple [], Value.Tuple [] |),
                            [
                              fun γ =>
                                ltac:(M.monadic
                                  (let γ :=
                                    M.alloc (|
                                      Ty.path "bool",
                                      M.call_closure (|
                                        Ty.path "bool",
                                        BinOp.gt,
                                        [ M.read (| v |); Value.Integer IntegerKind.U64 0 ]
                                      |)
                                    |) in
                                  let _ :=
                                    is_constant_or_break_match (|
                                      M.read (| γ |),
                                      Value.Bool true
                                    |) in
                                  M.read (|
                                    let~ _ : Ty.tuple [] :=
                                      M.write (|
                                        M.SubPointer.get_array_field (| base, M.read (| sz |) |),
                                        M.cast (Ty.path "u8") (M.read (| v |))
                                      |) in
                                    let~ _ : Ty.tuple [] :=
                                      let β := v in
                                      M.write (|
                                        β,
                                        M.call_closure (|
                                          Ty.path "u64",
                                          BinOp.Wrap.shr,
                                          [
                                            M.read (| β |);
                                            M.read (|
                                              get_associated_constant (|
                                                Ty.path "u8",
                                                "BITS",
                                                Ty.path "u32"
                                              |)
                                            |)
                                          ]
                                        |)
                                      |) in
                                    let~ _ : Ty.tuple [] :=
                                      let β := sz in
                                      M.write (|
                                        β,
                                        M.call_closure (|
                                          Ty.path "usize",
                                          BinOp.Wrap.add,
                                          [ M.read (| β |); Value.Integer IntegerKind.Usize 1 ]
                                        |)
                                      |) in
                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                  |)));
                              fun γ =>
                                ltac:(M.monadic
                                  (M.never_to_any (|
                                    M.read (|
                                      let~ _ : Ty.tuple [] :=
                                        M.never_to_any (| M.read (| M.break (||) |) |) in
                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                    |)
                                  |)))
                            ]
                          |)
                        |)))
                    |)
                  |) in
                M.alloc (|
                  Ty.path "core::num::bignum::tests::Big8x3",
                  Value.mkStructRecord
                    "core::num::bignum::tests::Big8x3"
                    []
                    []
                    [ ("size", M.read (| sz |)); ("base", M.read (| base |)) ]
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_from_u64 :
          M.IsAssociatedFunction.C Self "from_u64" from_u64.
        Admitted.
        Global Typeclasses Opaque from_u64.
        
        (*
                    pub fn digits(&self) -> &[$ty] {
                        &self.base[..self.size]
                    }
        *)
        Definition digits (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              M.borrow (|
                Pointer.Kind.Ref,
                M.deref (|
                  M.borrow (|
                    Pointer.Kind.Ref,
                    M.deref (|
                      M.call_closure (|
                        Ty.apply
                          (Ty.path "&")
                          []
                          [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                        M.get_trait_method (|
                          "core::ops::index::Index",
                          Ty.apply
                            (Ty.path "array")
                            [ Value.Integer IntegerKind.Usize 3 ]
                            [ Ty.path "u8" ],
                          [],
                          [ Ty.apply (Ty.path "core::ops::range::RangeTo") [] [ Ty.path "usize" ] ],
                          "index",
                          [],
                          []
                        |),
                        [
                          M.borrow (|
                            Pointer.Kind.Ref,
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| self |) |),
                              "core::num::bignum::tests::Big8x3",
                              "base"
                            |)
                          |);
                          Value.mkStructRecord
                            "core::ops::range::RangeTo"
                            []
                            [ Ty.path "usize" ]
                            [
                              ("end_",
                                M.read (|
                                  M.SubPointer.get_struct_record_field (|
                                    M.deref (| M.read (| self |) |),
                                    "core::num::bignum::tests::Big8x3",
                                    "size"
                                  |)
                                |))
                            ]
                        ]
                      |)
                    |)
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_digits : M.IsAssociatedFunction.C Self "digits" digits.
        Admitted.
        Global Typeclasses Opaque digits.
        
        (*
                    pub fn get_bit(&self, i: usize) -> u8 {
                        let digitbits = <$ty>::BITS as usize;
                        let d = i / digitbits;
                        let b = i % digitbits;
                        ((self.base[d] >> b) & 1) as u8
                    }
        *)
        Definition get_bit (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; i ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let i := M.alloc (| Ty.path "usize", i |) in
              M.read (|
                let~ digitbits : Ty.path "usize" :=
                  M.cast
                    (Ty.path "usize")
                    (M.read (|
                      get_associated_constant (| Ty.path "u8", "BITS", Ty.path "u32" |)
                    |)) in
                let~ d : Ty.path "usize" :=
                  M.call_closure (|
                    Ty.path "usize",
                    BinOp.Wrap.div,
                    [ M.read (| i |); M.read (| digitbits |) ]
                  |) in
                let~ b : Ty.path "usize" :=
                  M.call_closure (|
                    Ty.path "usize",
                    BinOp.Wrap.rem,
                    [ M.read (| i |); M.read (| digitbits |) ]
                  |) in
                M.use
                  (M.alloc (|
                    Ty.path "u8",
                    M.call_closure (|
                      Ty.path "u8",
                      BinOp.Wrap.bit_and,
                      [
                        M.call_closure (|
                          Ty.path "u8",
                          BinOp.Wrap.shr,
                          [
                            M.read (|
                              M.SubPointer.get_array_field (|
                                M.SubPointer.get_struct_record_field (|
                                  M.deref (| M.read (| self |) |),
                                  "core::num::bignum::tests::Big8x3",
                                  "base"
                                |),
                                M.read (| d |)
                              |)
                            |);
                            M.read (| b |)
                          ]
                        |);
                        Value.Integer IntegerKind.U8 1
                      ]
                    |)
                  |))
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_get_bit :
          M.IsAssociatedFunction.C Self "get_bit" get_bit.
        Admitted.
        Global Typeclasses Opaque get_bit.
        
        (*
                    pub fn is_zero(&self) -> bool {
                        self.digits().iter().all(|&v| v == 0)
                    }
        *)
        Definition is_zero (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              M.call_closure (|
                Ty.path "bool",
                M.get_trait_method (|
                  "core::iter::traits::iterator::Iterator",
                  Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                  [],
                  [],
                  "all",
                  [],
                  [ Ty.function [ Ty.apply (Ty.path "&") [] [ Ty.path "u8" ] ] (Ty.path "bool") ]
                |),
                [
                  M.borrow (|
                    Pointer.Kind.MutRef,
                    M.alloc (|
                      Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                      M.call_closure (|
                        Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                        M.get_associated_function (|
                          Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ],
                          "iter",
                          [],
                          []
                        |),
                        [
                          M.borrow (|
                            Pointer.Kind.Ref,
                            M.deref (|
                              M.call_closure (|
                                Ty.apply
                                  (Ty.path "&")
                                  []
                                  [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                M.get_associated_function (|
                                  Ty.path "core::num::bignum::tests::Big8x3",
                                  "digits",
                                  [],
                                  []
                                |),
                                [ M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| self |) |) |) ]
                              |)
                            |)
                          |)
                        ]
                      |)
                    |)
                  |);
                  M.closure
                    (fun γ =>
                      ltac:(M.monadic
                        match γ with
                        | [ α0 ] =>
                          ltac:(M.monadic
                            (M.match_operator (|
                              Ty.path "bool",
                              M.alloc (| Ty.apply (Ty.path "&") [] [ Ty.path "u8" ], α0 |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let γ := M.deref (| M.read (| γ |) |) in
                                    let v := M.copy (| Ty.path "u8", γ |) in
                                    M.call_closure (|
                                      Ty.path "bool",
                                      BinOp.eq,
                                      [ M.read (| v |); Value.Integer IntegerKind.U8 0 ]
                                    |)))
                              ]
                            |)))
                        | _ => M.impossible "wrong number of arguments"
                        end))
                ]
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_is_zero :
          M.IsAssociatedFunction.C Self "is_zero" is_zero.
        Admitted.
        Global Typeclasses Opaque is_zero.
        
        (*
                    pub fn bit_length(&self) -> usize {
                        let digitbits = <$ty>::BITS as usize;
                        let digits = self.digits();
                        // Find the most significant non-zero digit.
                        let msd = digits.iter().rposition(|&x| x != 0);
                        match msd {
                            Some(msd) => msd * digitbits + digits[msd].ilog2() as usize + 1,
                            // There are no non-zero digits, i.e., the number is zero.
                            _ => 0,
                        }
                    }
        *)
        Definition bit_length (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              M.read (|
                let~ digitbits : Ty.path "usize" :=
                  M.cast
                    (Ty.path "usize")
                    (M.read (|
                      get_associated_constant (| Ty.path "u8", "BITS", Ty.path "u32" |)
                    |)) in
                let~ digits :
                    Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ] :=
                  M.call_closure (|
                    Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                    M.get_associated_function (|
                      Ty.path "core::num::bignum::tests::Big8x3",
                      "digits",
                      [],
                      []
                    |),
                    [ M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| self |) |) |) ]
                  |) in
                let~ msd : Ty.apply (Ty.path "core::option::Option") [] [ Ty.path "usize" ] :=
                  M.call_closure (|
                    Ty.apply (Ty.path "core::option::Option") [] [ Ty.path "usize" ],
                    M.get_trait_method (|
                      "core::iter::traits::iterator::Iterator",
                      Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                      [],
                      [],
                      "rposition",
                      [],
                      [ Ty.function [ Ty.apply (Ty.path "&") [] [ Ty.path "u8" ] ] (Ty.path "bool")
                      ]
                    |),
                    [
                      M.borrow (|
                        Pointer.Kind.MutRef,
                        M.alloc (|
                          Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                          M.call_closure (|
                            Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                            M.get_associated_function (|
                              Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ],
                              "iter",
                              [],
                              []
                            |),
                            [ M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| digits |) |) |) ]
                          |)
                        |)
                      |);
                      M.closure
                        (fun γ =>
                          ltac:(M.monadic
                            match γ with
                            | [ α0 ] =>
                              ltac:(M.monadic
                                (M.match_operator (|
                                  Ty.path "bool",
                                  M.alloc (| Ty.apply (Ty.path "&") [] [ Ty.path "u8" ], α0 |),
                                  [
                                    fun γ =>
                                      ltac:(M.monadic
                                        (let γ := M.deref (| M.read (| γ |) |) in
                                        let x := M.copy (| Ty.path "u8", γ |) in
                                        M.call_closure (|
                                          Ty.path "bool",
                                          BinOp.ne,
                                          [ M.read (| x |); Value.Integer IntegerKind.U8 0 ]
                                        |)))
                                  ]
                                |)))
                            | _ => M.impossible "wrong number of arguments"
                            end))
                    ]
                  |) in
                M.alloc (|
                  Ty.path "usize",
                  M.match_operator (|
                    Ty.path "usize",
                    msd,
                    [
                      fun γ =>
                        ltac:(M.monadic
                          (let γ0_0 :=
                            M.SubPointer.get_struct_tuple_field (|
                              γ,
                              "core::option::Option::Some",
                              0
                            |) in
                          let msd := M.copy (| Ty.path "usize", γ0_0 |) in
                          M.call_closure (|
                            Ty.path "usize",
                            BinOp.Wrap.add,
                            [
                              M.call_closure (|
                                Ty.path "usize",
                                BinOp.Wrap.add,
                                [
                                  M.call_closure (|
                                    Ty.path "usize",
                                    BinOp.Wrap.mul,
                                    [ M.read (| msd |); M.read (| digitbits |) ]
                                  |);
                                  M.cast
                                    (Ty.path "usize")
                                    (M.call_closure (|
                                      Ty.path "u32",
                                      M.get_associated_function (| Ty.path "u8", "ilog2", [], [] |),
                                      [
                                        M.read (|
                                          M.SubPointer.get_array_field (|
                                            M.deref (| M.read (| digits |) |),
                                            M.read (| msd |)
                                          |)
                                        |)
                                      ]
                                    |))
                                ]
                              |);
                              Value.Integer IntegerKind.Usize 1
                            ]
                          |)));
                      fun γ => ltac:(M.monadic (Value.Integer IntegerKind.Usize 0))
                    ]
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_bit_length :
          M.IsAssociatedFunction.C Self "bit_length" bit_length.
        Admitted.
        Global Typeclasses Opaque bit_length.
        
        (*
                    pub fn add<'a>(&'a mut self, other: &$name) -> &'a mut $name {
                        use crate::{cmp, iter};
        
                        let mut sz = cmp::max(self.size, other.size);
                        let mut carry = false;
                        for (a, b) in iter::zip(&mut self.base[..sz], &other.base[..sz]) {
                            let (v, c) = ( *a).carrying_add( *b, carry);
                            *a = v;
                            carry = c;
                        }
                        if carry {
                            self.base[sz] = 1;
                            sz += 1;
                        }
                        self.size = sz;
                        self
                    }
        *)
        Definition add (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  other
                |) in
              M.borrow (|
                Pointer.Kind.MutRef,
                M.deref (|
                  M.read (|
                    let~ sz : Ty.path "usize" :=
                      M.call_closure (|
                        Ty.path "usize",
                        M.get_function (| "core::cmp::max", [], [ Ty.path "usize" ] |),
                        [
                          M.read (|
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| self |) |),
                              "core::num::bignum::tests::Big8x3",
                              "size"
                            |)
                          |);
                          M.read (|
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| other |) |),
                              "core::num::bignum::tests::Big8x3",
                              "size"
                            |)
                          |)
                        ]
                      |) in
                    let~ carry : Ty.path "bool" := Value.Bool false in
                    let~ _ : Ty.tuple [] :=
                      M.read (|
                        M.use
                          (M.alloc (|
                            Ty.tuple [],
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::zip::Zip")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::slice::iter::IterMut")
                                      []
                                      [ Ty.path "u8" ];
                                    Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ]
                                  ],
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::zip::Zip")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::slice::iter::IterMut")
                                        []
                                        [ Ty.path "u8" ];
                                      Ty.apply
                                        (Ty.path "core::slice::iter::Iter")
                                        []
                                        [ Ty.path "u8" ]
                                    ],
                                  M.get_trait_method (|
                                    "core::iter::traits::collect::IntoIterator",
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::zip::Zip")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::slice::iter::IterMut")
                                          []
                                          [ Ty.path "u8" ];
                                        Ty.apply
                                          (Ty.path "core::slice::iter::Iter")
                                          []
                                          [ Ty.path "u8" ]
                                      ],
                                    [],
                                    [],
                                    "into_iter",
                                    [],
                                    []
                                  |),
                                  [
                                    M.call_closure (|
                                      Ty.apply
                                        (Ty.path "core::iter::adapters::zip::Zip")
                                        []
                                        [
                                          Ty.apply
                                            (Ty.path "core::slice::iter::IterMut")
                                            []
                                            [ Ty.path "u8" ];
                                          Ty.apply
                                            (Ty.path "core::slice::iter::Iter")
                                            []
                                            [ Ty.path "u8" ]
                                        ],
                                      M.get_function (|
                                        "core::iter::adapters::zip::zip",
                                        [],
                                        [
                                          Ty.apply
                                            (Ty.path "&mut")
                                            []
                                            [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ];
                                          Ty.apply
                                            (Ty.path "&")
                                            []
                                            [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ]
                                        ]
                                      |),
                                      [
                                        M.borrow (|
                                          Pointer.Kind.MutRef,
                                          M.deref (|
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "&mut")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                              M.get_trait_method (|
                                                "core::ops::index::IndexMut",
                                                Ty.apply
                                                  (Ty.path "array")
                                                  [ Value.Integer IntegerKind.Usize 3 ]
                                                  [ Ty.path "u8" ],
                                                [],
                                                [
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::RangeTo")
                                                    []
                                                    [ Ty.path "usize" ]
                                                ],
                                                "index_mut",
                                                [],
                                                []
                                              |),
                                              [
                                                M.borrow (|
                                                  Pointer.Kind.MutRef,
                                                  M.SubPointer.get_struct_record_field (|
                                                    M.deref (| M.read (| self |) |),
                                                    "core::num::bignum::tests::Big8x3",
                                                    "base"
                                                  |)
                                                |);
                                                Value.mkStructRecord
                                                  "core::ops::range::RangeTo"
                                                  []
                                                  [ Ty.path "usize" ]
                                                  [ ("end_", M.read (| sz |)) ]
                                              ]
                                            |)
                                          |)
                                        |);
                                        M.borrow (|
                                          Pointer.Kind.Ref,
                                          M.deref (|
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "&")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                              M.get_trait_method (|
                                                "core::ops::index::Index",
                                                Ty.apply
                                                  (Ty.path "array")
                                                  [ Value.Integer IntegerKind.Usize 3 ]
                                                  [ Ty.path "u8" ],
                                                [],
                                                [
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::RangeTo")
                                                    []
                                                    [ Ty.path "usize" ]
                                                ],
                                                "index",
                                                [],
                                                []
                                              |),
                                              [
                                                M.borrow (|
                                                  Pointer.Kind.Ref,
                                                  M.SubPointer.get_struct_record_field (|
                                                    M.deref (| M.read (| other |) |),
                                                    "core::num::bignum::tests::Big8x3",
                                                    "base"
                                                  |)
                                                |);
                                                Value.mkStructRecord
                                                  "core::ops::range::RangeTo"
                                                  []
                                                  [ Ty.path "usize" ]
                                                  [ ("end_", M.read (| sz |)) ]
                                              ]
                                            |)
                                          |)
                                        |)
                                      ]
                                    |)
                                  ]
                                |)
                              |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let~ iter :
                                        Ty.apply
                                          (Ty.path "core::iter::adapters::zip::Zip")
                                          []
                                          [
                                            Ty.apply
                                              (Ty.path "core::slice::iter::IterMut")
                                              []
                                              [ Ty.path "u8" ];
                                            Ty.apply
                                              (Ty.path "core::slice::iter::Iter")
                                              []
                                              [ Ty.path "u8" ]
                                          ] :=
                                      M.read (| γ |) in
                                    M.read (|
                                      M.loop (|
                                        Ty.tuple [],
                                        ltac:(M.monadic
                                          (let~ _ : Ty.tuple [] :=
                                            M.match_operator (|
                                              Ty.tuple [],
                                              M.alloc (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [
                                                    Ty.tuple
                                                      [
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [ Ty.path "u8" ];
                                                        Ty.apply (Ty.path "&") [] [ Ty.path "u8" ]
                                                      ]
                                                  ],
                                                M.call_closure (|
                                                  Ty.apply
                                                    (Ty.path "core::option::Option")
                                                    []
                                                    [
                                                      Ty.tuple
                                                        [
                                                          Ty.apply
                                                            (Ty.path "&mut")
                                                            []
                                                            [ Ty.path "u8" ];
                                                          Ty.apply (Ty.path "&") [] [ Ty.path "u8" ]
                                                        ]
                                                    ],
                                                  M.get_trait_method (|
                                                    "core::iter::traits::iterator::Iterator",
                                                    Ty.apply
                                                      (Ty.path "core::iter::adapters::zip::Zip")
                                                      []
                                                      [
                                                        Ty.apply
                                                          (Ty.path "core::slice::iter::IterMut")
                                                          []
                                                          [ Ty.path "u8" ];
                                                        Ty.apply
                                                          (Ty.path "core::slice::iter::Iter")
                                                          []
                                                          [ Ty.path "u8" ]
                                                      ],
                                                    [],
                                                    [],
                                                    "next",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    M.borrow (|
                                                      Pointer.Kind.MutRef,
                                                      M.deref (|
                                                        M.borrow (| Pointer.Kind.MutRef, iter |)
                                                      |)
                                                    |)
                                                  ]
                                                |)
                                              |),
                                              [
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let _ :=
                                                      M.is_struct_tuple (|
                                                        γ,
                                                        "core::option::Option::None"
                                                      |) in
                                                    M.never_to_any (|
                                                      M.read (| M.break (||) |)
                                                    |)));
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let γ0_0 :=
                                                      M.SubPointer.get_struct_tuple_field (|
                                                        γ,
                                                        "core::option::Option::Some",
                                                        0
                                                      |) in
                                                    let γ1_0 :=
                                                      M.SubPointer.get_tuple_field (| γ0_0, 0 |) in
                                                    let γ1_1 :=
                                                      M.SubPointer.get_tuple_field (| γ0_0, 1 |) in
                                                    let a :=
                                                      M.copy (|
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [ Ty.path "u8" ],
                                                        γ1_0
                                                      |) in
                                                    let b :=
                                                      M.copy (|
                                                        Ty.apply (Ty.path "&") [] [ Ty.path "u8" ],
                                                        γ1_1
                                                      |) in
                                                    M.match_operator (|
                                                      Ty.tuple [],
                                                      M.alloc (|
                                                        Ty.tuple [ Ty.path "u8"; Ty.path "bool" ],
                                                        M.call_closure (|
                                                          Ty.tuple [ Ty.path "u8"; Ty.path "bool" ],
                                                          M.get_associated_function (|
                                                            Ty.path "u8",
                                                            "carrying_add",
                                                            [],
                                                            []
                                                          |),
                                                          [
                                                            M.read (|
                                                              M.deref (| M.read (| a |) |)
                                                            |);
                                                            M.read (|
                                                              M.deref (| M.read (| b |) |)
                                                            |);
                                                            M.read (| carry |)
                                                          ]
                                                        |)
                                                      |),
                                                      [
                                                        fun γ =>
                                                          ltac:(M.monadic
                                                            (let γ0_0 :=
                                                              M.SubPointer.get_tuple_field (|
                                                                γ,
                                                                0
                                                              |) in
                                                            let γ0_1 :=
                                                              M.SubPointer.get_tuple_field (|
                                                                γ,
                                                                1
                                                              |) in
                                                            let v :=
                                                              M.copy (| Ty.path "u8", γ0_0 |) in
                                                            let c :=
                                                              M.copy (| Ty.path "bool", γ0_1 |) in
                                                            M.read (|
                                                              let~ _ : Ty.tuple [] :=
                                                                M.write (|
                                                                  M.deref (| M.read (| a |) |),
                                                                  M.read (| v |)
                                                                |) in
                                                              let~ _ : Ty.tuple [] :=
                                                                M.write (|
                                                                  carry,
                                                                  M.read (| c |)
                                                                |) in
                                                              M.alloc (|
                                                                Ty.tuple [],
                                                                Value.Tuple []
                                                              |)
                                                            |)))
                                                      ]
                                                    |)))
                                              ]
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                      |)
                                    |)))
                              ]
                            |)
                          |))
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ := carry in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.read (|
                                let~ _ : Ty.tuple [] :=
                                  M.write (|
                                    M.SubPointer.get_array_field (|
                                      M.SubPointer.get_struct_record_field (|
                                        M.deref (| M.read (| self |) |),
                                        "core::num::bignum::tests::Big8x3",
                                        "base"
                                      |),
                                      M.read (| sz |)
                                    |),
                                    Value.Integer IntegerKind.U8 1
                                  |) in
                                let~ _ : Ty.tuple [] :=
                                  let β := sz in
                                  M.write (|
                                    β,
                                    M.call_closure (|
                                      Ty.path "usize",
                                      BinOp.Wrap.add,
                                      [ M.read (| β |); Value.Integer IntegerKind.Usize 1 ]
                                    |)
                                  |) in
                                M.alloc (| Ty.tuple [], Value.Tuple [] |)
                              |)));
                          fun γ => ltac:(M.monadic (Value.Tuple []))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.write (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| self |) |),
                          "core::num::bignum::tests::Big8x3",
                          "size"
                        |),
                        M.read (| sz |)
                      |) in
                    M.alloc (|
                      Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                      M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                    |)
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_add : M.IsAssociatedFunction.C Self "add" add.
        Admitted.
        Global Typeclasses Opaque add.
        
        (*
                    pub fn add_small(&mut self, other: $ty) -> &mut $name {
                        let (v, mut carry) = self.base[0].carrying_add(other, false);
                        self.base[0] = v;
                        let mut i = 1;
                        while carry {
                            let (v, c) = self.base[i].carrying_add(0, carry);
                            self.base[i] = v;
                            carry = c;
                            i += 1;
                        }
                        if i > self.size {
                            self.size = i;
                        }
                        self
                    }
        *)
        Definition add_small (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other := M.alloc (| Ty.path "u8", other |) in
              M.borrow (|
                Pointer.Kind.MutRef,
                M.deref (|
                  M.match_operator (|
                    Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                    M.alloc (|
                      Ty.tuple [ Ty.path "u8"; Ty.path "bool" ],
                      M.call_closure (|
                        Ty.tuple [ Ty.path "u8"; Ty.path "bool" ],
                        M.get_associated_function (| Ty.path "u8", "carrying_add", [], [] |),
                        [
                          M.read (|
                            M.SubPointer.get_array_field (|
                              M.SubPointer.get_struct_record_field (|
                                M.deref (| M.read (| self |) |),
                                "core::num::bignum::tests::Big8x3",
                                "base"
                              |),
                              Value.Integer IntegerKind.Usize 0
                            |)
                          |);
                          M.read (| other |);
                          Value.Bool false
                        ]
                      |)
                    |),
                    [
                      fun γ =>
                        ltac:(M.monadic
                          (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                          let γ0_1 := M.SubPointer.get_tuple_field (| γ, 1 |) in
                          let v := M.copy (| Ty.path "u8", γ0_0 |) in
                          let~ carry : Ty.path "bool" := M.read (| γ0_1 |) in
                          M.read (|
                            let~ _ : Ty.tuple [] :=
                              M.write (|
                                M.SubPointer.get_array_field (|
                                  M.SubPointer.get_struct_record_field (|
                                    M.deref (| M.read (| self |) |),
                                    "core::num::bignum::tests::Big8x3",
                                    "base"
                                  |),
                                  Value.Integer IntegerKind.Usize 0
                                |),
                                M.read (| v |)
                              |) in
                            let~ i : Ty.path "usize" := Value.Integer IntegerKind.Usize 1 in
                            let~ _ : Ty.tuple [] :=
                              M.read (|
                                M.loop (|
                                  Ty.tuple [],
                                  ltac:(M.monadic
                                    (M.alloc (|
                                      Ty.tuple [],
                                      M.match_operator (|
                                        Ty.tuple [],
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                        [
                                          fun γ =>
                                            ltac:(M.monadic
                                              (let γ := carry in
                                              let _ :=
                                                is_constant_or_break_match (|
                                                  M.read (| γ |),
                                                  Value.Bool true
                                                |) in
                                              M.match_operator (|
                                                Ty.tuple [],
                                                M.alloc (|
                                                  Ty.tuple [ Ty.path "u8"; Ty.path "bool" ],
                                                  M.call_closure (|
                                                    Ty.tuple [ Ty.path "u8"; Ty.path "bool" ],
                                                    M.get_associated_function (|
                                                      Ty.path "u8",
                                                      "carrying_add",
                                                      [],
                                                      []
                                                    |),
                                                    [
                                                      M.read (|
                                                        M.SubPointer.get_array_field (|
                                                          M.SubPointer.get_struct_record_field (|
                                                            M.deref (| M.read (| self |) |),
                                                            "core::num::bignum::tests::Big8x3",
                                                            "base"
                                                          |),
                                                          M.read (| i |)
                                                        |)
                                                      |);
                                                      Value.Integer IntegerKind.U8 0;
                                                      M.read (| carry |)
                                                    ]
                                                  |)
                                                |),
                                                [
                                                  fun γ =>
                                                    ltac:(M.monadic
                                                      (let γ0_0 :=
                                                        M.SubPointer.get_tuple_field (| γ, 0 |) in
                                                      let γ0_1 :=
                                                        M.SubPointer.get_tuple_field (| γ, 1 |) in
                                                      let v := M.copy (| Ty.path "u8", γ0_0 |) in
                                                      let c := M.copy (| Ty.path "bool", γ0_1 |) in
                                                      M.read (|
                                                        let~ _ : Ty.tuple [] :=
                                                          M.write (|
                                                            M.SubPointer.get_array_field (|
                                                              M.SubPointer.get_struct_record_field (|
                                                                M.deref (| M.read (| self |) |),
                                                                "core::num::bignum::tests::Big8x3",
                                                                "base"
                                                              |),
                                                              M.read (| i |)
                                                            |),
                                                            M.read (| v |)
                                                          |) in
                                                        let~ _ : Ty.tuple [] :=
                                                          M.write (| carry, M.read (| c |) |) in
                                                        let~ _ : Ty.tuple [] :=
                                                          let β := i in
                                                          M.write (|
                                                            β,
                                                            M.call_closure (|
                                                              Ty.path "usize",
                                                              BinOp.Wrap.add,
                                                              [
                                                                M.read (| β |);
                                                                Value.Integer IntegerKind.Usize 1
                                                              ]
                                                            |)
                                                          |) in
                                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                      |)))
                                                ]
                                              |)));
                                          fun γ =>
                                            ltac:(M.monadic
                                              (M.never_to_any (|
                                                M.read (|
                                                  let~ _ : Ty.tuple [] :=
                                                    M.never_to_any (|
                                                      M.read (| M.break (||) |)
                                                    |) in
                                                  M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                |)
                                              |)))
                                        ]
                                      |)
                                    |)))
                                |)
                              |) in
                            let~ _ : Ty.tuple [] :=
                              M.match_operator (|
                                Ty.tuple [],
                                M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                [
                                  fun γ =>
                                    ltac:(M.monadic
                                      (let γ :=
                                        M.alloc (|
                                          Ty.path "bool",
                                          M.call_closure (|
                                            Ty.path "bool",
                                            BinOp.gt,
                                            [
                                              M.read (| i |);
                                              M.read (|
                                                M.SubPointer.get_struct_record_field (|
                                                  M.deref (| M.read (| self |) |),
                                                  "core::num::bignum::tests::Big8x3",
                                                  "size"
                                                |)
                                              |)
                                            ]
                                          |)
                                        |) in
                                      let _ :=
                                        is_constant_or_break_match (|
                                          M.read (| γ |),
                                          Value.Bool true
                                        |) in
                                      M.read (|
                                        let~ _ : Ty.tuple [] :=
                                          M.write (|
                                            M.SubPointer.get_struct_record_field (|
                                              M.deref (| M.read (| self |) |),
                                              "core::num::bignum::tests::Big8x3",
                                              "size"
                                            |),
                                            M.read (| i |)
                                          |) in
                                        M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                      |)));
                                  fun γ => ltac:(M.monadic (Value.Tuple []))
                                ]
                              |) in
                            M.alloc (|
                              Ty.apply
                                (Ty.path "&mut")
                                []
                                [ Ty.path "core::num::bignum::tests::Big8x3" ],
                              M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                            |)
                          |)))
                    ]
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_add_small :
          M.IsAssociatedFunction.C Self "add_small" add_small.
        Admitted.
        Global Typeclasses Opaque add_small.
        
        (*
                    pub fn sub<'a>(&'a mut self, other: &$name) -> &'a mut $name {
                        use crate::{cmp, iter};
        
                        let sz = cmp::max(self.size, other.size);
                        let mut noborrow = true;
                        for (a, b) in iter::zip(&mut self.base[..sz], &other.base[..sz]) {
                            let (v, c) = ( *a).carrying_add(!*b, noborrow);
                            *a = v;
                            noborrow = c;
                        }
                        assert!(noborrow);
                        self.size = sz;
                        self
                    }
        *)
        Definition sub (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  other
                |) in
              M.borrow (|
                Pointer.Kind.MutRef,
                M.deref (|
                  M.read (|
                    let~ sz : Ty.path "usize" :=
                      M.call_closure (|
                        Ty.path "usize",
                        M.get_function (| "core::cmp::max", [], [ Ty.path "usize" ] |),
                        [
                          M.read (|
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| self |) |),
                              "core::num::bignum::tests::Big8x3",
                              "size"
                            |)
                          |);
                          M.read (|
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| other |) |),
                              "core::num::bignum::tests::Big8x3",
                              "size"
                            |)
                          |)
                        ]
                      |) in
                    let~ noborrow : Ty.path "bool" := Value.Bool true in
                    let~ _ : Ty.tuple [] :=
                      M.read (|
                        M.use
                          (M.alloc (|
                            Ty.tuple [],
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::zip::Zip")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::slice::iter::IterMut")
                                      []
                                      [ Ty.path "u8" ];
                                    Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ]
                                  ],
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::zip::Zip")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::slice::iter::IterMut")
                                        []
                                        [ Ty.path "u8" ];
                                      Ty.apply
                                        (Ty.path "core::slice::iter::Iter")
                                        []
                                        [ Ty.path "u8" ]
                                    ],
                                  M.get_trait_method (|
                                    "core::iter::traits::collect::IntoIterator",
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::zip::Zip")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::slice::iter::IterMut")
                                          []
                                          [ Ty.path "u8" ];
                                        Ty.apply
                                          (Ty.path "core::slice::iter::Iter")
                                          []
                                          [ Ty.path "u8" ]
                                      ],
                                    [],
                                    [],
                                    "into_iter",
                                    [],
                                    []
                                  |),
                                  [
                                    M.call_closure (|
                                      Ty.apply
                                        (Ty.path "core::iter::adapters::zip::Zip")
                                        []
                                        [
                                          Ty.apply
                                            (Ty.path "core::slice::iter::IterMut")
                                            []
                                            [ Ty.path "u8" ];
                                          Ty.apply
                                            (Ty.path "core::slice::iter::Iter")
                                            []
                                            [ Ty.path "u8" ]
                                        ],
                                      M.get_function (|
                                        "core::iter::adapters::zip::zip",
                                        [],
                                        [
                                          Ty.apply
                                            (Ty.path "&mut")
                                            []
                                            [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ];
                                          Ty.apply
                                            (Ty.path "&")
                                            []
                                            [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ]
                                        ]
                                      |),
                                      [
                                        M.borrow (|
                                          Pointer.Kind.MutRef,
                                          M.deref (|
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "&mut")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                              M.get_trait_method (|
                                                "core::ops::index::IndexMut",
                                                Ty.apply
                                                  (Ty.path "array")
                                                  [ Value.Integer IntegerKind.Usize 3 ]
                                                  [ Ty.path "u8" ],
                                                [],
                                                [
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::RangeTo")
                                                    []
                                                    [ Ty.path "usize" ]
                                                ],
                                                "index_mut",
                                                [],
                                                []
                                              |),
                                              [
                                                M.borrow (|
                                                  Pointer.Kind.MutRef,
                                                  M.SubPointer.get_struct_record_field (|
                                                    M.deref (| M.read (| self |) |),
                                                    "core::num::bignum::tests::Big8x3",
                                                    "base"
                                                  |)
                                                |);
                                                Value.mkStructRecord
                                                  "core::ops::range::RangeTo"
                                                  []
                                                  [ Ty.path "usize" ]
                                                  [ ("end_", M.read (| sz |)) ]
                                              ]
                                            |)
                                          |)
                                        |);
                                        M.borrow (|
                                          Pointer.Kind.Ref,
                                          M.deref (|
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "&")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                              M.get_trait_method (|
                                                "core::ops::index::Index",
                                                Ty.apply
                                                  (Ty.path "array")
                                                  [ Value.Integer IntegerKind.Usize 3 ]
                                                  [ Ty.path "u8" ],
                                                [],
                                                [
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::RangeTo")
                                                    []
                                                    [ Ty.path "usize" ]
                                                ],
                                                "index",
                                                [],
                                                []
                                              |),
                                              [
                                                M.borrow (|
                                                  Pointer.Kind.Ref,
                                                  M.SubPointer.get_struct_record_field (|
                                                    M.deref (| M.read (| other |) |),
                                                    "core::num::bignum::tests::Big8x3",
                                                    "base"
                                                  |)
                                                |);
                                                Value.mkStructRecord
                                                  "core::ops::range::RangeTo"
                                                  []
                                                  [ Ty.path "usize" ]
                                                  [ ("end_", M.read (| sz |)) ]
                                              ]
                                            |)
                                          |)
                                        |)
                                      ]
                                    |)
                                  ]
                                |)
                              |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let~ iter :
                                        Ty.apply
                                          (Ty.path "core::iter::adapters::zip::Zip")
                                          []
                                          [
                                            Ty.apply
                                              (Ty.path "core::slice::iter::IterMut")
                                              []
                                              [ Ty.path "u8" ];
                                            Ty.apply
                                              (Ty.path "core::slice::iter::Iter")
                                              []
                                              [ Ty.path "u8" ]
                                          ] :=
                                      M.read (| γ |) in
                                    M.read (|
                                      M.loop (|
                                        Ty.tuple [],
                                        ltac:(M.monadic
                                          (let~ _ : Ty.tuple [] :=
                                            M.match_operator (|
                                              Ty.tuple [],
                                              M.alloc (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [
                                                    Ty.tuple
                                                      [
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [ Ty.path "u8" ];
                                                        Ty.apply (Ty.path "&") [] [ Ty.path "u8" ]
                                                      ]
                                                  ],
                                                M.call_closure (|
                                                  Ty.apply
                                                    (Ty.path "core::option::Option")
                                                    []
                                                    [
                                                      Ty.tuple
                                                        [
                                                          Ty.apply
                                                            (Ty.path "&mut")
                                                            []
                                                            [ Ty.path "u8" ];
                                                          Ty.apply (Ty.path "&") [] [ Ty.path "u8" ]
                                                        ]
                                                    ],
                                                  M.get_trait_method (|
                                                    "core::iter::traits::iterator::Iterator",
                                                    Ty.apply
                                                      (Ty.path "core::iter::adapters::zip::Zip")
                                                      []
                                                      [
                                                        Ty.apply
                                                          (Ty.path "core::slice::iter::IterMut")
                                                          []
                                                          [ Ty.path "u8" ];
                                                        Ty.apply
                                                          (Ty.path "core::slice::iter::Iter")
                                                          []
                                                          [ Ty.path "u8" ]
                                                      ],
                                                    [],
                                                    [],
                                                    "next",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    M.borrow (|
                                                      Pointer.Kind.MutRef,
                                                      M.deref (|
                                                        M.borrow (| Pointer.Kind.MutRef, iter |)
                                                      |)
                                                    |)
                                                  ]
                                                |)
                                              |),
                                              [
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let _ :=
                                                      M.is_struct_tuple (|
                                                        γ,
                                                        "core::option::Option::None"
                                                      |) in
                                                    M.never_to_any (|
                                                      M.read (| M.break (||) |)
                                                    |)));
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let γ0_0 :=
                                                      M.SubPointer.get_struct_tuple_field (|
                                                        γ,
                                                        "core::option::Option::Some",
                                                        0
                                                      |) in
                                                    let γ1_0 :=
                                                      M.SubPointer.get_tuple_field (| γ0_0, 0 |) in
                                                    let γ1_1 :=
                                                      M.SubPointer.get_tuple_field (| γ0_0, 1 |) in
                                                    let a :=
                                                      M.copy (|
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [ Ty.path "u8" ],
                                                        γ1_0
                                                      |) in
                                                    let b :=
                                                      M.copy (|
                                                        Ty.apply (Ty.path "&") [] [ Ty.path "u8" ],
                                                        γ1_1
                                                      |) in
                                                    M.match_operator (|
                                                      Ty.tuple [],
                                                      M.alloc (|
                                                        Ty.tuple [ Ty.path "u8"; Ty.path "bool" ],
                                                        M.call_closure (|
                                                          Ty.tuple [ Ty.path "u8"; Ty.path "bool" ],
                                                          M.get_associated_function (|
                                                            Ty.path "u8",
                                                            "carrying_add",
                                                            [],
                                                            []
                                                          |),
                                                          [
                                                            M.read (|
                                                              M.deref (| M.read (| a |) |)
                                                            |);
                                                            M.call_closure (|
                                                              Ty.path "u8",
                                                              UnOp.not,
                                                              [
                                                                M.read (|
                                                                  M.deref (| M.read (| b |) |)
                                                                |)
                                                              ]
                                                            |);
                                                            M.read (| noborrow |)
                                                          ]
                                                        |)
                                                      |),
                                                      [
                                                        fun γ =>
                                                          ltac:(M.monadic
                                                            (let γ0_0 :=
                                                              M.SubPointer.get_tuple_field (|
                                                                γ,
                                                                0
                                                              |) in
                                                            let γ0_1 :=
                                                              M.SubPointer.get_tuple_field (|
                                                                γ,
                                                                1
                                                              |) in
                                                            let v :=
                                                              M.copy (| Ty.path "u8", γ0_0 |) in
                                                            let c :=
                                                              M.copy (| Ty.path "bool", γ0_1 |) in
                                                            M.read (|
                                                              let~ _ : Ty.tuple [] :=
                                                                M.write (|
                                                                  M.deref (| M.read (| a |) |),
                                                                  M.read (| v |)
                                                                |) in
                                                              let~ _ : Ty.tuple [] :=
                                                                M.write (|
                                                                  noborrow,
                                                                  M.read (| c |)
                                                                |) in
                                                              M.alloc (|
                                                                Ty.tuple [],
                                                                Value.Tuple []
                                                              |)
                                                            |)))
                                                      ]
                                                    |)))
                                              ]
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                      |)
                                    |)))
                              ]
                            |)
                          |))
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ :=
                                M.alloc (|
                                  Ty.path "bool",
                                  M.call_closure (|
                                    Ty.path "bool",
                                    UnOp.not,
                                    [ M.read (| noborrow |) ]
                                  |)
                                |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.never_to_any (|
                                M.call_closure (|
                                  Ty.path "never",
                                  M.get_function (| "core::panicking::panic", [], [] |),
                                  [ mk_str (| "assertion failed: noborrow" |) ]
                                |)
                              |)));
                          fun γ => ltac:(M.monadic (Value.Tuple []))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.write (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| self |) |),
                          "core::num::bignum::tests::Big8x3",
                          "size"
                        |),
                        M.read (| sz |)
                      |) in
                    M.alloc (|
                      Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                      M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                    |)
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_sub : M.IsAssociatedFunction.C Self "sub" sub.
        Admitted.
        Global Typeclasses Opaque sub.
        
        (*
                    pub fn mul_small(&mut self, other: $ty) -> &mut $name {
                        let mut sz = self.size;
                        let mut carry = 0;
                        for a in &mut self.base[..sz] {
                            let (v, c) = ( *a).carrying_mul(other, carry);
                            *a = v;
                            carry = c;
                        }
                        if carry > 0 {
                            self.base[sz] = carry;
                            sz += 1;
                        }
                        self.size = sz;
                        self
                    }
        *)
        Definition mul_small (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other := M.alloc (| Ty.path "u8", other |) in
              M.borrow (|
                Pointer.Kind.MutRef,
                M.deref (|
                  M.read (|
                    let~ sz : Ty.path "usize" :=
                      M.read (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| self |) |),
                          "core::num::bignum::tests::Big8x3",
                          "size"
                        |)
                      |) in
                    let~ carry : Ty.path "u8" := Value.Integer IntegerKind.U8 0 in
                    let~ _ : Ty.tuple [] :=
                      M.read (|
                        M.use
                          (M.alloc (|
                            Ty.tuple [],
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (|
                                Ty.apply (Ty.path "core::slice::iter::IterMut") [] [ Ty.path "u8" ],
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "core::slice::iter::IterMut")
                                    []
                                    [ Ty.path "u8" ],
                                  M.get_trait_method (|
                                    "core::iter::traits::collect::IntoIterator",
                                    Ty.apply
                                      (Ty.path "&mut")
                                      []
                                      [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                    [],
                                    [],
                                    "into_iter",
                                    [],
                                    []
                                  |),
                                  [
                                    M.borrow (|
                                      Pointer.Kind.MutRef,
                                      M.deref (|
                                        M.call_closure (|
                                          Ty.apply
                                            (Ty.path "&mut")
                                            []
                                            [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                          M.get_trait_method (|
                                            "core::ops::index::IndexMut",
                                            Ty.apply
                                              (Ty.path "array")
                                              [ Value.Integer IntegerKind.Usize 3 ]
                                              [ Ty.path "u8" ],
                                            [],
                                            [
                                              Ty.apply
                                                (Ty.path "core::ops::range::RangeTo")
                                                []
                                                [ Ty.path "usize" ]
                                            ],
                                            "index_mut",
                                            [],
                                            []
                                          |),
                                          [
                                            M.borrow (|
                                              Pointer.Kind.MutRef,
                                              M.SubPointer.get_struct_record_field (|
                                                M.deref (| M.read (| self |) |),
                                                "core::num::bignum::tests::Big8x3",
                                                "base"
                                              |)
                                            |);
                                            Value.mkStructRecord
                                              "core::ops::range::RangeTo"
                                              []
                                              [ Ty.path "usize" ]
                                              [ ("end_", M.read (| sz |)) ]
                                          ]
                                        |)
                                      |)
                                    |)
                                  ]
                                |)
                              |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let~ iter :
                                        Ty.apply
                                          (Ty.path "core::slice::iter::IterMut")
                                          []
                                          [ Ty.path "u8" ] :=
                                      M.read (| γ |) in
                                    M.read (|
                                      M.loop (|
                                        Ty.tuple [],
                                        ltac:(M.monadic
                                          (let~ _ : Ty.tuple [] :=
                                            M.match_operator (|
                                              Ty.tuple [],
                                              M.alloc (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [ Ty.apply (Ty.path "&mut") [] [ Ty.path "u8" ] ],
                                                M.call_closure (|
                                                  Ty.apply
                                                    (Ty.path "core::option::Option")
                                                    []
                                                    [ Ty.apply (Ty.path "&mut") [] [ Ty.path "u8" ]
                                                    ],
                                                  M.get_trait_method (|
                                                    "core::iter::traits::iterator::Iterator",
                                                    Ty.apply
                                                      (Ty.path "core::slice::iter::IterMut")
                                                      []
                                                      [ Ty.path "u8" ],
                                                    [],
                                                    [],
                                                    "next",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    M.borrow (|
                                                      Pointer.Kind.MutRef,
                                                      M.deref (|
                                                        M.borrow (| Pointer.Kind.MutRef, iter |)
                                                      |)
                                                    |)
                                                  ]
                                                |)
                                              |),
                                              [
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let _ :=
                                                      M.is_struct_tuple (|
                                                        γ,
                                                        "core::option::Option::None"
                                                      |) in
                                                    M.never_to_any (|
                                                      M.read (| M.break (||) |)
                                                    |)));
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let γ0_0 :=
                                                      M.SubPointer.get_struct_tuple_field (|
                                                        γ,
                                                        "core::option::Option::Some",
                                                        0
                                                      |) in
                                                    let a :=
                                                      M.copy (|
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [ Ty.path "u8" ],
                                                        γ0_0
                                                      |) in
                                                    M.match_operator (|
                                                      Ty.tuple [],
                                                      M.alloc (|
                                                        Ty.tuple [ Ty.path "u8"; Ty.path "u8" ],
                                                        M.call_closure (|
                                                          Ty.tuple [ Ty.path "u8"; Ty.path "u8" ],
                                                          M.get_associated_function (|
                                                            Ty.path "u8",
                                                            "carrying_mul",
                                                            [],
                                                            []
                                                          |),
                                                          [
                                                            M.read (|
                                                              M.deref (| M.read (| a |) |)
                                                            |);
                                                            M.read (| other |);
                                                            M.read (| carry |)
                                                          ]
                                                        |)
                                                      |),
                                                      [
                                                        fun γ =>
                                                          ltac:(M.monadic
                                                            (let γ0_0 :=
                                                              M.SubPointer.get_tuple_field (|
                                                                γ,
                                                                0
                                                              |) in
                                                            let γ0_1 :=
                                                              M.SubPointer.get_tuple_field (|
                                                                γ,
                                                                1
                                                              |) in
                                                            let v :=
                                                              M.copy (| Ty.path "u8", γ0_0 |) in
                                                            let c :=
                                                              M.copy (| Ty.path "u8", γ0_1 |) in
                                                            M.read (|
                                                              let~ _ : Ty.tuple [] :=
                                                                M.write (|
                                                                  M.deref (| M.read (| a |) |),
                                                                  M.read (| v |)
                                                                |) in
                                                              let~ _ : Ty.tuple [] :=
                                                                M.write (|
                                                                  carry,
                                                                  M.read (| c |)
                                                                |) in
                                                              M.alloc (|
                                                                Ty.tuple [],
                                                                Value.Tuple []
                                                              |)
                                                            |)))
                                                      ]
                                                    |)))
                                              ]
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                      |)
                                    |)))
                              ]
                            |)
                          |))
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ :=
                                M.alloc (|
                                  Ty.path "bool",
                                  M.call_closure (|
                                    Ty.path "bool",
                                    BinOp.gt,
                                    [ M.read (| carry |); Value.Integer IntegerKind.U8 0 ]
                                  |)
                                |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.read (|
                                let~ _ : Ty.tuple [] :=
                                  M.write (|
                                    M.SubPointer.get_array_field (|
                                      M.SubPointer.get_struct_record_field (|
                                        M.deref (| M.read (| self |) |),
                                        "core::num::bignum::tests::Big8x3",
                                        "base"
                                      |),
                                      M.read (| sz |)
                                    |),
                                    M.read (| carry |)
                                  |) in
                                let~ _ : Ty.tuple [] :=
                                  let β := sz in
                                  M.write (|
                                    β,
                                    M.call_closure (|
                                      Ty.path "usize",
                                      BinOp.Wrap.add,
                                      [ M.read (| β |); Value.Integer IntegerKind.Usize 1 ]
                                    |)
                                  |) in
                                M.alloc (| Ty.tuple [], Value.Tuple [] |)
                              |)));
                          fun γ => ltac:(M.monadic (Value.Tuple []))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.write (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| self |) |),
                          "core::num::bignum::tests::Big8x3",
                          "size"
                        |),
                        M.read (| sz |)
                      |) in
                    M.alloc (|
                      Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                      M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                    |)
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_mul_small :
          M.IsAssociatedFunction.C Self "mul_small" mul_small.
        Admitted.
        Global Typeclasses Opaque mul_small.
        
        (*
                    pub fn mul_pow2(&mut self, bits: usize) -> &mut $name {
                        let digitbits = <$ty>::BITS as usize;
                        let digits = bits / digitbits;
                        let bits = bits % digitbits;
        
                        assert!(digits < $n);
                        debug_assert!(self.base[$n - digits..].iter().all(|&v| v == 0));
                        debug_assert!(bits == 0 || (self.base[$n - digits - 1] >> (digitbits - bits)) == 0);
        
                        // shift by `digits * digitbits` bits
                        for i in (0..self.size).rev() {
                            self.base[i + digits] = self.base[i];
                        }
                        for i in 0..digits {
                            self.base[i] = 0;
                        }
        
                        // shift by `bits` bits
                        let mut sz = self.size + digits;
                        if bits > 0 {
                            let last = sz;
                            let overflow = self.base[last - 1] >> (digitbits - bits);
                            if overflow > 0 {
                                self.base[last] = overflow;
                                sz += 1;
                            }
                            for i in (digits + 1..last).rev() {
                                self.base[i] =
                                    (self.base[i] << bits) | (self.base[i - 1] >> (digitbits - bits));
                            }
                            self.base[digits] <<= bits;
                            // self.base[..digits] is zero, no need to shift
                        }
        
                        self.size = sz;
                        self
                    }
        *)
        Definition mul_pow2 (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; bits ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let bits := M.alloc (| Ty.path "usize", bits |) in
              M.borrow (|
                Pointer.Kind.MutRef,
                M.deref (|
                  M.read (|
                    let~ digitbits : Ty.path "usize" :=
                      M.cast
                        (Ty.path "usize")
                        (M.read (|
                          get_associated_constant (| Ty.path "u8", "BITS", Ty.path "u32" |)
                        |)) in
                    let~ digits : Ty.path "usize" :=
                      M.call_closure (|
                        Ty.path "usize",
                        BinOp.Wrap.div,
                        [ M.read (| bits |); M.read (| digitbits |) ]
                      |) in
                    let~ bits : Ty.path "usize" :=
                      M.call_closure (|
                        Ty.path "usize",
                        BinOp.Wrap.rem,
                        [ M.read (| bits |); M.read (| digitbits |) ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ :=
                                M.alloc (|
                                  Ty.path "bool",
                                  M.call_closure (|
                                    Ty.path "bool",
                                    UnOp.not,
                                    [
                                      M.call_closure (|
                                        Ty.path "bool",
                                        BinOp.lt,
                                        [ M.read (| digits |); Value.Integer IntegerKind.Usize 3 ]
                                      |)
                                    ]
                                  |)
                                |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.never_to_any (|
                                M.call_closure (|
                                  Ty.path "never",
                                  M.get_function (| "core::panicking::panic", [], [] |),
                                  [ mk_str (| "assertion failed: digits < 3" |) ]
                                |)
                              |)));
                          fun γ => ltac:(M.monadic (Value.Tuple []))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ := M.alloc (| Ty.path "bool", Value.Bool true |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.read (|
                                let~ _ : Ty.tuple [] :=
                                  M.match_operator (|
                                    Ty.tuple [],
                                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                    [
                                      fun γ =>
                                        ltac:(M.monadic
                                          (let γ :=
                                            M.alloc (|
                                              Ty.path "bool",
                                              M.call_closure (|
                                                Ty.path "bool",
                                                UnOp.not,
                                                [
                                                  M.call_closure (|
                                                    Ty.path "bool",
                                                    M.get_trait_method (|
                                                      "core::iter::traits::iterator::Iterator",
                                                      Ty.apply
                                                        (Ty.path "core::slice::iter::Iter")
                                                        []
                                                        [ Ty.path "u8" ],
                                                      [],
                                                      [],
                                                      "all",
                                                      [],
                                                      [
                                                        Ty.function
                                                          [
                                                            Ty.apply
                                                              (Ty.path "&")
                                                              []
                                                              [ Ty.path "u8" ]
                                                          ]
                                                          (Ty.path "bool")
                                                      ]
                                                    |),
                                                    [
                                                      M.borrow (|
                                                        Pointer.Kind.MutRef,
                                                        M.alloc (|
                                                          Ty.apply
                                                            (Ty.path "core::slice::iter::Iter")
                                                            []
                                                            [ Ty.path "u8" ],
                                                          M.call_closure (|
                                                            Ty.apply
                                                              (Ty.path "core::slice::iter::Iter")
                                                              []
                                                              [ Ty.path "u8" ],
                                                            M.get_associated_function (|
                                                              Ty.apply
                                                                (Ty.path "slice")
                                                                []
                                                                [ Ty.path "u8" ],
                                                              "iter",
                                                              [],
                                                              []
                                                            |),
                                                            [
                                                              M.borrow (|
                                                                Pointer.Kind.Ref,
                                                                M.deref (|
                                                                  M.call_closure (|
                                                                    Ty.apply
                                                                      (Ty.path "&")
                                                                      []
                                                                      [
                                                                        Ty.apply
                                                                          (Ty.path "slice")
                                                                          []
                                                                          [ Ty.path "u8" ]
                                                                      ],
                                                                    M.get_trait_method (|
                                                                      "core::ops::index::Index",
                                                                      Ty.apply
                                                                        (Ty.path "array")
                                                                        [
                                                                          Value.Integer
                                                                            IntegerKind.Usize
                                                                            3
                                                                        ]
                                                                        [ Ty.path "u8" ],
                                                                      [],
                                                                      [
                                                                        Ty.apply
                                                                          (Ty.path
                                                                            "core::ops::range::RangeFrom")
                                                                          []
                                                                          [ Ty.path "usize" ]
                                                                      ],
                                                                      "index",
                                                                      [],
                                                                      []
                                                                    |),
                                                                    [
                                                                      M.borrow (|
                                                                        Pointer.Kind.Ref,
                                                                        M.SubPointer.get_struct_record_field (|
                                                                          M.deref (|
                                                                            M.read (| self |)
                                                                          |),
                                                                          "core::num::bignum::tests::Big8x3",
                                                                          "base"
                                                                        |)
                                                                      |);
                                                                      Value.mkStructRecord
                                                                        "core::ops::range::RangeFrom"
                                                                        []
                                                                        [ Ty.path "usize" ]
                                                                        [
                                                                          ("start",
                                                                            M.call_closure (|
                                                                              Ty.path "usize",
                                                                              BinOp.Wrap.sub,
                                                                              [
                                                                                Value.Integer
                                                                                  IntegerKind.Usize
                                                                                  3;
                                                                                M.read (| digits |)
                                                                              ]
                                                                            |))
                                                                        ]
                                                                    ]
                                                                  |)
                                                                |)
                                                              |)
                                                            ]
                                                          |)
                                                        |)
                                                      |);
                                                      M.closure
                                                        (fun γ =>
                                                          ltac:(M.monadic
                                                            match γ with
                                                            | [ α0 ] =>
                                                              ltac:(M.monadic
                                                                (M.match_operator (|
                                                                  Ty.path "bool",
                                                                  M.alloc (|
                                                                    Ty.apply
                                                                      (Ty.path "&")
                                                                      []
                                                                      [ Ty.path "u8" ],
                                                                    α0
                                                                  |),
                                                                  [
                                                                    fun γ =>
                                                                      ltac:(M.monadic
                                                                        (let γ :=
                                                                          M.deref (|
                                                                            M.read (| γ |)
                                                                          |) in
                                                                        let v :=
                                                                          M.copy (|
                                                                            Ty.path "u8",
                                                                            γ
                                                                          |) in
                                                                        M.call_closure (|
                                                                          Ty.path "bool",
                                                                          BinOp.eq,
                                                                          [
                                                                            M.read (| v |);
                                                                            Value.Integer
                                                                              IntegerKind.U8
                                                                              0
                                                                          ]
                                                                        |)))
                                                                  ]
                                                                |)))
                                                            | _ =>
                                                              M.impossible
                                                                "wrong number of arguments"
                                                            end))
                                                    ]
                                                  |)
                                                ]
                                              |)
                                            |) in
                                          let _ :=
                                            is_constant_or_break_match (|
                                              M.read (| γ |),
                                              Value.Bool true
                                            |) in
                                          M.never_to_any (|
                                            M.call_closure (|
                                              Ty.path "never",
                                              M.get_function (| "core::panicking::panic", [], [] |),
                                              [
                                                mk_str (|
                                                  "assertion failed: self.base[3 - digits..].iter().all(|&v| v == 0)"
                                                |)
                                              ]
                                            |)
                                          |)));
                                      fun γ => ltac:(M.monadic (Value.Tuple []))
                                    ]
                                  |) in
                                M.alloc (| Ty.tuple [], Value.Tuple [] |)
                              |)));
                          fun γ => ltac:(M.monadic (Value.Tuple []))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ := M.alloc (| Ty.path "bool", Value.Bool true |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.read (|
                                let~ _ : Ty.tuple [] :=
                                  M.match_operator (|
                                    Ty.tuple [],
                                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                    [
                                      fun γ =>
                                        ltac:(M.monadic
                                          (let γ :=
                                            M.alloc (|
                                              Ty.path "bool",
                                              M.call_closure (|
                                                Ty.path "bool",
                                                UnOp.not,
                                                [
                                                  LogicalOp.or (|
                                                    M.call_closure (|
                                                      Ty.path "bool",
                                                      BinOp.eq,
                                                      [
                                                        M.read (| bits |);
                                                        Value.Integer IntegerKind.Usize 0
                                                      ]
                                                    |),
                                                    ltac:(M.monadic
                                                      (M.call_closure (|
                                                        Ty.path "bool",
                                                        BinOp.eq,
                                                        [
                                                          M.call_closure (|
                                                            Ty.path "u8",
                                                            BinOp.Wrap.shr,
                                                            [
                                                              M.read (|
                                                                M.SubPointer.get_array_field (|
                                                                  M.SubPointer.get_struct_record_field (|
                                                                    M.deref (| M.read (| self |) |),
                                                                    "core::num::bignum::tests::Big8x3",
                                                                    "base"
                                                                  |),
                                                                  M.call_closure (|
                                                                    Ty.path "usize",
                                                                    BinOp.Wrap.sub,
                                                                    [
                                                                      M.call_closure (|
                                                                        Ty.path "usize",
                                                                        BinOp.Wrap.sub,
                                                                        [
                                                                          Value.Integer
                                                                            IntegerKind.Usize
                                                                            3;
                                                                          M.read (| digits |)
                                                                        ]
                                                                      |);
                                                                      Value.Integer
                                                                        IntegerKind.Usize
                                                                        1
                                                                    ]
                                                                  |)
                                                                |)
                                                              |);
                                                              M.call_closure (|
                                                                Ty.path "usize",
                                                                BinOp.Wrap.sub,
                                                                [
                                                                  M.read (| digitbits |);
                                                                  M.read (| bits |)
                                                                ]
                                                              |)
                                                            ]
                                                          |);
                                                          Value.Integer IntegerKind.U8 0
                                                        ]
                                                      |)))
                                                  |)
                                                ]
                                              |)
                                            |) in
                                          let _ :=
                                            is_constant_or_break_match (|
                                              M.read (| γ |),
                                              Value.Bool true
                                            |) in
                                          M.never_to_any (|
                                            M.call_closure (|
                                              Ty.path "never",
                                              M.get_function (| "core::panicking::panic", [], [] |),
                                              [
                                                mk_str (|
                                                  "assertion failed: bits == 0 || (self.base[3 - digits - 1] >> (digitbits - bits)) == 0"
                                                |)
                                              ]
                                            |)
                                          |)));
                                      fun γ => ltac:(M.monadic (Value.Tuple []))
                                    ]
                                  |) in
                                M.alloc (| Ty.tuple [], Value.Tuple [] |)
                              |)));
                          fun γ => ltac:(M.monadic (Value.Tuple []))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.read (|
                        M.use
                          (M.alloc (|
                            Ty.tuple [],
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::rev::Rev")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::ops::range::Range")
                                      []
                                      [ Ty.path "usize" ]
                                  ],
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::rev::Rev")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::ops::range::Range")
                                        []
                                        [ Ty.path "usize" ]
                                    ],
                                  M.get_trait_method (|
                                    "core::iter::traits::collect::IntoIterator",
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::rev::Rev")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::ops::range::Range")
                                          []
                                          [ Ty.path "usize" ]
                                      ],
                                    [],
                                    [],
                                    "into_iter",
                                    [],
                                    []
                                  |),
                                  [
                                    M.call_closure (|
                                      Ty.apply
                                        (Ty.path "core::iter::adapters::rev::Rev")
                                        []
                                        [
                                          Ty.apply
                                            (Ty.path "core::ops::range::Range")
                                            []
                                            [ Ty.path "usize" ]
                                        ],
                                      M.get_trait_method (|
                                        "core::iter::traits::iterator::Iterator",
                                        Ty.apply
                                          (Ty.path "core::ops::range::Range")
                                          []
                                          [ Ty.path "usize" ],
                                        [],
                                        [],
                                        "rev",
                                        [],
                                        []
                                      |),
                                      [
                                        Value.mkStructRecord
                                          "core::ops::range::Range"
                                          []
                                          [ Ty.path "usize" ]
                                          [
                                            ("start", Value.Integer IntegerKind.Usize 0);
                                            ("end_",
                                              M.read (|
                                                M.SubPointer.get_struct_record_field (|
                                                  M.deref (| M.read (| self |) |),
                                                  "core::num::bignum::tests::Big8x3",
                                                  "size"
                                                |)
                                              |))
                                          ]
                                      ]
                                    |)
                                  ]
                                |)
                              |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let~ iter :
                                        Ty.apply
                                          (Ty.path "core::iter::adapters::rev::Rev")
                                          []
                                          [
                                            Ty.apply
                                              (Ty.path "core::ops::range::Range")
                                              []
                                              [ Ty.path "usize" ]
                                          ] :=
                                      M.read (| γ |) in
                                    M.read (|
                                      M.loop (|
                                        Ty.tuple [],
                                        ltac:(M.monadic
                                          (let~ _ : Ty.tuple [] :=
                                            M.match_operator (|
                                              Ty.tuple [],
                                              M.alloc (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [ Ty.path "usize" ],
                                                M.call_closure (|
                                                  Ty.apply
                                                    (Ty.path "core::option::Option")
                                                    []
                                                    [ Ty.path "usize" ],
                                                  M.get_trait_method (|
                                                    "core::iter::traits::iterator::Iterator",
                                                    Ty.apply
                                                      (Ty.path "core::iter::adapters::rev::Rev")
                                                      []
                                                      [
                                                        Ty.apply
                                                          (Ty.path "core::ops::range::Range")
                                                          []
                                                          [ Ty.path "usize" ]
                                                      ],
                                                    [],
                                                    [],
                                                    "next",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    M.borrow (|
                                                      Pointer.Kind.MutRef,
                                                      M.deref (|
                                                        M.borrow (| Pointer.Kind.MutRef, iter |)
                                                      |)
                                                    |)
                                                  ]
                                                |)
                                              |),
                                              [
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let _ :=
                                                      M.is_struct_tuple (|
                                                        γ,
                                                        "core::option::Option::None"
                                                      |) in
                                                    M.never_to_any (|
                                                      M.read (| M.break (||) |)
                                                    |)));
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let γ0_0 :=
                                                      M.SubPointer.get_struct_tuple_field (|
                                                        γ,
                                                        "core::option::Option::Some",
                                                        0
                                                      |) in
                                                    let i := M.copy (| Ty.path "usize", γ0_0 |) in
                                                    M.read (|
                                                      let~ _ : Ty.tuple [] :=
                                                        M.write (|
                                                          M.SubPointer.get_array_field (|
                                                            M.SubPointer.get_struct_record_field (|
                                                              M.deref (| M.read (| self |) |),
                                                              "core::num::bignum::tests::Big8x3",
                                                              "base"
                                                            |),
                                                            M.call_closure (|
                                                              Ty.path "usize",
                                                              BinOp.Wrap.add,
                                                              [ M.read (| i |); M.read (| digits |)
                                                              ]
                                                            |)
                                                          |),
                                                          M.read (|
                                                            M.SubPointer.get_array_field (|
                                                              M.SubPointer.get_struct_record_field (|
                                                                M.deref (| M.read (| self |) |),
                                                                "core::num::bignum::tests::Big8x3",
                                                                "base"
                                                              |),
                                                              M.read (| i |)
                                                            |)
                                                          |)
                                                        |) in
                                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                    |)))
                                              ]
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                      |)
                                    |)))
                              ]
                            |)
                          |))
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.read (|
                        M.use
                          (M.alloc (|
                            Ty.tuple [],
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (|
                                Ty.apply (Ty.path "core::ops::range::Range") [] [ Ty.path "usize" ],
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "core::ops::range::Range")
                                    []
                                    [ Ty.path "usize" ],
                                  M.get_trait_method (|
                                    "core::iter::traits::collect::IntoIterator",
                                    Ty.apply
                                      (Ty.path "core::ops::range::Range")
                                      []
                                      [ Ty.path "usize" ],
                                    [],
                                    [],
                                    "into_iter",
                                    [],
                                    []
                                  |),
                                  [
                                    Value.mkStructRecord
                                      "core::ops::range::Range"
                                      []
                                      [ Ty.path "usize" ]
                                      [
                                        ("start", Value.Integer IntegerKind.Usize 0);
                                        ("end_", M.read (| digits |))
                                      ]
                                  ]
                                |)
                              |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let~ iter :
                                        Ty.apply
                                          (Ty.path "core::ops::range::Range")
                                          []
                                          [ Ty.path "usize" ] :=
                                      M.read (| γ |) in
                                    M.read (|
                                      M.loop (|
                                        Ty.tuple [],
                                        ltac:(M.monadic
                                          (let~ _ : Ty.tuple [] :=
                                            M.match_operator (|
                                              Ty.tuple [],
                                              M.alloc (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [ Ty.path "usize" ],
                                                M.call_closure (|
                                                  Ty.apply
                                                    (Ty.path "core::option::Option")
                                                    []
                                                    [ Ty.path "usize" ],
                                                  M.get_trait_method (|
                                                    "core::iter::traits::iterator::Iterator",
                                                    Ty.apply
                                                      (Ty.path "core::ops::range::Range")
                                                      []
                                                      [ Ty.path "usize" ],
                                                    [],
                                                    [],
                                                    "next",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    M.borrow (|
                                                      Pointer.Kind.MutRef,
                                                      M.deref (|
                                                        M.borrow (| Pointer.Kind.MutRef, iter |)
                                                      |)
                                                    |)
                                                  ]
                                                |)
                                              |),
                                              [
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let _ :=
                                                      M.is_struct_tuple (|
                                                        γ,
                                                        "core::option::Option::None"
                                                      |) in
                                                    M.never_to_any (|
                                                      M.read (| M.break (||) |)
                                                    |)));
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let γ0_0 :=
                                                      M.SubPointer.get_struct_tuple_field (|
                                                        γ,
                                                        "core::option::Option::Some",
                                                        0
                                                      |) in
                                                    let i := M.copy (| Ty.path "usize", γ0_0 |) in
                                                    M.read (|
                                                      let~ _ : Ty.tuple [] :=
                                                        M.write (|
                                                          M.SubPointer.get_array_field (|
                                                            M.SubPointer.get_struct_record_field (|
                                                              M.deref (| M.read (| self |) |),
                                                              "core::num::bignum::tests::Big8x3",
                                                              "base"
                                                            |),
                                                            M.read (| i |)
                                                          |),
                                                          Value.Integer IntegerKind.U8 0
                                                        |) in
                                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                    |)))
                                              ]
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                      |)
                                    |)))
                              ]
                            |)
                          |))
                      |) in
                    let~ sz : Ty.path "usize" :=
                      M.call_closure (|
                        Ty.path "usize",
                        BinOp.Wrap.add,
                        [
                          M.read (|
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| self |) |),
                              "core::num::bignum::tests::Big8x3",
                              "size"
                            |)
                          |);
                          M.read (| digits |)
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ :=
                                M.alloc (|
                                  Ty.path "bool",
                                  M.call_closure (|
                                    Ty.path "bool",
                                    BinOp.gt,
                                    [ M.read (| bits |); Value.Integer IntegerKind.Usize 0 ]
                                  |)
                                |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.read (|
                                let~ last : Ty.path "usize" := M.read (| sz |) in
                                let~ overflow : Ty.path "u8" :=
                                  M.call_closure (|
                                    Ty.path "u8",
                                    BinOp.Wrap.shr,
                                    [
                                      M.read (|
                                        M.SubPointer.get_array_field (|
                                          M.SubPointer.get_struct_record_field (|
                                            M.deref (| M.read (| self |) |),
                                            "core::num::bignum::tests::Big8x3",
                                            "base"
                                          |),
                                          M.call_closure (|
                                            Ty.path "usize",
                                            BinOp.Wrap.sub,
                                            [ M.read (| last |); Value.Integer IntegerKind.Usize 1 ]
                                          |)
                                        |)
                                      |);
                                      M.call_closure (|
                                        Ty.path "usize",
                                        BinOp.Wrap.sub,
                                        [ M.read (| digitbits |); M.read (| bits |) ]
                                      |)
                                    ]
                                  |) in
                                let~ _ : Ty.tuple [] :=
                                  M.match_operator (|
                                    Ty.tuple [],
                                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                    [
                                      fun γ =>
                                        ltac:(M.monadic
                                          (let γ :=
                                            M.alloc (|
                                              Ty.path "bool",
                                              M.call_closure (|
                                                Ty.path "bool",
                                                BinOp.gt,
                                                [
                                                  M.read (| overflow |);
                                                  Value.Integer IntegerKind.U8 0
                                                ]
                                              |)
                                            |) in
                                          let _ :=
                                            is_constant_or_break_match (|
                                              M.read (| γ |),
                                              Value.Bool true
                                            |) in
                                          M.read (|
                                            let~ _ : Ty.tuple [] :=
                                              M.write (|
                                                M.SubPointer.get_array_field (|
                                                  M.SubPointer.get_struct_record_field (|
                                                    M.deref (| M.read (| self |) |),
                                                    "core::num::bignum::tests::Big8x3",
                                                    "base"
                                                  |),
                                                  M.read (| last |)
                                                |),
                                                M.read (| overflow |)
                                              |) in
                                            let~ _ : Ty.tuple [] :=
                                              let β := sz in
                                              M.write (|
                                                β,
                                                M.call_closure (|
                                                  Ty.path "usize",
                                                  BinOp.Wrap.add,
                                                  [
                                                    M.read (| β |);
                                                    Value.Integer IntegerKind.Usize 1
                                                  ]
                                                |)
                                              |) in
                                            M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                          |)));
                                      fun γ => ltac:(M.monadic (Value.Tuple []))
                                    ]
                                  |) in
                                let~ _ : Ty.tuple [] :=
                                  M.read (|
                                    M.use
                                      (M.alloc (|
                                        Ty.tuple [],
                                        M.match_operator (|
                                          Ty.tuple [],
                                          M.alloc (|
                                            Ty.apply
                                              (Ty.path "core::iter::adapters::rev::Rev")
                                              []
                                              [
                                                Ty.apply
                                                  (Ty.path "core::ops::range::Range")
                                                  []
                                                  [ Ty.path "usize" ]
                                              ],
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "core::iter::adapters::rev::Rev")
                                                []
                                                [
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::Range")
                                                    []
                                                    [ Ty.path "usize" ]
                                                ],
                                              M.get_trait_method (|
                                                "core::iter::traits::collect::IntoIterator",
                                                Ty.apply
                                                  (Ty.path "core::iter::adapters::rev::Rev")
                                                  []
                                                  [
                                                    Ty.apply
                                                      (Ty.path "core::ops::range::Range")
                                                      []
                                                      [ Ty.path "usize" ]
                                                  ],
                                                [],
                                                [],
                                                "into_iter",
                                                [],
                                                []
                                              |),
                                              [
                                                M.call_closure (|
                                                  Ty.apply
                                                    (Ty.path "core::iter::adapters::rev::Rev")
                                                    []
                                                    [
                                                      Ty.apply
                                                        (Ty.path "core::ops::range::Range")
                                                        []
                                                        [ Ty.path "usize" ]
                                                    ],
                                                  M.get_trait_method (|
                                                    "core::iter::traits::iterator::Iterator",
                                                    Ty.apply
                                                      (Ty.path "core::ops::range::Range")
                                                      []
                                                      [ Ty.path "usize" ],
                                                    [],
                                                    [],
                                                    "rev",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    Value.mkStructRecord
                                                      "core::ops::range::Range"
                                                      []
                                                      [ Ty.path "usize" ]
                                                      [
                                                        ("start",
                                                          M.call_closure (|
                                                            Ty.path "usize",
                                                            BinOp.Wrap.add,
                                                            [
                                                              M.read (| digits |);
                                                              Value.Integer IntegerKind.Usize 1
                                                            ]
                                                          |));
                                                        ("end_", M.read (| last |))
                                                      ]
                                                  ]
                                                |)
                                              ]
                                            |)
                                          |),
                                          [
                                            fun γ =>
                                              ltac:(M.monadic
                                                (let~ iter :
                                                    Ty.apply
                                                      (Ty.path "core::iter::adapters::rev::Rev")
                                                      []
                                                      [
                                                        Ty.apply
                                                          (Ty.path "core::ops::range::Range")
                                                          []
                                                          [ Ty.path "usize" ]
                                                      ] :=
                                                  M.read (| γ |) in
                                                M.read (|
                                                  M.loop (|
                                                    Ty.tuple [],
                                                    ltac:(M.monadic
                                                      (let~ _ : Ty.tuple [] :=
                                                        M.match_operator (|
                                                          Ty.tuple [],
                                                          M.alloc (|
                                                            Ty.apply
                                                              (Ty.path "core::option::Option")
                                                              []
                                                              [ Ty.path "usize" ],
                                                            M.call_closure (|
                                                              Ty.apply
                                                                (Ty.path "core::option::Option")
                                                                []
                                                                [ Ty.path "usize" ],
                                                              M.get_trait_method (|
                                                                "core::iter::traits::iterator::Iterator",
                                                                Ty.apply
                                                                  (Ty.path
                                                                    "core::iter::adapters::rev::Rev")
                                                                  []
                                                                  [
                                                                    Ty.apply
                                                                      (Ty.path
                                                                        "core::ops::range::Range")
                                                                      []
                                                                      [ Ty.path "usize" ]
                                                                  ],
                                                                [],
                                                                [],
                                                                "next",
                                                                [],
                                                                []
                                                              |),
                                                              [
                                                                M.borrow (|
                                                                  Pointer.Kind.MutRef,
                                                                  M.deref (|
                                                                    M.borrow (|
                                                                      Pointer.Kind.MutRef,
                                                                      iter
                                                                    |)
                                                                  |)
                                                                |)
                                                              ]
                                                            |)
                                                          |),
                                                          [
                                                            fun γ =>
                                                              ltac:(M.monadic
                                                                (let _ :=
                                                                  M.is_struct_tuple (|
                                                                    γ,
                                                                    "core::option::Option::None"
                                                                  |) in
                                                                M.never_to_any (|
                                                                  M.read (| M.break (||) |)
                                                                |)));
                                                            fun γ =>
                                                              ltac:(M.monadic
                                                                (let γ0_0 :=
                                                                  M.SubPointer.get_struct_tuple_field (|
                                                                    γ,
                                                                    "core::option::Option::Some",
                                                                    0
                                                                  |) in
                                                                let i :=
                                                                  M.copy (|
                                                                    Ty.path "usize",
                                                                    γ0_0
                                                                  |) in
                                                                M.read (|
                                                                  let~ _ : Ty.tuple [] :=
                                                                    M.write (|
                                                                      M.SubPointer.get_array_field (|
                                                                        M.SubPointer.get_struct_record_field (|
                                                                          M.deref (|
                                                                            M.read (| self |)
                                                                          |),
                                                                          "core::num::bignum::tests::Big8x3",
                                                                          "base"
                                                                        |),
                                                                        M.read (| i |)
                                                                      |),
                                                                      M.call_closure (|
                                                                        Ty.path "u8",
                                                                        BinOp.Wrap.bit_or,
                                                                        [
                                                                          M.call_closure (|
                                                                            Ty.path "u8",
                                                                            BinOp.Wrap.shl,
                                                                            [
                                                                              M.read (|
                                                                                M.SubPointer.get_array_field (|
                                                                                  M.SubPointer.get_struct_record_field (|
                                                                                    M.deref (|
                                                                                      M.read (|
                                                                                        self
                                                                                      |)
                                                                                    |),
                                                                                    "core::num::bignum::tests::Big8x3",
                                                                                    "base"
                                                                                  |),
                                                                                  M.read (| i |)
                                                                                |)
                                                                              |);
                                                                              M.read (| bits |)
                                                                            ]
                                                                          |);
                                                                          M.call_closure (|
                                                                            Ty.path "u8",
                                                                            BinOp.Wrap.shr,
                                                                            [
                                                                              M.read (|
                                                                                M.SubPointer.get_array_field (|
                                                                                  M.SubPointer.get_struct_record_field (|
                                                                                    M.deref (|
                                                                                      M.read (|
                                                                                        self
                                                                                      |)
                                                                                    |),
                                                                                    "core::num::bignum::tests::Big8x3",
                                                                                    "base"
                                                                                  |),
                                                                                  M.call_closure (|
                                                                                    Ty.path "usize",
                                                                                    BinOp.Wrap.sub,
                                                                                    [
                                                                                      M.read (|
                                                                                        i
                                                                                      |);
                                                                                      Value.Integer
                                                                                        IntegerKind.Usize
                                                                                        1
                                                                                    ]
                                                                                  |)
                                                                                |)
                                                                              |);
                                                                              M.call_closure (|
                                                                                Ty.path "usize",
                                                                                BinOp.Wrap.sub,
                                                                                [
                                                                                  M.read (|
                                                                                    digitbits
                                                                                  |);
                                                                                  M.read (| bits |)
                                                                                ]
                                                                              |)
                                                                            ]
                                                                          |)
                                                                        ]
                                                                      |)
                                                                    |) in
                                                                  M.alloc (|
                                                                    Ty.tuple [],
                                                                    Value.Tuple []
                                                                  |)
                                                                |)))
                                                          ]
                                                        |) in
                                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                                  |)
                                                |)))
                                          ]
                                        |)
                                      |))
                                  |) in
                                let~ _ : Ty.tuple [] :=
                                  let β :=
                                    M.SubPointer.get_array_field (|
                                      M.SubPointer.get_struct_record_field (|
                                        M.deref (| M.read (| self |) |),
                                        "core::num::bignum::tests::Big8x3",
                                        "base"
                                      |),
                                      M.read (| digits |)
                                    |) in
                                  M.write (|
                                    β,
                                    M.call_closure (|
                                      Ty.path "u8",
                                      BinOp.Wrap.shl,
                                      [ M.read (| β |); M.read (| bits |) ]
                                    |)
                                  |) in
                                M.alloc (| Ty.tuple [], Value.Tuple [] |)
                              |)));
                          fun γ => ltac:(M.monadic (Value.Tuple []))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.write (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| self |) |),
                          "core::num::bignum::tests::Big8x3",
                          "size"
                        |),
                        M.read (| sz |)
                      |) in
                    M.alloc (|
                      Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                      M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                    |)
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_mul_pow2 :
          M.IsAssociatedFunction.C Self "mul_pow2" mul_pow2.
        Admitted.
        Global Typeclasses Opaque mul_pow2.
        
        (*
                    pub fn mul_pow5(&mut self, mut e: usize) -> &mut $name {
                        use crate::num::bignum::SMALL_POW5;
        
                        // There are exactly n trailing zeros on 2^n, and the only relevant digit sizes
                        // are consecutive powers of two, so this is well suited index for the table.
                        let table_index = size_of::<$ty>().trailing_zeros() as usize;
                        let (small_power, small_e) = SMALL_POW5[table_index];
                        let small_power = small_power as $ty;
        
                        // Multiply with the largest single-digit power as long as possible ...
                        while e >= small_e {
                            self.mul_small(small_power);
                            e -= small_e;
                        }
        
                        // ... then finish off the remainder.
                        let mut rest_power = 1;
                        for _ in 0..e {
                            rest_power *= 5;
                        }
                        self.mul_small(rest_power);
        
                        self
                    }
        *)
        Definition mul_pow5 (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; e ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let e := M.alloc (| Ty.path "usize", e |) in
              M.borrow (|
                Pointer.Kind.MutRef,
                M.deref (|
                  M.read (|
                    let~ table_index : Ty.path "usize" :=
                      M.cast
                        (Ty.path "usize")
                        (M.call_closure (|
                          Ty.path "u32",
                          M.get_associated_function (| Ty.path "usize", "trailing_zeros", [], [] |),
                          [
                            M.call_closure (|
                              Ty.path "usize",
                              M.get_function (| "core::mem::size_of", [], [ Ty.path "u8" ] |),
                              []
                            |)
                          ]
                        |)) in
                    M.alloc (|
                      Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                      M.match_operator (|
                        Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                        M.SubPointer.get_array_field (|
                          get_constant (|
                            "core::num::bignum::SMALL_POW5",
                            Ty.apply
                              (Ty.path "array")
                              [ Value.Integer IntegerKind.Usize 3 ]
                              [ Ty.tuple [ Ty.path "u64"; Ty.path "usize" ] ]
                          |),
                          M.read (| table_index |)
                        |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ0_0 := M.SubPointer.get_tuple_field (| γ, 0 |) in
                              let γ0_1 := M.SubPointer.get_tuple_field (| γ, 1 |) in
                              let small_power := M.copy (| Ty.path "u64", γ0_0 |) in
                              let small_e := M.copy (| Ty.path "usize", γ0_1 |) in
                              M.read (|
                                let~ small_power : Ty.path "u8" :=
                                  M.cast (Ty.path "u8") (M.read (| small_power |)) in
                                let~ _ : Ty.tuple [] :=
                                  M.read (|
                                    M.loop (|
                                      Ty.tuple [],
                                      ltac:(M.monadic
                                        (M.alloc (|
                                          Ty.tuple [],
                                          M.match_operator (|
                                            Ty.tuple [],
                                            M.alloc (| Ty.tuple [], Value.Tuple [] |),
                                            [
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (let γ :=
                                                    M.alloc (|
                                                      Ty.path "bool",
                                                      M.call_closure (|
                                                        Ty.path "bool",
                                                        BinOp.ge,
                                                        [ M.read (| e |); M.read (| small_e |) ]
                                                      |)
                                                    |) in
                                                  let _ :=
                                                    is_constant_or_break_match (|
                                                      M.read (| γ |),
                                                      Value.Bool true
                                                    |) in
                                                  M.read (|
                                                    let~ _ :
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [
                                                            Ty.path
                                                              "core::num::bignum::tests::Big8x3"
                                                          ] :=
                                                      M.call_closure (|
                                                        Ty.apply
                                                          (Ty.path "&mut")
                                                          []
                                                          [
                                                            Ty.path
                                                              "core::num::bignum::tests::Big8x3"
                                                          ],
                                                        M.get_associated_function (|
                                                          Ty.path
                                                            "core::num::bignum::tests::Big8x3",
                                                          "mul_small",
                                                          [],
                                                          []
                                                        |),
                                                        [
                                                          M.borrow (|
                                                            Pointer.Kind.MutRef,
                                                            M.deref (| M.read (| self |) |)
                                                          |);
                                                          M.read (| small_power |)
                                                        ]
                                                      |) in
                                                    let~ _ : Ty.tuple [] :=
                                                      let β := e in
                                                      M.write (|
                                                        β,
                                                        M.call_closure (|
                                                          Ty.path "usize",
                                                          BinOp.Wrap.sub,
                                                          [ M.read (| β |); M.read (| small_e |) ]
                                                        |)
                                                      |) in
                                                    M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                  |)));
                                              fun γ =>
                                                ltac:(M.monadic
                                                  (M.never_to_any (|
                                                    M.read (|
                                                      let~ _ : Ty.tuple [] :=
                                                        M.never_to_any (|
                                                          M.read (| M.break (||) |)
                                                        |) in
                                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                    |)
                                                  |)))
                                            ]
                                          |)
                                        |)))
                                    |)
                                  |) in
                                let~ rest_power : Ty.path "u8" := Value.Integer IntegerKind.U8 1 in
                                let~ _ : Ty.tuple [] :=
                                  M.read (|
                                    M.use
                                      (M.alloc (|
                                        Ty.tuple [],
                                        M.match_operator (|
                                          Ty.tuple [],
                                          M.alloc (|
                                            Ty.apply
                                              (Ty.path "core::ops::range::Range")
                                              []
                                              [ Ty.path "usize" ],
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "core::ops::range::Range")
                                                []
                                                [ Ty.path "usize" ],
                                              M.get_trait_method (|
                                                "core::iter::traits::collect::IntoIterator",
                                                Ty.apply
                                                  (Ty.path "core::ops::range::Range")
                                                  []
                                                  [ Ty.path "usize" ],
                                                [],
                                                [],
                                                "into_iter",
                                                [],
                                                []
                                              |),
                                              [
                                                Value.mkStructRecord
                                                  "core::ops::range::Range"
                                                  []
                                                  [ Ty.path "usize" ]
                                                  [
                                                    ("start", Value.Integer IntegerKind.Usize 0);
                                                    ("end_", M.read (| e |))
                                                  ]
                                              ]
                                            |)
                                          |),
                                          [
                                            fun γ =>
                                              ltac:(M.monadic
                                                (let~ iter :
                                                    Ty.apply
                                                      (Ty.path "core::ops::range::Range")
                                                      []
                                                      [ Ty.path "usize" ] :=
                                                  M.read (| γ |) in
                                                M.read (|
                                                  M.loop (|
                                                    Ty.tuple [],
                                                    ltac:(M.monadic
                                                      (let~ _ : Ty.tuple [] :=
                                                        M.match_operator (|
                                                          Ty.tuple [],
                                                          M.alloc (|
                                                            Ty.apply
                                                              (Ty.path "core::option::Option")
                                                              []
                                                              [ Ty.path "usize" ],
                                                            M.call_closure (|
                                                              Ty.apply
                                                                (Ty.path "core::option::Option")
                                                                []
                                                                [ Ty.path "usize" ],
                                                              M.get_trait_method (|
                                                                "core::iter::traits::iterator::Iterator",
                                                                Ty.apply
                                                                  (Ty.path
                                                                    "core::ops::range::Range")
                                                                  []
                                                                  [ Ty.path "usize" ],
                                                                [],
                                                                [],
                                                                "next",
                                                                [],
                                                                []
                                                              |),
                                                              [
                                                                M.borrow (|
                                                                  Pointer.Kind.MutRef,
                                                                  M.deref (|
                                                                    M.borrow (|
                                                                      Pointer.Kind.MutRef,
                                                                      iter
                                                                    |)
                                                                  |)
                                                                |)
                                                              ]
                                                            |)
                                                          |),
                                                          [
                                                            fun γ =>
                                                              ltac:(M.monadic
                                                                (let _ :=
                                                                  M.is_struct_tuple (|
                                                                    γ,
                                                                    "core::option::Option::None"
                                                                  |) in
                                                                M.never_to_any (|
                                                                  M.read (| M.break (||) |)
                                                                |)));
                                                            fun γ =>
                                                              ltac:(M.monadic
                                                                (let γ0_0 :=
                                                                  M.SubPointer.get_struct_tuple_field (|
                                                                    γ,
                                                                    "core::option::Option::Some",
                                                                    0
                                                                  |) in
                                                                M.read (|
                                                                  let~ _ : Ty.tuple [] :=
                                                                    let β := rest_power in
                                                                    M.write (|
                                                                      β,
                                                                      M.call_closure (|
                                                                        Ty.path "u8",
                                                                        BinOp.Wrap.mul,
                                                                        [
                                                                          M.read (| β |);
                                                                          Value.Integer
                                                                            IntegerKind.U8
                                                                            5
                                                                        ]
                                                                      |)
                                                                    |) in
                                                                  M.alloc (|
                                                                    Ty.tuple [],
                                                                    Value.Tuple []
                                                                  |)
                                                                |)))
                                                          ]
                                                        |) in
                                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                                  |)
                                                |)))
                                          ]
                                        |)
                                      |))
                                  |) in
                                let~ _ :
                                    Ty.apply
                                      (Ty.path "&mut")
                                      []
                                      [ Ty.path "core::num::bignum::tests::Big8x3" ] :=
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "&mut")
                                      []
                                      [ Ty.path "core::num::bignum::tests::Big8x3" ],
                                    M.get_associated_function (|
                                      Ty.path "core::num::bignum::tests::Big8x3",
                                      "mul_small",
                                      [],
                                      []
                                    |),
                                    [
                                      M.borrow (|
                                        Pointer.Kind.MutRef,
                                        M.deref (| M.read (| self |) |)
                                      |);
                                      M.read (| rest_power |)
                                    ]
                                  |) in
                                M.alloc (|
                                  Ty.apply
                                    (Ty.path "&mut")
                                    []
                                    [ Ty.path "core::num::bignum::tests::Big8x3" ],
                                  M.borrow (|
                                    Pointer.Kind.MutRef,
                                    M.deref (| M.read (| self |) |)
                                  |)
                                |)
                              |)))
                        ]
                      |)
                    |)
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_mul_pow5 :
          M.IsAssociatedFunction.C Self "mul_pow5" mul_pow5.
        Admitted.
        Global Typeclasses Opaque mul_pow5.
        
        (*
                    pub fn mul_digits<'a>(&'a mut self, other: &[$ty]) -> &'a mut $name {
                        // the internal routine. works best when aa.len() <= bb.len().
                        fn mul_inner(ret: &mut [$ty; $n], aa: &[$ty], bb: &[$ty]) -> usize {
                            use crate::num::bignum::FullOps;
        
                            let mut retsz = 0;
                            for (i, &a) in aa.iter().enumerate() {
                                if a == 0 {
                                    continue;
                                }
                                let mut sz = bb.len();
                                let mut carry = 0;
                                for (j, &b) in bb.iter().enumerate() {
                                    let (c, v) = a.full_mul_add(b, ret[i + j], carry);
                                    ret[i + j] = v;
                                    carry = c;
                                }
                                if carry > 0 {
                                    ret[i + sz] = carry;
                                    sz += 1;
                                }
                                if retsz < i + sz {
                                    retsz = i + sz;
                                }
                            }
                            retsz
                        }
        
                        let mut ret = [0; $n];
                        let retsz = if self.size < other.len() {
                            mul_inner(&mut ret, &self.digits(), other)
                        } else {
                            mul_inner(&mut ret, other, &self.digits())
                        };
                        self.base = ret;
                        self.size = retsz;
                        self
                    }
        *)
        Definition mul_digits (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                  other
                |) in
              M.borrow (|
                Pointer.Kind.MutRef,
                M.deref (|
                  M.read (|
                    let~ ret :
                        Ty.apply
                          (Ty.path "array")
                          [ Value.Integer IntegerKind.Usize 3 ]
                          [ Ty.path "u8" ] :=
                      lib.repeat (|
                        Value.Integer IntegerKind.U8 0,
                        Value.Integer IntegerKind.Usize 3
                      |) in
                    let~ retsz : Ty.path "usize" :=
                      M.match_operator (|
                        Ty.path "usize",
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ :=
                                M.alloc (|
                                  Ty.path "bool",
                                  M.call_closure (|
                                    Ty.path "bool",
                                    BinOp.lt,
                                    [
                                      M.read (|
                                        M.SubPointer.get_struct_record_field (|
                                          M.deref (| M.read (| self |) |),
                                          "core::num::bignum::tests::Big8x3",
                                          "size"
                                        |)
                                      |);
                                      M.call_closure (|
                                        Ty.path "usize",
                                        M.get_associated_function (|
                                          Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ],
                                          "len",
                                          [],
                                          []
                                        |),
                                        [
                                          M.borrow (|
                                            Pointer.Kind.Ref,
                                            M.deref (| M.read (| other |) |)
                                          |)
                                        ]
                                      |)
                                    ]
                                  |)
                                |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              M.call_closure (|
                                Ty.path "usize",
                                M.get_associated_function (|
                                  Self,
                                  "mul_inner.mul_digits",
                                  [],
                                  []
                                |),
                                [
                                  M.borrow (|
                                    Pointer.Kind.MutRef,
                                    M.deref (| M.borrow (| Pointer.Kind.MutRef, ret |) |)
                                  |);
                                  M.borrow (|
                                    Pointer.Kind.Ref,
                                    M.deref (|
                                      M.read (|
                                        M.deref (|
                                          M.borrow (|
                                            Pointer.Kind.Ref,
                                            M.alloc (|
                                              Ty.apply
                                                (Ty.path "&")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "&")
                                                  []
                                                  [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ]
                                                  ],
                                                M.get_associated_function (|
                                                  Ty.path "core::num::bignum::tests::Big8x3",
                                                  "digits",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.Ref,
                                                    M.deref (| M.read (| self |) |)
                                                  |)
                                                ]
                                              |)
                                            |)
                                          |)
                                        |)
                                      |)
                                    |)
                                  |);
                                  M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| other |) |) |)
                                ]
                              |)));
                          fun γ =>
                            ltac:(M.monadic
                              (M.call_closure (|
                                Ty.path "usize",
                                M.get_associated_function (|
                                  Self,
                                  "mul_inner.mul_digits",
                                  [],
                                  []
                                |),
                                [
                                  M.borrow (|
                                    Pointer.Kind.MutRef,
                                    M.deref (| M.borrow (| Pointer.Kind.MutRef, ret |) |)
                                  |);
                                  M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| other |) |) |);
                                  M.borrow (|
                                    Pointer.Kind.Ref,
                                    M.deref (|
                                      M.read (|
                                        M.deref (|
                                          M.borrow (|
                                            Pointer.Kind.Ref,
                                            M.alloc (|
                                              Ty.apply
                                                (Ty.path "&")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                              M.call_closure (|
                                                Ty.apply
                                                  (Ty.path "&")
                                                  []
                                                  [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ]
                                                  ],
                                                M.get_associated_function (|
                                                  Ty.path "core::num::bignum::tests::Big8x3",
                                                  "digits",
                                                  [],
                                                  []
                                                |),
                                                [
                                                  M.borrow (|
                                                    Pointer.Kind.Ref,
                                                    M.deref (| M.read (| self |) |)
                                                  |)
                                                ]
                                              |)
                                            |)
                                          |)
                                        |)
                                      |)
                                    |)
                                  |)
                                ]
                              |)))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.write (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| self |) |),
                          "core::num::bignum::tests::Big8x3",
                          "base"
                        |),
                        M.read (| ret |)
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.write (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| self |) |),
                          "core::num::bignum::tests::Big8x3",
                          "size"
                        |),
                        M.read (| retsz |)
                      |) in
                    M.alloc (|
                      Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                      M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |)
                    |)
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_mul_digits :
          M.IsAssociatedFunction.C Self "mul_digits" mul_digits.
        Admitted.
        Global Typeclasses Opaque mul_digits.
        
        (*
                    pub fn div_rem_small(&mut self, other: $ty) -> (&mut $name, $ty) {
                        use crate::num::bignum::FullOps;
        
                        assert!(other > 0);
        
                        let sz = self.size;
                        let mut borrow = 0;
                        for a in self.base[..sz].iter_mut().rev() {
                            let (q, r) = ( *a).full_div_rem(other, borrow);
                            *a = q;
                            borrow = r;
                        }
                        (self, borrow)
                    }
        *)
        Definition div_rem_small (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other := M.alloc (| Ty.path "u8", other |) in
              M.read (|
                let~ _ : Ty.tuple [] :=
                  M.match_operator (|
                    Ty.tuple [],
                    M.alloc (| Ty.tuple [], Value.Tuple [] |),
                    [
                      fun γ =>
                        ltac:(M.monadic
                          (let γ :=
                            M.alloc (|
                              Ty.path "bool",
                              M.call_closure (|
                                Ty.path "bool",
                                UnOp.not,
                                [
                                  M.call_closure (|
                                    Ty.path "bool",
                                    BinOp.gt,
                                    [ M.read (| other |); Value.Integer IntegerKind.U8 0 ]
                                  |)
                                ]
                              |)
                            |) in
                          let _ :=
                            is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                          M.never_to_any (|
                            M.call_closure (|
                              Ty.path "never",
                              M.get_function (| "core::panicking::panic", [], [] |),
                              [ mk_str (| "assertion failed: other > 0" |) ]
                            |)
                          |)));
                      fun γ => ltac:(M.monadic (Value.Tuple []))
                    ]
                  |) in
                let~ sz : Ty.path "usize" :=
                  M.read (|
                    M.SubPointer.get_struct_record_field (|
                      M.deref (| M.read (| self |) |),
                      "core::num::bignum::tests::Big8x3",
                      "size"
                    |)
                  |) in
                let~ borrow : Ty.path "u8" := Value.Integer IntegerKind.U8 0 in
                let~ _ : Ty.tuple [] :=
                  M.read (|
                    M.use
                      (M.alloc (|
                        Ty.tuple [],
                        M.match_operator (|
                          Ty.tuple [],
                          M.alloc (|
                            Ty.apply
                              (Ty.path "core::iter::adapters::rev::Rev")
                              []
                              [ Ty.apply (Ty.path "core::slice::iter::IterMut") [] [ Ty.path "u8" ]
                              ],
                            M.call_closure (|
                              Ty.apply
                                (Ty.path "core::iter::adapters::rev::Rev")
                                []
                                [
                                  Ty.apply
                                    (Ty.path "core::slice::iter::IterMut")
                                    []
                                    [ Ty.path "u8" ]
                                ],
                              M.get_trait_method (|
                                "core::iter::traits::collect::IntoIterator",
                                Ty.apply
                                  (Ty.path "core::iter::adapters::rev::Rev")
                                  []
                                  [
                                    Ty.apply
                                      (Ty.path "core::slice::iter::IterMut")
                                      []
                                      [ Ty.path "u8" ]
                                  ],
                                [],
                                [],
                                "into_iter",
                                [],
                                []
                              |),
                              [
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::rev::Rev")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::slice::iter::IterMut")
                                        []
                                        [ Ty.path "u8" ]
                                    ],
                                  M.get_trait_method (|
                                    "core::iter::traits::iterator::Iterator",
                                    Ty.apply
                                      (Ty.path "core::slice::iter::IterMut")
                                      []
                                      [ Ty.path "u8" ],
                                    [],
                                    [],
                                    "rev",
                                    [],
                                    []
                                  |),
                                  [
                                    M.call_closure (|
                                      Ty.apply
                                        (Ty.path "core::slice::iter::IterMut")
                                        []
                                        [ Ty.path "u8" ],
                                      M.get_associated_function (|
                                        Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ],
                                        "iter_mut",
                                        [],
                                        []
                                      |),
                                      [
                                        M.borrow (|
                                          Pointer.Kind.MutRef,
                                          M.deref (|
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "&mut")
                                                []
                                                [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                              M.get_trait_method (|
                                                "core::ops::index::IndexMut",
                                                Ty.apply
                                                  (Ty.path "array")
                                                  [ Value.Integer IntegerKind.Usize 3 ]
                                                  [ Ty.path "u8" ],
                                                [],
                                                [
                                                  Ty.apply
                                                    (Ty.path "core::ops::range::RangeTo")
                                                    []
                                                    [ Ty.path "usize" ]
                                                ],
                                                "index_mut",
                                                [],
                                                []
                                              |),
                                              [
                                                M.borrow (|
                                                  Pointer.Kind.MutRef,
                                                  M.SubPointer.get_struct_record_field (|
                                                    M.deref (| M.read (| self |) |),
                                                    "core::num::bignum::tests::Big8x3",
                                                    "base"
                                                  |)
                                                |);
                                                Value.mkStructRecord
                                                  "core::ops::range::RangeTo"
                                                  []
                                                  [ Ty.path "usize" ]
                                                  [ ("end_", M.read (| sz |)) ]
                                              ]
                                            |)
                                          |)
                                        |)
                                      ]
                                    |)
                                  ]
                                |)
                              ]
                            |)
                          |),
                          [
                            fun γ =>
                              ltac:(M.monadic
                                (let~ iter :
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::rev::Rev")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::slice::iter::IterMut")
                                          []
                                          [ Ty.path "u8" ]
                                      ] :=
                                  M.read (| γ |) in
                                M.read (|
                                  M.loop (|
                                    Ty.tuple [],
                                    ltac:(M.monadic
                                      (let~ _ : Ty.tuple [] :=
                                        M.match_operator (|
                                          Ty.tuple [],
                                          M.alloc (|
                                            Ty.apply
                                              (Ty.path "core::option::Option")
                                              []
                                              [ Ty.apply (Ty.path "&mut") [] [ Ty.path "u8" ] ],
                                            M.call_closure (|
                                              Ty.apply
                                                (Ty.path "core::option::Option")
                                                []
                                                [ Ty.apply (Ty.path "&mut") [] [ Ty.path "u8" ] ],
                                              M.get_trait_method (|
                                                "core::iter::traits::iterator::Iterator",
                                                Ty.apply
                                                  (Ty.path "core::iter::adapters::rev::Rev")
                                                  []
                                                  [
                                                    Ty.apply
                                                      (Ty.path "core::slice::iter::IterMut")
                                                      []
                                                      [ Ty.path "u8" ]
                                                  ],
                                                [],
                                                [],
                                                "next",
                                                [],
                                                []
                                              |),
                                              [
                                                M.borrow (|
                                                  Pointer.Kind.MutRef,
                                                  M.deref (|
                                                    M.borrow (| Pointer.Kind.MutRef, iter |)
                                                  |)
                                                |)
                                              ]
                                            |)
                                          |),
                                          [
                                            fun γ =>
                                              ltac:(M.monadic
                                                (let _ :=
                                                  M.is_struct_tuple (|
                                                    γ,
                                                    "core::option::Option::None"
                                                  |) in
                                                M.never_to_any (| M.read (| M.break (||) |) |)));
                                            fun γ =>
                                              ltac:(M.monadic
                                                (let γ0_0 :=
                                                  M.SubPointer.get_struct_tuple_field (|
                                                    γ,
                                                    "core::option::Option::Some",
                                                    0
                                                  |) in
                                                let a :=
                                                  M.copy (|
                                                    Ty.apply (Ty.path "&mut") [] [ Ty.path "u8" ],
                                                    γ0_0
                                                  |) in
                                                M.match_operator (|
                                                  Ty.tuple [],
                                                  M.alloc (|
                                                    Ty.tuple [ Ty.path "u8"; Ty.path "u8" ],
                                                    M.call_closure (|
                                                      Ty.tuple [ Ty.path "u8"; Ty.path "u8" ],
                                                      M.get_trait_method (|
                                                        "core::num::bignum::FullOps",
                                                        Ty.path "u8",
                                                        [],
                                                        [],
                                                        "full_div_rem",
                                                        [],
                                                        []
                                                      |),
                                                      [
                                                        M.read (| M.deref (| M.read (| a |) |) |);
                                                        M.read (| other |);
                                                        M.read (| borrow |)
                                                      ]
                                                    |)
                                                  |),
                                                  [
                                                    fun γ =>
                                                      ltac:(M.monadic
                                                        (let γ0_0 :=
                                                          M.SubPointer.get_tuple_field (| γ, 0 |) in
                                                        let γ0_1 :=
                                                          M.SubPointer.get_tuple_field (| γ, 1 |) in
                                                        let q := M.copy (| Ty.path "u8", γ0_0 |) in
                                                        let r := M.copy (| Ty.path "u8", γ0_1 |) in
                                                        M.read (|
                                                          let~ _ : Ty.tuple [] :=
                                                            M.write (|
                                                              M.deref (| M.read (| a |) |),
                                                              M.read (| q |)
                                                            |) in
                                                          let~ _ : Ty.tuple [] :=
                                                            M.write (| borrow, M.read (| r |) |) in
                                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                        |)))
                                                  ]
                                                |)))
                                          ]
                                        |) in
                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                  |)
                                |)))
                          ]
                        |)
                      |))
                  |) in
                M.alloc (|
                  Ty.tuple
                    [
                      Ty.apply (Ty.path "&mut") [] [ Ty.path "core::num::bignum::tests::Big8x3" ];
                      Ty.path "u8"
                    ],
                  Value.Tuple
                    [
                      M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| self |) |) |);
                      M.read (| borrow |)
                    ]
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Global Instance AssociatedFunction_div_rem_small :
          M.IsAssociatedFunction.C Self "div_rem_small" div_rem_small.
        Admitted.
        Global Typeclasses Opaque div_rem_small.
      End Impl_core_num_bignum_tests_Big8x3.
      
      Module Impl_core_cmp_PartialEq_core_num_bignum_tests_Big8x3_for_core_num_bignum_tests_Big8x3.
        Definition Self : Ty.t := Ty.path "core::num::bignum::tests::Big8x3".
        
        (*
                    fn eq(&self, other: &$name) -> bool {
                        self.base[..] == other.base[..]
                    }
        *)
        Definition eq (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  other
                |) in
              M.call_closure (|
                Ty.path "bool",
                M.get_trait_method (|
                  "core::cmp::PartialEq",
                  Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ],
                  [],
                  [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                  "eq",
                  [],
                  []
                |),
                [
                  M.borrow (|
                    Pointer.Kind.Ref,
                    M.deref (|
                      M.call_closure (|
                        Ty.apply
                          (Ty.path "&")
                          []
                          [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                        M.get_trait_method (|
                          "core::ops::index::Index",
                          Ty.apply
                            (Ty.path "array")
                            [ Value.Integer IntegerKind.Usize 3 ]
                            [ Ty.path "u8" ],
                          [],
                          [ Ty.path "core::ops::range::RangeFull" ],
                          "index",
                          [],
                          []
                        |),
                        [
                          M.borrow (|
                            Pointer.Kind.Ref,
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| self |) |),
                              "core::num::bignum::tests::Big8x3",
                              "base"
                            |)
                          |);
                          Value.StructTuple "core::ops::range::RangeFull" [] [] []
                        ]
                      |)
                    |)
                  |);
                  M.borrow (|
                    Pointer.Kind.Ref,
                    M.deref (|
                      M.call_closure (|
                        Ty.apply
                          (Ty.path "&")
                          []
                          [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                        M.get_trait_method (|
                          "core::ops::index::Index",
                          Ty.apply
                            (Ty.path "array")
                            [ Value.Integer IntegerKind.Usize 3 ]
                            [ Ty.path "u8" ],
                          [],
                          [ Ty.path "core::ops::range::RangeFull" ],
                          "index",
                          [],
                          []
                        |),
                        [
                          M.borrow (|
                            Pointer.Kind.Ref,
                            M.SubPointer.get_struct_record_field (|
                              M.deref (| M.read (| other |) |),
                              "core::num::bignum::tests::Big8x3",
                              "base"
                            |)
                          |);
                          Value.StructTuple "core::ops::range::RangeFull" [] [] []
                        ]
                      |)
                    |)
                  |)
                ]
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Axiom Implements :
          M.IsTraitInstance
            "core::cmp::PartialEq"
            (* Trait polymorphic consts *) []
            (* Trait polymorphic types *) [ Ty.path "core::num::bignum::tests::Big8x3" ]
            Self
            (* Instance *) [ ("eq", InstanceField.Method eq) ].
      End Impl_core_cmp_PartialEq_core_num_bignum_tests_Big8x3_for_core_num_bignum_tests_Big8x3.
      
      Module Impl_core_cmp_Eq_for_core_num_bignum_tests_Big8x3.
        Definition Self : Ty.t := Ty.path "core::num::bignum::tests::Big8x3".
        
        Axiom Implements :
          M.IsTraitInstance
            "core::cmp::Eq"
            (* Trait polymorphic consts *) []
            (* Trait polymorphic types *) []
            Self
            (* Instance *) [].
      End Impl_core_cmp_Eq_for_core_num_bignum_tests_Big8x3.
      
      Module Impl_core_cmp_PartialOrd_core_num_bignum_tests_Big8x3_for_core_num_bignum_tests_Big8x3.
        Definition Self : Ty.t := Ty.path "core::num::bignum::tests::Big8x3".
        
        (*
                    fn partial_cmp(&self, other: &$name) -> crate::option::Option<crate::cmp::Ordering> {
                        crate::option::Option::Some(self.cmp(other))
                    }
        *)
        Definition partial_cmp (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  other
                |) in
              Value.StructTuple
                "core::option::Option::Some"
                []
                [ Ty.path "core::cmp::Ordering" ]
                [
                  M.call_closure (|
                    Ty.path "core::cmp::Ordering",
                    M.get_trait_method (|
                      "core::cmp::Ord",
                      Ty.path "core::num::bignum::tests::Big8x3",
                      [],
                      [],
                      "cmp",
                      [],
                      []
                    |),
                    [
                      M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| self |) |) |);
                      M.borrow (| Pointer.Kind.Ref, M.deref (| M.read (| other |) |) |)
                    ]
                  |)
                ]))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Axiom Implements :
          M.IsTraitInstance
            "core::cmp::PartialOrd"
            (* Trait polymorphic consts *) []
            (* Trait polymorphic types *) [ Ty.path "core::num::bignum::tests::Big8x3" ]
            Self
            (* Instance *) [ ("partial_cmp", InstanceField.Method partial_cmp) ].
      End Impl_core_cmp_PartialOrd_core_num_bignum_tests_Big8x3_for_core_num_bignum_tests_Big8x3.
      
      Module Impl_core_cmp_Ord_for_core_num_bignum_tests_Big8x3.
        Definition Self : Ty.t := Ty.path "core::num::bignum::tests::Big8x3".
        
        (*
                    fn cmp(&self, other: &$name) -> crate::cmp::Ordering {
                        use crate::cmp::max;
                        let sz = max(self.size, other.size);
                        let lhs = self.base[..sz].iter().cloned().rev();
                        let rhs = other.base[..sz].iter().cloned().rev();
                        lhs.cmp(rhs)
                    }
        *)
        Definition cmp (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; other ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let other :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  other
                |) in
              M.read (|
                let~ sz : Ty.path "usize" :=
                  M.call_closure (|
                    Ty.path "usize",
                    M.get_function (| "core::cmp::max", [], [ Ty.path "usize" ] |),
                    [
                      M.read (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| self |) |),
                          "core::num::bignum::tests::Big8x3",
                          "size"
                        |)
                      |);
                      M.read (|
                        M.SubPointer.get_struct_record_field (|
                          M.deref (| M.read (| other |) |),
                          "core::num::bignum::tests::Big8x3",
                          "size"
                        |)
                      |)
                    ]
                  |) in
                let~ lhs :
                    Ty.apply
                      (Ty.path "core::iter::adapters::rev::Rev")
                      []
                      [
                        Ty.apply
                          (Ty.path "core::iter::adapters::cloned::Cloned")
                          []
                          [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ]
                      ] :=
                  M.call_closure (|
                    Ty.apply
                      (Ty.path "core::iter::adapters::rev::Rev")
                      []
                      [
                        Ty.apply
                          (Ty.path "core::iter::adapters::cloned::Cloned")
                          []
                          [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ]
                      ],
                    M.get_trait_method (|
                      "core::iter::traits::iterator::Iterator",
                      Ty.apply
                        (Ty.path "core::iter::adapters::cloned::Cloned")
                        []
                        [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ],
                      [],
                      [],
                      "rev",
                      [],
                      []
                    |),
                    [
                      M.call_closure (|
                        Ty.apply
                          (Ty.path "core::iter::adapters::cloned::Cloned")
                          []
                          [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ],
                        M.get_trait_method (|
                          "core::iter::traits::iterator::Iterator",
                          Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                          [],
                          [],
                          "cloned",
                          [],
                          [ Ty.path "u8" ]
                        |),
                        [
                          M.call_closure (|
                            Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                            M.get_associated_function (|
                              Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ],
                              "iter",
                              [],
                              []
                            |),
                            [
                              M.borrow (|
                                Pointer.Kind.Ref,
                                M.deref (|
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "&")
                                      []
                                      [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                    M.get_trait_method (|
                                      "core::ops::index::Index",
                                      Ty.apply
                                        (Ty.path "array")
                                        [ Value.Integer IntegerKind.Usize 3 ]
                                        [ Ty.path "u8" ],
                                      [],
                                      [
                                        Ty.apply
                                          (Ty.path "core::ops::range::RangeTo")
                                          []
                                          [ Ty.path "usize" ]
                                      ],
                                      "index",
                                      [],
                                      []
                                    |),
                                    [
                                      M.borrow (|
                                        Pointer.Kind.Ref,
                                        M.SubPointer.get_struct_record_field (|
                                          M.deref (| M.read (| self |) |),
                                          "core::num::bignum::tests::Big8x3",
                                          "base"
                                        |)
                                      |);
                                      Value.mkStructRecord
                                        "core::ops::range::RangeTo"
                                        []
                                        [ Ty.path "usize" ]
                                        [ ("end_", M.read (| sz |)) ]
                                    ]
                                  |)
                                |)
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |) in
                let~ rhs :
                    Ty.apply
                      (Ty.path "core::iter::adapters::rev::Rev")
                      []
                      [
                        Ty.apply
                          (Ty.path "core::iter::adapters::cloned::Cloned")
                          []
                          [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ]
                      ] :=
                  M.call_closure (|
                    Ty.apply
                      (Ty.path "core::iter::adapters::rev::Rev")
                      []
                      [
                        Ty.apply
                          (Ty.path "core::iter::adapters::cloned::Cloned")
                          []
                          [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ]
                      ],
                    M.get_trait_method (|
                      "core::iter::traits::iterator::Iterator",
                      Ty.apply
                        (Ty.path "core::iter::adapters::cloned::Cloned")
                        []
                        [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ],
                      [],
                      [],
                      "rev",
                      [],
                      []
                    |),
                    [
                      M.call_closure (|
                        Ty.apply
                          (Ty.path "core::iter::adapters::cloned::Cloned")
                          []
                          [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ],
                        M.get_trait_method (|
                          "core::iter::traits::iterator::Iterator",
                          Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                          [],
                          [],
                          "cloned",
                          [],
                          [ Ty.path "u8" ]
                        |),
                        [
                          M.call_closure (|
                            Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ],
                            M.get_associated_function (|
                              Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ],
                              "iter",
                              [],
                              []
                            |),
                            [
                              M.borrow (|
                                Pointer.Kind.Ref,
                                M.deref (|
                                  M.call_closure (|
                                    Ty.apply
                                      (Ty.path "&")
                                      []
                                      [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ] ],
                                    M.get_trait_method (|
                                      "core::ops::index::Index",
                                      Ty.apply
                                        (Ty.path "array")
                                        [ Value.Integer IntegerKind.Usize 3 ]
                                        [ Ty.path "u8" ],
                                      [],
                                      [
                                        Ty.apply
                                          (Ty.path "core::ops::range::RangeTo")
                                          []
                                          [ Ty.path "usize" ]
                                      ],
                                      "index",
                                      [],
                                      []
                                    |),
                                    [
                                      M.borrow (|
                                        Pointer.Kind.Ref,
                                        M.SubPointer.get_struct_record_field (|
                                          M.deref (| M.read (| other |) |),
                                          "core::num::bignum::tests::Big8x3",
                                          "base"
                                        |)
                                      |);
                                      Value.mkStructRecord
                                        "core::ops::range::RangeTo"
                                        []
                                        [ Ty.path "usize" ]
                                        [ ("end_", M.read (| sz |)) ]
                                    ]
                                  |)
                                |)
                              |)
                            ]
                          |)
                        ]
                      |)
                    ]
                  |) in
                M.alloc (|
                  Ty.path "core::cmp::Ordering",
                  M.call_closure (|
                    Ty.path "core::cmp::Ordering",
                    M.get_trait_method (|
                      "core::iter::traits::iterator::Iterator",
                      Ty.apply
                        (Ty.path "core::iter::adapters::rev::Rev")
                        []
                        [
                          Ty.apply
                            (Ty.path "core::iter::adapters::cloned::Cloned")
                            []
                            [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ]
                        ],
                      [],
                      [],
                      "cmp",
                      [],
                      [
                        Ty.apply
                          (Ty.path "core::iter::adapters::rev::Rev")
                          []
                          [
                            Ty.apply
                              (Ty.path "core::iter::adapters::cloned::Cloned")
                              []
                              [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ] ]
                          ]
                      ]
                    |),
                    [ M.read (| lhs |); M.read (| rhs |) ]
                  |)
                |)
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Axiom Implements :
          M.IsTraitInstance
            "core::cmp::Ord"
            (* Trait polymorphic consts *) []
            (* Trait polymorphic types *) []
            Self
            (* Instance *) [ ("cmp", InstanceField.Method cmp) ].
      End Impl_core_cmp_Ord_for_core_num_bignum_tests_Big8x3.
      
      Module Impl_core_clone_Clone_for_core_num_bignum_tests_Big8x3.
        Definition Self : Ty.t := Ty.path "core::num::bignum::tests::Big8x3".
        
        (*
                    fn clone(&self) -> Self {
                        Self { size: self.size, base: self.base }
                    }
        *)
        Definition clone (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              Value.mkStructRecord
                "core::num::bignum::tests::Big8x3"
                []
                []
                [
                  ("size",
                    M.read (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::tests::Big8x3",
                        "size"
                      |)
                    |));
                  ("base",
                    M.read (|
                      M.SubPointer.get_struct_record_field (|
                        M.deref (| M.read (| self |) |),
                        "core::num::bignum::tests::Big8x3",
                        "base"
                      |)
                    |))
                ]))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Axiom Implements :
          M.IsTraitInstance
            "core::clone::Clone"
            (* Trait polymorphic consts *) []
            (* Trait polymorphic types *) []
            Self
            (* Instance *) [ ("clone", InstanceField.Method clone) ].
      End Impl_core_clone_Clone_for_core_num_bignum_tests_Big8x3.
      
      Module Impl_core_clone_UseCloned_for_core_num_bignum_tests_Big8x3.
        Definition Self : Ty.t := Ty.path "core::num::bignum::tests::Big8x3".
        
        Axiom Implements :
          M.IsTraitInstance
            "core::clone::UseCloned"
            (* Trait polymorphic consts *) []
            (* Trait polymorphic types *) []
            Self
            (* Instance *) [].
      End Impl_core_clone_UseCloned_for_core_num_bignum_tests_Big8x3.
      
      Module Impl_core_fmt_Debug_for_core_num_bignum_tests_Big8x3.
        Definition Self : Ty.t := Ty.path "core::num::bignum::tests::Big8x3".
        
        (*
                    fn fmt(&self, f: &mut crate::fmt::Formatter<'_>) -> crate::fmt::Result {
                        let sz = if self.size < 1 { 1 } else { self.size };
                        let digitlen = <$ty>::BITS as usize / 4;
        
                        write!(f, "{:#x}", self.base[sz - 1])?;
                        for &v in self.base[..sz - 1].iter().rev() {
                            write!(f, "_{:01$x}", v, digitlen)?;
                        }
                        crate::result::Result::Ok(())
                    }
        *)
        Definition fmt (ε : list Value.t) (τ : list Ty.t) (α : list Value.t) : M :=
          match ε, τ, α with
          | [], [], [ self; f ] =>
            ltac:(M.monadic
              (let self :=
                M.alloc (|
                  Ty.apply (Ty.path "&") [] [ Ty.path "core::num::bignum::tests::Big8x3" ],
                  self
                |) in
              let f :=
                M.alloc (| Ty.apply (Ty.path "&mut") [] [ Ty.path "core::fmt::Formatter" ], f |) in
              M.catch_return
                (Ty.apply
                  (Ty.path "core::result::Result")
                  []
                  [ Ty.tuple []; Ty.path "core::fmt::Error" ]) (|
                ltac:(M.monadic
                  (M.read (|
                    let~ sz : Ty.path "usize" :=
                      M.match_operator (|
                        Ty.path "usize",
                        M.alloc (| Ty.tuple [], Value.Tuple [] |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ :=
                                M.alloc (|
                                  Ty.path "bool",
                                  M.call_closure (|
                                    Ty.path "bool",
                                    BinOp.lt,
                                    [
                                      M.read (|
                                        M.SubPointer.get_struct_record_field (|
                                          M.deref (| M.read (| self |) |),
                                          "core::num::bignum::tests::Big8x3",
                                          "size"
                                        |)
                                      |);
                                      Value.Integer IntegerKind.Usize 1
                                    ]
                                  |)
                                |) in
                              let _ :=
                                is_constant_or_break_match (| M.read (| γ |), Value.Bool true |) in
                              Value.Integer IntegerKind.Usize 1));
                          fun γ =>
                            ltac:(M.monadic
                              (M.read (|
                                M.SubPointer.get_struct_record_field (|
                                  M.deref (| M.read (| self |) |),
                                  "core::num::bignum::tests::Big8x3",
                                  "size"
                                |)
                              |)))
                        ]
                      |) in
                    let~ digitlen : Ty.path "usize" :=
                      M.call_closure (|
                        Ty.path "usize",
                        BinOp.Wrap.div,
                        [
                          M.cast
                            (Ty.path "usize")
                            (M.read (|
                              get_associated_constant (| Ty.path "u8", "BITS", Ty.path "u32" |)
                            |));
                          Value.Integer IntegerKind.Usize 4
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.match_operator (|
                        Ty.tuple [],
                        M.alloc (|
                          Ty.apply
                            (Ty.path "core::ops::control_flow::ControlFlow")
                            []
                            [
                              Ty.apply
                                (Ty.path "core::result::Result")
                                []
                                [ Ty.path "core::convert::Infallible"; Ty.path "core::fmt::Error" ];
                              Ty.tuple []
                            ],
                          M.call_closure (|
                            Ty.apply
                              (Ty.path "core::ops::control_flow::ControlFlow")
                              []
                              [
                                Ty.apply
                                  (Ty.path "core::result::Result")
                                  []
                                  [ Ty.path "core::convert::Infallible"; Ty.path "core::fmt::Error"
                                  ];
                                Ty.tuple []
                              ],
                            M.get_trait_method (|
                              "core::ops::try_trait::Try",
                              Ty.apply
                                (Ty.path "core::result::Result")
                                []
                                [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                              [],
                              [],
                              "branch",
                              [],
                              []
                            |),
                            [
                              M.call_closure (|
                                Ty.apply
                                  (Ty.path "core::result::Result")
                                  []
                                  [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                                M.get_associated_function (|
                                  Ty.path "core::fmt::Formatter",
                                  "write_fmt",
                                  [],
                                  []
                                |),
                                [
                                  M.borrow (| Pointer.Kind.MutRef, M.deref (| M.read (| f |) |) |);
                                  M.read (|
                                    let~ args :
                                        Ty.tuple [ Ty.apply (Ty.path "&") [] [ Ty.path "u8" ] ] :=
                                      Value.Tuple
                                        [
                                          M.borrow (|
                                            Pointer.Kind.Ref,
                                            M.SubPointer.get_array_field (|
                                              M.SubPointer.get_struct_record_field (|
                                                M.deref (| M.read (| self |) |),
                                                "core::num::bignum::tests::Big8x3",
                                                "base"
                                              |),
                                              M.call_closure (|
                                                Ty.path "usize",
                                                BinOp.Wrap.sub,
                                                [ M.read (| sz |); Value.Integer IntegerKind.Usize 1
                                                ]
                                              |)
                                            |)
                                          |)
                                        ] in
                                    let~ args :
                                        Ty.apply
                                          (Ty.path "array")
                                          [ Value.Integer IntegerKind.Usize 1 ]
                                          [ Ty.path "core::fmt::rt::Argument" ] :=
                                      Value.Array
                                        [
                                          M.call_closure (|
                                            Ty.path "core::fmt::rt::Argument",
                                            M.get_associated_function (|
                                              Ty.path "core::fmt::rt::Argument",
                                              "new_lower_hex",
                                              [],
                                              [ Ty.path "u8" ]
                                            |),
                                            [
                                              M.borrow (|
                                                Pointer.Kind.Ref,
                                                M.deref (|
                                                  M.read (|
                                                    M.SubPointer.get_tuple_field (| args, 0 |)
                                                  |)
                                                |)
                                              |)
                                            ]
                                          |)
                                        ] in
                                    M.alloc (|
                                      Ty.path "core::fmt::Arguments",
                                      M.call_closure (|
                                        Ty.path "core::fmt::Arguments",
                                        M.get_associated_function (|
                                          Ty.path "core::fmt::Arguments",
                                          "new",
                                          [
                                            Value.Integer IntegerKind.Usize 6;
                                            Value.Integer IntegerKind.Usize 1
                                          ],
                                          []
                                        |),
                                        [
                                          M.borrow (|
                                            Pointer.Kind.Ref,
                                            M.deref (|
                                              M.mk_byte_str_ref 6 [ 193; 32; 0; 128; 96; 0 ]
                                            |)
                                          |);
                                          M.borrow (|
                                            Pointer.Kind.Ref,
                                            M.deref (| M.borrow (| Pointer.Kind.Ref, args |) |)
                                          |)
                                        ]
                                      |)
                                    |)
                                  |)
                                ]
                              |)
                            ]
                          |)
                        |),
                        [
                          fun γ =>
                            ltac:(M.monadic
                              (let γ0_0 :=
                                M.SubPointer.get_struct_tuple_field (|
                                  γ,
                                  "core::ops::control_flow::ControlFlow::Break",
                                  0
                                |) in
                              let residual :=
                                M.copy (|
                                  Ty.apply
                                    (Ty.path "core::result::Result")
                                    []
                                    [
                                      Ty.path "core::convert::Infallible";
                                      Ty.path "core::fmt::Error"
                                    ],
                                  γ0_0
                                |) in
                              M.never_to_any (|
                                M.read (|
                                  M.return_ (|
                                    M.call_closure (|
                                      Ty.apply
                                        (Ty.path "core::result::Result")
                                        []
                                        [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                                      M.get_trait_method (|
                                        "core::ops::try_trait::FromResidual",
                                        Ty.apply
                                          (Ty.path "core::result::Result")
                                          []
                                          [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                                        [],
                                        [
                                          Ty.apply
                                            (Ty.path "core::result::Result")
                                            []
                                            [
                                              Ty.path "core::convert::Infallible";
                                              Ty.path "core::fmt::Error"
                                            ]
                                        ],
                                        "from_residual",
                                        [],
                                        []
                                      |),
                                      [ M.read (| residual |) ]
                                    |)
                                  |)
                                |)
                              |)));
                          fun γ =>
                            ltac:(M.monadic
                              (let γ0_0 :=
                                M.SubPointer.get_struct_tuple_field (|
                                  γ,
                                  "core::ops::control_flow::ControlFlow::Continue",
                                  0
                                |) in
                              let val := M.copy (| Ty.tuple [], γ0_0 |) in
                              M.read (| val |)))
                        ]
                      |) in
                    let~ _ : Ty.tuple [] :=
                      M.read (|
                        M.use
                          (M.alloc (|
                            Ty.tuple [],
                            M.match_operator (|
                              Ty.tuple [],
                              M.alloc (|
                                Ty.apply
                                  (Ty.path "core::iter::adapters::rev::Rev")
                                  []
                                  [ Ty.apply (Ty.path "core::slice::iter::Iter") [] [ Ty.path "u8" ]
                                  ],
                                M.call_closure (|
                                  Ty.apply
                                    (Ty.path "core::iter::adapters::rev::Rev")
                                    []
                                    [
                                      Ty.apply
                                        (Ty.path "core::slice::iter::Iter")
                                        []
                                        [ Ty.path "u8" ]
                                    ],
                                  M.get_trait_method (|
                                    "core::iter::traits::collect::IntoIterator",
                                    Ty.apply
                                      (Ty.path "core::iter::adapters::rev::Rev")
                                      []
                                      [
                                        Ty.apply
                                          (Ty.path "core::slice::iter::Iter")
                                          []
                                          [ Ty.path "u8" ]
                                      ],
                                    [],
                                    [],
                                    "into_iter",
                                    [],
                                    []
                                  |),
                                  [
                                    M.call_closure (|
                                      Ty.apply
                                        (Ty.path "core::iter::adapters::rev::Rev")
                                        []
                                        [
                                          Ty.apply
                                            (Ty.path "core::slice::iter::Iter")
                                            []
                                            [ Ty.path "u8" ]
                                        ],
                                      M.get_trait_method (|
                                        "core::iter::traits::iterator::Iterator",
                                        Ty.apply
                                          (Ty.path "core::slice::iter::Iter")
                                          []
                                          [ Ty.path "u8" ],
                                        [],
                                        [],
                                        "rev",
                                        [],
                                        []
                                      |),
                                      [
                                        M.call_closure (|
                                          Ty.apply
                                            (Ty.path "core::slice::iter::Iter")
                                            []
                                            [ Ty.path "u8" ],
                                          M.get_associated_function (|
                                            Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ],
                                            "iter",
                                            [],
                                            []
                                          |),
                                          [
                                            M.borrow (|
                                              Pointer.Kind.Ref,
                                              M.deref (|
                                                M.call_closure (|
                                                  Ty.apply
                                                    (Ty.path "&")
                                                    []
                                                    [ Ty.apply (Ty.path "slice") [] [ Ty.path "u8" ]
                                                    ],
                                                  M.get_trait_method (|
                                                    "core::ops::index::Index",
                                                    Ty.apply
                                                      (Ty.path "array")
                                                      [ Value.Integer IntegerKind.Usize 3 ]
                                                      [ Ty.path "u8" ],
                                                    [],
                                                    [
                                                      Ty.apply
                                                        (Ty.path "core::ops::range::RangeTo")
                                                        []
                                                        [ Ty.path "usize" ]
                                                    ],
                                                    "index",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    M.borrow (|
                                                      Pointer.Kind.Ref,
                                                      M.SubPointer.get_struct_record_field (|
                                                        M.deref (| M.read (| self |) |),
                                                        "core::num::bignum::tests::Big8x3",
                                                        "base"
                                                      |)
                                                    |);
                                                    Value.mkStructRecord
                                                      "core::ops::range::RangeTo"
                                                      []
                                                      [ Ty.path "usize" ]
                                                      [
                                                        ("end_",
                                                          M.call_closure (|
                                                            Ty.path "usize",
                                                            BinOp.Wrap.sub,
                                                            [
                                                              M.read (| sz |);
                                                              Value.Integer IntegerKind.Usize 1
                                                            ]
                                                          |))
                                                      ]
                                                  ]
                                                |)
                                              |)
                                            |)
                                          ]
                                        |)
                                      ]
                                    |)
                                  ]
                                |)
                              |),
                              [
                                fun γ =>
                                  ltac:(M.monadic
                                    (let~ iter :
                                        Ty.apply
                                          (Ty.path "core::iter::adapters::rev::Rev")
                                          []
                                          [
                                            Ty.apply
                                              (Ty.path "core::slice::iter::Iter")
                                              []
                                              [ Ty.path "u8" ]
                                          ] :=
                                      M.read (| γ |) in
                                    M.read (|
                                      M.loop (|
                                        Ty.tuple [],
                                        ltac:(M.monadic
                                          (let~ _ : Ty.tuple [] :=
                                            M.match_operator (|
                                              Ty.tuple [],
                                              M.alloc (|
                                                Ty.apply
                                                  (Ty.path "core::option::Option")
                                                  []
                                                  [ Ty.apply (Ty.path "&") [] [ Ty.path "u8" ] ],
                                                M.call_closure (|
                                                  Ty.apply
                                                    (Ty.path "core::option::Option")
                                                    []
                                                    [ Ty.apply (Ty.path "&") [] [ Ty.path "u8" ] ],
                                                  M.get_trait_method (|
                                                    "core::iter::traits::iterator::Iterator",
                                                    Ty.apply
                                                      (Ty.path "core::iter::adapters::rev::Rev")
                                                      []
                                                      [
                                                        Ty.apply
                                                          (Ty.path "core::slice::iter::Iter")
                                                          []
                                                          [ Ty.path "u8" ]
                                                      ],
                                                    [],
                                                    [],
                                                    "next",
                                                    [],
                                                    []
                                                  |),
                                                  [
                                                    M.borrow (|
                                                      Pointer.Kind.MutRef,
                                                      M.deref (|
                                                        M.borrow (| Pointer.Kind.MutRef, iter |)
                                                      |)
                                                    |)
                                                  ]
                                                |)
                                              |),
                                              [
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let _ :=
                                                      M.is_struct_tuple (|
                                                        γ,
                                                        "core::option::Option::None"
                                                      |) in
                                                    M.never_to_any (|
                                                      M.read (| M.break (||) |)
                                                    |)));
                                                fun γ =>
                                                  ltac:(M.monadic
                                                    (let γ0_0 :=
                                                      M.SubPointer.get_struct_tuple_field (|
                                                        γ,
                                                        "core::option::Option::Some",
                                                        0
                                                      |) in
                                                    let γ0_0 := M.deref (| M.read (| γ0_0 |) |) in
                                                    let v := M.copy (| Ty.path "u8", γ0_0 |) in
                                                    M.read (|
                                                      let~ _ : Ty.tuple [] :=
                                                        M.match_operator (|
                                                          Ty.tuple [],
                                                          M.alloc (|
                                                            Ty.apply
                                                              (Ty.path
                                                                "core::ops::control_flow::ControlFlow")
                                                              []
                                                              [
                                                                Ty.apply
                                                                  (Ty.path "core::result::Result")
                                                                  []
                                                                  [
                                                                    Ty.path
                                                                      "core::convert::Infallible";
                                                                    Ty.path "core::fmt::Error"
                                                                  ];
                                                                Ty.tuple []
                                                              ],
                                                            M.call_closure (|
                                                              Ty.apply
                                                                (Ty.path
                                                                  "core::ops::control_flow::ControlFlow")
                                                                []
                                                                [
                                                                  Ty.apply
                                                                    (Ty.path "core::result::Result")
                                                                    []
                                                                    [
                                                                      Ty.path
                                                                        "core::convert::Infallible";
                                                                      Ty.path "core::fmt::Error"
                                                                    ];
                                                                  Ty.tuple []
                                                                ],
                                                              M.get_trait_method (|
                                                                "core::ops::try_trait::Try",
                                                                Ty.apply
                                                                  (Ty.path "core::result::Result")
                                                                  []
                                                                  [
                                                                    Ty.tuple [];
                                                                    Ty.path "core::fmt::Error"
                                                                  ],
                                                                [],
                                                                [],
                                                                "branch",
                                                                [],
                                                                []
                                                              |),
                                                              [
                                                                M.call_closure (|
                                                                  Ty.apply
                                                                    (Ty.path "core::result::Result")
                                                                    []
                                                                    [
                                                                      Ty.tuple [];
                                                                      Ty.path "core::fmt::Error"
                                                                    ],
                                                                  M.get_associated_function (|
                                                                    Ty.path "core::fmt::Formatter",
                                                                    "write_fmt",
                                                                    [],
                                                                    []
                                                                  |),
                                                                  [
                                                                    M.borrow (|
                                                                      Pointer.Kind.MutRef,
                                                                      M.deref (| M.read (| f |) |)
                                                                    |);
                                                                    M.read (|
                                                                      let~ args :
                                                                          Ty.tuple
                                                                            [
                                                                              Ty.apply
                                                                                (Ty.path "&")
                                                                                []
                                                                                [ Ty.path "u8" ];
                                                                              Ty.apply
                                                                                (Ty.path "&")
                                                                                []
                                                                                [ Ty.path "usize" ]
                                                                            ] :=
                                                                        Value.Tuple
                                                                          [
                                                                            M.borrow (|
                                                                              Pointer.Kind.Ref,
                                                                              v
                                                                            |);
                                                                            M.borrow (|
                                                                              Pointer.Kind.Ref,
                                                                              digitlen
                                                                            |)
                                                                          ] in
                                                                      let~ args :
                                                                          Ty.apply
                                                                            (Ty.path "array")
                                                                            [
                                                                              Value.Integer
                                                                                IntegerKind.Usize
                                                                                2
                                                                            ]
                                                                            [
                                                                              Ty.path
                                                                                "core::fmt::rt::Argument"
                                                                            ] :=
                                                                        Value.Array
                                                                          [
                                                                            M.call_closure (|
                                                                              Ty.path
                                                                                "core::fmt::rt::Argument",
                                                                              M.get_associated_function (|
                                                                                Ty.path
                                                                                  "core::fmt::rt::Argument",
                                                                                "new_lower_hex",
                                                                                [],
                                                                                [ Ty.path "u8" ]
                                                                              |),
                                                                              [
                                                                                M.borrow (|
                                                                                  Pointer.Kind.Ref,
                                                                                  M.deref (|
                                                                                    M.read (|
                                                                                      M.SubPointer.get_tuple_field (|
                                                                                        args,
                                                                                        0
                                                                                      |)
                                                                                    |)
                                                                                  |)
                                                                                |)
                                                                              ]
                                                                            |);
                                                                            M.call_closure (|
                                                                              Ty.path
                                                                                "core::fmt::rt::Argument",
                                                                              M.get_associated_function (|
                                                                                Ty.path
                                                                                  "core::fmt::rt::Argument",
                                                                                "from_usize",
                                                                                [],
                                                                                []
                                                                              |),
                                                                              [
                                                                                M.borrow (|
                                                                                  Pointer.Kind.Ref,
                                                                                  M.deref (|
                                                                                    M.read (|
                                                                                      M.SubPointer.get_tuple_field (|
                                                                                        args,
                                                                                        1
                                                                                      |)
                                                                                    |)
                                                                                  |)
                                                                                |)
                                                                              ]
                                                                            |)
                                                                          ] in
                                                                      M.alloc (|
                                                                        Ty.path
                                                                          "core::fmt::Arguments",
                                                                        M.call_closure (|
                                                                          Ty.path
                                                                            "core::fmt::Arguments",
                                                                          M.get_associated_function (|
                                                                            Ty.path
                                                                              "core::fmt::Arguments",
                                                                            "new",
                                                                            [
                                                                              Value.Integer
                                                                                IntegerKind.Usize
                                                                                10;
                                                                              Value.Integer
                                                                                IntegerKind.Usize
                                                                                2
                                                                            ],
                                                                            []
                                                                          |),
                                                                          [
                                                                            M.borrow (|
                                                                              Pointer.Kind.Ref,
                                                                              M.deref (|
                                                                                M.mk_byte_str_ref
                                                                                  10
                                                                                  [
                                                                                    1;
                                                                                    95;
                                                                                    211;
                                                                                    32;
                                                                                    0;
                                                                                    0;
                                                                                    105;
                                                                                    1;
                                                                                    0;
                                                                                    0
                                                                                  ]
                                                                              |)
                                                                            |);
                                                                            M.borrow (|
                                                                              Pointer.Kind.Ref,
                                                                              M.deref (|
                                                                                M.borrow (|
                                                                                  Pointer.Kind.Ref,
                                                                                  args
                                                                                |)
                                                                              |)
                                                                            |)
                                                                          ]
                                                                        |)
                                                                      |)
                                                                    |)
                                                                  ]
                                                                |)
                                                              ]
                                                            |)
                                                          |),
                                                          [
                                                            fun γ =>
                                                              ltac:(M.monadic
                                                                (let γ0_0 :=
                                                                  M.SubPointer.get_struct_tuple_field (|
                                                                    γ,
                                                                    "core::ops::control_flow::ControlFlow::Break",
                                                                    0
                                                                  |) in
                                                                let residual :=
                                                                  M.copy (|
                                                                    Ty.apply
                                                                      (Ty.path
                                                                        "core::result::Result")
                                                                      []
                                                                      [
                                                                        Ty.path
                                                                          "core::convert::Infallible";
                                                                        Ty.path "core::fmt::Error"
                                                                      ],
                                                                    γ0_0
                                                                  |) in
                                                                M.never_to_any (|
                                                                  M.read (|
                                                                    M.return_ (|
                                                                      M.call_closure (|
                                                                        Ty.apply
                                                                          (Ty.path
                                                                            "core::result::Result")
                                                                          []
                                                                          [
                                                                            Ty.tuple [];
                                                                            Ty.path
                                                                              "core::fmt::Error"
                                                                          ],
                                                                        M.get_trait_method (|
                                                                          "core::ops::try_trait::FromResidual",
                                                                          Ty.apply
                                                                            (Ty.path
                                                                              "core::result::Result")
                                                                            []
                                                                            [
                                                                              Ty.tuple [];
                                                                              Ty.path
                                                                                "core::fmt::Error"
                                                                            ],
                                                                          [],
                                                                          [
                                                                            Ty.apply
                                                                              (Ty.path
                                                                                "core::result::Result")
                                                                              []
                                                                              [
                                                                                Ty.path
                                                                                  "core::convert::Infallible";
                                                                                Ty.path
                                                                                  "core::fmt::Error"
                                                                              ]
                                                                          ],
                                                                          "from_residual",
                                                                          [],
                                                                          []
                                                                        |),
                                                                        [ M.read (| residual |) ]
                                                                      |)
                                                                    |)
                                                                  |)
                                                                |)));
                                                            fun γ =>
                                                              ltac:(M.monadic
                                                                (let γ0_0 :=
                                                                  M.SubPointer.get_struct_tuple_field (|
                                                                    γ,
                                                                    "core::ops::control_flow::ControlFlow::Continue",
                                                                    0
                                                                  |) in
                                                                let val :=
                                                                  M.copy (| Ty.tuple [], γ0_0 |) in
                                                                M.read (| val |)))
                                                          ]
                                                        |) in
                                                      M.alloc (| Ty.tuple [], Value.Tuple [] |)
                                                    |)))
                                              ]
                                            |) in
                                          M.alloc (| Ty.tuple [], Value.Tuple [] |)))
                                      |)
                                    |)))
                              ]
                            |)
                          |))
                      |) in
                    M.alloc (|
                      Ty.apply
                        (Ty.path "core::result::Result")
                        []
                        [ Ty.tuple []; Ty.path "core::fmt::Error" ],
                      Value.StructTuple
                        "core::result::Result::Ok"
                        []
                        [ Ty.tuple []; Ty.path "core::fmt::Error" ]
                        [ Value.Tuple [] ]
                    |)
                  |)))
              |)))
          | _, _, _ => M.impossible "wrong number of arguments"
          end.
        
        Axiom Implements :
          M.IsTraitInstance
            "core::fmt::Debug"
            (* Trait polymorphic consts *) []
            (* Trait polymorphic types *) []
            Self
            (* Instance *) [ ("fmt", InstanceField.Method fmt) ].
      End Impl_core_fmt_Debug_for_core_num_bignum_tests_Big8x3.
    End tests.
  End bignum.
End num.
