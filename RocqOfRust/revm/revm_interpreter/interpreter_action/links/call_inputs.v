...
Module CallInputs.
  ...
  Instance IsLink : Link t := {
    ...
    φ x :=
      Value.StructRecord "revm_interpreter::interpreter_action::call_inputs::CallInputs" [] [] [
        ...
        ("known_bytecode", φ x.(known_bytecode));
  ...
  Instance IsOfValueWith
      ...
      (input' : Value.t) {H_input : OfValueWith.C (CallInput.t) input'}
      (is_static' : Value.t) {H_is_static : OfValueWith.C (bool) is_static'}
      (known_bytecode' : Value.t) {H_known_bytecode : OfValueWith.C (option (aliases.B256.t * Bytecode.t)) known_bytecode'}
    ...
    ]) :=
  {
    value := Build_t
      ...
      H_known_bytecode.(OfValueWith.value)
...
;
  ...
  Module SubPointer.
    ...
    Definition get_known_bytecode : SubPointer.Runner.t t
      (Pointer.Index.StructRecord "revm_interpreter::interpreter_action::call_inputs::CallInputs" "known_bytecode") :=
    {|
      SubPointer.Runner.projection x := Some x.(known_bytecode);
      SubPointer.Runner.injection x y := Some (x <| known_bytecode := y |>);
    |}.

    Lemma get_known_bytecode_is_valid :
      SubPointer.Runner.Valid.t get_known_bytecode.
    Proof.
      now constructor.
    Qed.
    Smpl Add apply get_known_bytecode_is_valid : run_sub_pointer.
