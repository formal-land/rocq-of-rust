...
fn setup_plugin() {
    ...
    let rocq_of_rust: RocqOfRustArgs = if is_wrapper {
        serde_json::from_str(&std::env::var("ROCQ_OF_RUST_ARGS").unwrap()).unwrap()
    } else {
        let all_args = Args::parse_from(&args);
        args = all_args.rust_flags;
        all_args.rocq_of_rust
    };
    ...
    // Did the user ask to compile this crate? Either they explicitly invoked `rocq_of_rust-rustc`
