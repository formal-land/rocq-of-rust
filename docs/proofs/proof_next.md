...
2. Writing simulations of the Rust code, made by hand and in idiomatic Rocq, optimized for the proof.
...
> Ongoing work in the pull request [#562](https://github.com/formal-land/rocq-of-rust/pull/562)
...
We put everything in `links` folders following the names of the original files. We define for each Rust type a corresponding Rocq type with a `to_ty` and `to_value` function. We make sure that there is enough information in the `Value.t` type so that we can come back to our Rocq type in a unique way. This will simplify the design of a `of_value` tactic to go from `Value.t` to our Rocq type.

In particular, we add back the tags for the integer values, to distinguish between the various kinds of integers (`u8`, `i32`, ...). We also add tags for the pointers, to distinguish between the various kinds of pointers (`&`, `&mut`, ...). The tags are necessary to define the `to_ty` functions as the Rust types of the various kinds of integers and pointers are different, even if we want to represent them in the same way in Rocq, and this information can be used on the Rust side to know which trait instance to call for example.
...
We write the simulations in the same way as before. The idea is to have idiomatic Rocq code that follows the structure of the original code but is written in a way that will simplify the proofs.
...
Another optimization we can hopefully do is to have `Box` pointers that really "owns" their value, so that we can actually put their values on the stack in the Rocq semantics. We might be able to totally avoid having a heap in the semantics of programs using only `&`, `&mut`, `Box`, and `Rc` pointers (we will add interior mutability later).
